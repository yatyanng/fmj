 ...
 *
 */
public class PullDataSourceCallbackURLProtocolHandler implements CallbackURLProtocolHandler {
	...
	private void doOpen() throws IOException {
		...
		if (mustClone) {
			if (!isSourceCloneable) {
				logger.severe(
						"PullSourceStreamCallbackURLProtocolHandler: cannot reopen because source is not SourceCloneable");
	...
	public long seek(URLContext h, long pos, int whence) {
		...
		try {
			if (!isSeekable) {
				...
				if (whence == SEEK_SET) {
					if (pss.getContentLength() > 0 && pos >= pss.getContentLength()) {
						if (TRACE)
					...
					} else if (pos >= curpos) { // absolute seek to after our position: simply do a
												// SEEK_CUR below.
						whence = SEEK_CUR;
						pos -= curpos;
					} else {
				...
				if (whence == SEEK_CUR) { // implement seek by repeated read:
