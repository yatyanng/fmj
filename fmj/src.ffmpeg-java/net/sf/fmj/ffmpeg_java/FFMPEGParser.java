...
public class FFMPEGParser extends AbstractDemultiplexer {
	private class AudioTrack extends PullSourceStreamTrack {
		...
		public AudioTrack(int audioStreamIndex, AVStream stream, AVCodecContext codecCtx)
			...
			synchronized (AV_SYNC_OBJ) {
				...
				if (codec == null)
					throw new ResourceUnavailableException("Codec not found for codec_id " + codecCtx.codec_id + " (0x"
							+ Integer.toHexString(codecCtx.codec_id) + ")"); // Codec not found - see
			...
			if (packet != null) {
				synchronized (AV_SYNC_OBJ) {
					...
					if (frameSize.getValue() < 0) {
						...
						if (frameSize.getValue() > this.bufferSize) { // realloc buffer to make room:
	...
	private class VideoTrack extends PullSourceStreamTrack {
		...
		/**
		 * We have to keep track of frame number ourselves. frame.display_picture_number
		 * seems to often always be zero. See: http:
		 * //lists.mplayerhq.hu/pipermail/ffmpeg-user/2005-September/001244.html
		 */
		private long frameNo;

		public VideoTrack(int videoStreamIndex, AVStream stream, AVCodecContext codecCtx)
				throws ResourceUnavailableException {
			super();
			...
			synchronized (AV_SYNC_OBJ) {
				...
				format = ListFormats.convertCodecPixelFormat(dstPixFmt, codecCtx.width, codecCtx.height, frameRate);
				if (format == null) {
				...
				final int numBytes = AVCODEC.avpicture_get_size(dstPixFmt, codecCtx.width, codecCtx.height);
				buffer = AVUTIL.av_malloc(numBytes);

				// Assign appropriate parts of buffer to image planes in
				// dstFrame
				AVCODEC.avpicture_fill(dstFrame, buffer, dstPixFmt, codecCtx.width, codecCtx.height);
			}
		}
			...
			if (packet != null) {
				synchronized (AV_SYNC_OBJ) {
					...
					if (frameFinished.getValue() != 0) {
						int res = imageConverter.img_convert(dstFrame, dstPixFmt, srcFrame, codecCtx.pix_fmt,
						...
						buffer.setEOM(false);
						buffer.setDiscard(false);
						buffer.setTimeStamp(getTimestamp(srcFrame, stream, codecCtx, frameNo++, dts));
	...
	static final String[] FLIC_MIMETYPES = { "video/fli", "video/flc", "video/x-fli", "video/x-flc" };
	static final String[] MATROSKA_MIMETYPES = { "video/x-matroska", "audio/x-matroska" };
	static final String[] MJPEG_MIMETYPE = { "video/x-motion-jpeg" };
	static final String[] MOV_MP4_M4A_3GP_3G2_MJ2_MIMETYPES = { "video/quicktime", "video/mp4", "video/3gpp",
			"video/mj2" };
	...
	public static AudioFormat convertCodecAudioFormat(AVCodecContext codecCtx) {
		...
		case AVCodecLibrary.CODEC_ID_AC3:
			...
			break;
		/*
		 * TODO: add DTS to audioformat case AVCodecLibrary.CODEC_ID_DTS: result = new
		 * AudioFormat(AudioFormat.DTS, codecCtx.sample_rate, 16, codecCtx.channels,
	...
	static long getTimestamp(final AVFrame frame, final AVStream stream, final AVCodecContext codecCtx, long frameNo,
			long packetDts) {
		// from AVFrame, regarding int64_t pts:
		/**
		 * presentation timestamp in time_base units (time when frame should be shown to
		 * user) If AV_NOPTS_VALUE then frame_rate = 1/time_base will be assumed.
		 */
		 ...
		 * 1/framerate and timestamp increments should be identically 1.
		 */
		 ...
		 *
		 * // TODO: the frame rate is in frames, where half of an interlaced frame
		 * counts as 1. so for interlaced video, // this has to be taken into account.
		 * // for example safexmas.move is reported as : // Duration: 00:00:16.4, start:
		 * 0.000000, bitrate: 1730 kb/s // Stream #0.0(eng): Video: cinepak, yuv420p,
		 * 320x200, 30.00 fps(r) // and it has 220 frames. But 220/16.4=13.4. // see
		 * http://www.dranger.com/ffmpeg/tutorial05.html // for a good discussion on
		 * pts. // TODO: for now, we'll just use the packetDts, since pts seems to
		 * always be zero.
		 *
		 * if (packetDts == AVCodecLibrary.AV_NOPTS_VALUE) // TODO: with some movies,
		 * pts is just always zero, so we'll handle it the same way. { // If
		 * AV_NOPTS_VALUE then frame_rate = 1/time_base will be assumed. // therefore we
		 * need to know the frame # return (1000000000L * frameNo * (long)
		 * time_base.num) / (long) time_base.den;
		 *
		 * } else { // TODO: the code to do the calculation based on the dts is wrong,
		 * so we'll just use the frame number based // calculation for now. // not sure
		 * how to calculate the correct dts for a frame. // try 4harmonic.mpg for an
		 * example of this. return (1000000000L * frameNo * (long) time_base.num) /
		 * (long) time_base.den; //return ( 1000000000L * packetDts * (long)
		 * time_base.num) / (long) time_base.den; // TODO: is this correct? it appears
		 * to be based on the AVFrame comment, but has not been tested yet.
		 *
		 *
		 * }
		 */
		...
		if (packetDts != AVCodecLibrary.AV_NOPTS_VALUE) {
			pts = packetDts;
		} else if (frame.opaque != null && frame.opaque.getInt(0) != AVCodecLibrary.AV_NOPTS_VALUE) {
		...
		/*
		 * double frame_delay = codecCtx.time_base.num / (double)codecCtx.time_base.den;
	...
	public void open() throws ResourceUnavailableException {
		synchronized (AV_SYNC_OBJ) {
			...
			if (AVCODEC.avcodec_version() != AVCodecLibrary.LIBAVCODEC_VERSION_INT)
				logger.warning(
						"ffmpeg-java and ffmpeg versions do not match: avcodec_version=" + AVCODEC.avcodec_version()
								+ " LIBAVCODEC_VERSION_INT=" + AVCodecLibrary.LIBAVCODEC_VERSION_INT);
			...
			} else { // use the real java datasource, via callbacks.
				CallbackURLProtocolMgr.register(AVFORMAT);

				// TODO: do this in start?
				final String callbackURL = CallbackURLProtocolMgr
						.addCallbackURLProtocolHandler(new PullDataSourceCallbackURLProtocolHandler(source));
			...
			if (ret != 0) {
				throw new ResourceUnavailableException("av_open_input_file failed: " + ret); // Couldn't open
																								// file
			}
			...
			if (AVFORMAT.av_find_stream_info(formatCtx) < 0)
				throw new ResourceUnavailableException("Couldn't find stream information"); // Couldn't find
																							// stream
																							// information
	...
	protected void queryInputContentDescriptors() {
		Map<String, String[]> additionalFormatMimeTypes = new HashMap<String, String[]>();
		...
		additionalFormatMimeTypes.put("mm", MM_MIMETYPE); // Format of american
															// laser games
		additionalFormatMimeTypes.put("mov,mp4,m4a,3gp,3g2,mj2", MOV_MP4_M4A_3GP_3G2_MJ2_MIMETYPES); // QuickTime/MPEG4/Motion
																										// JPEG 2000
																										// format
		...
		Map<String, ContentDescriptor> mimeTypes = new HashMap<String, ContentDescriptor>();
		int i = 1;
		...
		while (avInputFormat != null) {
			...
			String[] additionalMimeTypes = additionalFormatMimeTypes.get(avInputFormat.name);
			if (additionalMimeTypes != null) {
				if (additionalMimeTypes == NOT_SUPPORTED_FORMAT) {
					logger.log(Level.FINE,
							"Ignoring input format: " + avInputFormat.name + " (" + avInputFormat.long_name + ")");
				} else {
					for (int j = 0; j < additionalMimeTypes.length; j++) {
						mimeType = additionalMimeTypes[j];
						mimeTypes.put(mimeType,
								new ContentDescriptor(ContentDescriptor.mimeTypeToPackageName(mimeType)));
		...
		supportedInputContentDescriptors = mimeTypes.values().toArray(new ContentDescriptor[0]);
	}
