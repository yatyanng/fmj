...
public class ListFormats {
	...
	public static AudioFormat convertCodecAudioFormat(AVCodecContext codecCtx) {
		...
		// of the source.
		return new AudioFormat(AudioFormat.LINEAR, codecCtx.sample_rate, 16, codecCtx.channels); // / TODO: endian,
																									// signed?
	...
	public static VideoFormat convertCodecPixelFormat(int pixFmt, int width, int height, float frameRate) {
		...
		case FFMPEGLibrary.PIX_FMT_RGB24: // /< Packed RGB 8:8:8, 24bpp,
			...
			blue = 3;
			bitsPerPixel = 24;
			result = new RGBFormat(new Dimension(width, height), -1, Format.byteArray, frameRate, bitsPerPixel, red,
					green, blue);
			break;
		...
		case FFMPEGLibrary.PIX_FMT_RGB32: // /< Packed RGB 8:8:8, 32bpp, (msb)8A
			...
			result = new RGBFormat(new Dimension(width, height), -1, Format.intArray, frameRate, bitsPerPixel, red,
					green, blue, pixelStride, lineStride, Format.FALSE, endianess);
			break;
		case FFMPEGLibrary.PIX_FMT_RGB565: // /< Packed RGB 5:6:5, 16bpp, (msb)
			...
			pixelStride = 1;
			lineStride = width * pixelStride;
			result = new RGBFormat(new Dimension(width, height), -1, Format.shortArray, frameRate, bitsPerPixel, red,
					green, blue, pixelStride, lineStride, Format.FALSE, endianess);
			break;
		 ...
		 * TODO: mask arrays result = new IndexedColorFormat(new Dimension(width,
		 * height), -1, Format.byteArray, frameRate, width, 256, new byte[256], new
		 * byte[256], new byte[256]); break;
		 */
		/*
		 * case AVCodecLibrary.PIX_FMT_YUV420P: ///< Planar YUV 4:2:0, 12bpp, (1 Cr & Cb
		 * sample per 2x2 Y samples) // TODO: test it result = new YUVFormat(new
		 * Dimension(width*2, height*2), -1, Format.byteArray, frameRate,
		 * YUVFormat.YUV_420, -1, -1, -1, -1, -1); break;
		 */
		/*
		 * case AVCodecLibrary.PIX_FMT_YUV422P: ///< Planar YUV 4:2:2, 16bpp, (1 Cr & Cb
		 * sample per 2x1 Y samples) // TODO: test it result = new YUVFormat(new
		 * Dimension(width, height), -1, Format.byteArray, frameRate, YUVFormat.YUV_422,
		 * -1, -1, -1, -1, -1); break; case AVCodecLibrary.PIX_FMT_YUV411P: ///< Planar
		 * YUV 4:1:1, 12bpp, (1 Cr & Cb sample per 4x1 Y samples) // TODO: test it
		 * result = new YUVFormat(YUVFormat.YUV_411); break; case
		 * AVCodecLibrary.PIX_FMT_YUVJ420P: ///< Planar YUV 4:2:0, 12bpp, full scale
		 * (jpeg) // TODO: test it result = new JPEGFormat(new Dimension(width, height),
		 * -1, Format.byteArray, frameRate, JPEG_QUALITY, JPEGFormat.DEC_420); break;
		 * case AVCodecLibrary.PIX_FMT_YUVJ422P: ///< Planar YUV 4:2:2, 16bpp, full
		 * scale (jpeg) // TODO: test it result = new JPEGFormat(new Dimension(width,
		 * height), -1, Format.byteArray, frameRate, JPEG_QUALITY, JPEGFormat.DEC_422);
		 * break; case AVCodecLibrary.PIX_FMT_YUVJ444P: ///< Planar YUV 4:4:4, 24bpp,
		 * full scale (jpeg) // TODO: test it result = new JPEGFormat(new
		 * Dimension(width, height), -1, Format.byteArray, frameRate, JPEG_QUALITY,
		 * JPEGFormat.DEC_444); break;
		 */
	...
	public static int getPixelFormatFromBufferedImageType(int bufferedImageType) {
		...
		case BufferedImage.TYPE_BYTE_BINARY:
			 ...
			 * AVCodecLibrary.PIX_FMT_MONOWHITE; pixelFormat = AVCodecLibrary.PIX_FMT_BGR4;
	...
	static List<String> listMuxes() {
		...
		while (avOutputFormat != null) {
			...
			logger.log(Level.FINEST,
					i++ + ". " + avOutputFormat.name + " - " + avOutputFormat.long_name + " : " + mimeType);

			contentDescriptors.add(ContentDescriptor.mimeTypeToPackageName(mimeType));

			avOutputFormat = (avOutputFormat.next != null /* && avOutputFormat.next.isValid() */)
