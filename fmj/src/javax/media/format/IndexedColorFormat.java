...
public class IndexedColorFormat extends VideoFormat {
	...
	public IndexedColorFormat(java.awt.Dimension size, int maxDataLength, Class<?> dataType, float frameRate,
			int lineStride, int mapSize, byte[] red, byte[] green, byte[] blue) {
	...
	@Override
	public Object clone() {
		return new IndexedColorFormat(FormatUtils.clone(size), maxDataLength, dataType, frameRate, lineStride, mapSize,
	...
	public boolean equals(Object format) {
		...
		return this.lineStride == oCast.lineStride && this.mapSize == oCast.mapSize && this.redValues == oCast.redValues
				&& // strange, compare arrays using ==. This makes it so that
					// serializing and deserializing this obj results in an obj
					// that is not "equal"
				this.greenValues == oCast.greenValues && this.blueValues == oCast.blueValues;
	}
	...
	public boolean matches(Format format) {
		...
		final boolean result = FormatUtils.matches(oCast.lineStride, this.lineStride)
