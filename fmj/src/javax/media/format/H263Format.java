...
public class H263Format extends VideoFormat {
	...
	public H263Format(java.awt.Dimension size, int maxDataLength, Class<?> dataType, float frameRate,
			int advancedPrediction, int arithmeticCoding, int errorCompensation, int hrDB, int pbFrames,
	...
	@Override
	public Object clone() {
		return new H263Format(FormatUtils.clone(size), maxDataLength, dataType, frameRate, advancedPrediction,
				arithmeticCoding, errorCompensation, hrDB, pbFrames, unrestrictedVector);
	}
	...
	public boolean equals(Object format) {
		...
		return this.advancedPrediction == oCast.advancedPrediction && this.arithmeticCoding == oCast.arithmeticCoding
				&& this.errorCompensation == oCast.errorCompensation && this.hrDB == oCast.hrDB
				&& this.pbFrames == oCast.pbFrames && this.unrestrictedVector == oCast.unrestrictedVector;
	}
	...
	public boolean matches(Format format) {
		...
		final boolean result = FormatUtils.matches(this.advancedPrediction, oCast.advancedPrediction)
				&& FormatUtils.matches(this.arithmeticCoding, oCast.arithmeticCoding)
				&& FormatUtils.matches(this.errorCompensation, oCast.errorCompensation)
				&& FormatUtils.matches(this.hrDB, oCast.hrDB) && FormatUtils.matches(this.pbFrames, oCast.pbFrames)
