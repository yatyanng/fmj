...
public class YUVFormat extends VideoFormat {
	...
	/**
	 * YUV Planar YVU9 type. Contains a Y value for every pixel and U and V values
	 * for every 4x4 block of pixels.
	 */
	public static final int YUV_YVU9 = 16;
	/**
	 * YUV 4:2:2 interleaved format. The components are ordered as specified by the
	 * offsetY, offsetU and offsetV attributes. For example, if the ordering is Y,
	 * V, Y and U, the offsets would be offsetY=0;offsetU=3;offsetV=1. The position
	 * of the second Y is implied. Y pixel stride is assumed to be 2 and the U and V
	 * pixel strides are assumed to be 4.
	 */
	...
	/**
	 * When the YUV data is in planar format, specifies the offset into the data for
	 * the Y plane. This value is ignored in the interleaved formats.
	 */
	protected int offsetY = NOT_SPECIFIED;
	/**
	 * When the YUV data is in planar format, specifies the offset into the data for
	 * the U plane. This value is ignored in the interleaved formats.
	 */
	protected int offsetU = NOT_SPECIFIED;
	/**
	 * When the YUV data is in planar format, specifies the offset into the data for
	 * the V plane. This value is ignored in the interleaved formats.
	 */
	 ...
	 *                      component in a row and the first Y component in the next
	 *                      row.
	 * @param strideUV      The number of data elements between the first U
	 *                      component in a row and the first U component in the next
	 *                      row. The same value is expected for the V component.
	 ...
	 */
	public YUVFormat(Dimension size, int maxDataLength, Class<?> dataType, float frameRate, int yuvType, int strideY,
	...
	@Override
	public Object clone() {
		YUVFormat f = new YUVFormat(size, maxDataLength, dataType, frameRate, yuvType, strideY, strideUV, offsetY,
				offsetU, offsetV);
		f.copy(this);
		return f;
	}
	...
	public boolean equals(Object format) {
		if (format instanceof YUVFormat) {
			...
			return super.equals(format) && yuvType == other.yuvType && strideY == other.strideY
					&& strideUV == other.strideUV && offsetY == other.offsetY && offsetU == other.offsetU
					&& offsetV == other.offsetV;
		} else
			return false;
	}
	...
	public boolean matches(Format format) {
		...
		return (yuvType == NOT_SPECIFIED || other.yuvType == NOT_SPECIFIED || yuvType == other.yuvType) &&
				...
				&& (strideUV == NOT_SPECIFIED || other.strideUV == NOT_SPECIFIED || strideUV == other.strideUV) &&
	...
	public String toString() {
		return "YUV Video Format: Size = " + size + " MaxDataLength = " + maxDataLength + " DataType = " + dataType
				+ " yuvType = " + yuvType + " StrideY = " + strideY + " StrideUV = " + strideUV + " OffsetY = "
				+ offsetY + " OffsetU = " + offsetU + " OffsetV = " + offsetV + "\n";
	}
}
