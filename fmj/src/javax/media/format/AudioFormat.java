...
public class AudioFormat extends Format {
	 ...
	 *
	 * @param encoding         A <tt>String</tt> that describes the encoding type
	 *                         for this <tt>AudioFormat</tt>.
	 * @param sampleRate       The sample rate.
	 * @param sampleSizeInBits The sample size in bits.
	 * @param channels         The number of channels as an integer. For example, 1
	 ...
	 *                         <tt>AudioFormat</tt>-- <tt>BIG_ENDIAN</tt> or
	 *                         <tt>LITTLE_ENDIAN</tt>.
	 * @param signed           Indicates whether the samples are stored in a signed
	 *                         or unsigned format. Specify
	 *                         <CITE><tt>true</tt></CITE> if the
	...
	public long computeDuration(long length) {
		...
		} else if (encoding.equalsIgnoreCase(AudioFormat.LINEAR) || encoding.equalsIgnoreCase(AudioFormat.ULAW)) {
	...
	public boolean equals(Object format) {
		if (format instanceof AudioFormat) {
			...
			return super.equals(format) && sampleRate == other.sampleRate && sampleSizeInBits == other.sampleSizeInBits
					&& channels == other.channels && endian == other.endian && signed == other.signed
					&& frameSizeInBits == other.frameSizeInBits && frameRate == other.frameRate;
		}
		return false;
	}
	...
	public boolean matches(Format format) {
		...
		return (sampleRate == NOT_SPECIFIED || other.sampleRate == NOT_SPECIFIED || sampleRate == other.sampleRate)
				&& (sampleSizeInBits == NOT_SPECIFIED || other.sampleSizeInBits == NOT_SPECIFIED
						|| sampleSizeInBits == other.sampleSizeInBits)
				&& (channels == NOT_SPECIFIED || other.channels == NOT_SPECIFIED || channels == other.channels)
				...
				&& (frameSizeInBits == NOT_SPECIFIED || other.frameSizeInBits == NOT_SPECIFIED
						|| frameSizeInBits == other.frameSizeInBits)
				&& (frameRate == NOT_SPECIFIED || other.frameRate == NOT_SPECIFIED || frameRate == other.frameRate);
	}
	...
	public String toString() {
		...
		return getEncoding() + ((sampleRate != NOT_SPECIFIED) ? (", " + sampleRate + " Hz") : ", Unknown Sample Rate")
				+ ((sampleSizeInBits != NOT_SPECIFIED) ? (", " + sampleSizeInBits + "-bit") : "") + strChannels
				+ strEndian + ((signed != NOT_SPECIFIED) ? ((signed == SIGNED ? ", Signed" : ", Unsigned")) : "")
