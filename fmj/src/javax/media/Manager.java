...
public final class Manager {
	private static class BlockingRealizer implements ControllerListener {
		...
		public synchronized void controllerUpdate(ControllerEvent event) {
			if (event instanceof RealizeCompleteEvent) {
				...
				{
					cannotRealizeExceptionMessage = "Cannot realize: received ControllerClosedEvent: " + event
							+ "; message: " + ((ControllerClosedEvent) event).getMessage();
					logger.info(cannotRealizeExceptionMessage);
				}
		...
		for (String handlerClassName : getDataSinkClassList(protocol)) {
			try {
				...
				if (handler instanceof DataSink) {
					...
					Vector<String> handlerClassList2 = getDataSinkClassList(
							protocol + "." + toPackageFriendly(mediaProxy.getContentType(destLocator)));
					...
					for (String handlerClassName2 : handlerClassList2) {
						try {
							...
							logger.fine("createDataSink(" + datasource + ", " + destLocator + "), proxy="
			...
			for (int j = 0; j < outputFormats.length; ++j) {
				...
				for (int i = 0; i < trackControl.length; i++) {
					...
					if (!(trackControl[i] instanceof FormatControl)) {
						logger.warning(
								"Disabling track " + i + "; trackControl is not a FormatControl: " + trackControl[i]);
			...
			for (int j = 0; j < outputFormats.length; ++j) {
				if (!outputFormatUsed[j])
					throw new CannotRealizeException(
							"No tracks found that are compatible with format " + outputFormats[j]); // TODO: others that
																									// weren't used.
	...
	public static Vector<String> getClassList(String contentName, Vector packages, String component2,
			String className) {
	...
	public static Vector<String> getDataSinkClassList(String contentName) {
		return getClassList(toPackageFriendly(contentName), PackageManager.getContentPrefixList(), "datasink",
				"Handler");
	...
	public static Vector<String> getHandlerClassList(String contentName) {
		return getClassList(toPackageFriendly(contentName), PackageManager.getContentPrefixList(), "content",
				"Handler");
