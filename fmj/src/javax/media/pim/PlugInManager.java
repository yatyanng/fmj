...
public class PlugInManager extends javax.media.PlugInManager {
	...
	private static final HashMap<String, PlugInInfo>[] pluginMaps = new HashMap[] { new HashMap<String, PlugInInfo>(),
			new HashMap<String, PlugInInfo>(), new HashMap<String, PlugInInfo>(), new HashMap<String, PlugInInfo>(),
	...
	public static synchronized boolean addPlugIn(String classname, Format[] in, Format[] out, int type) {
		try {
			...
			logger.log(Level.WARNING,
					"Unable to addPlugIn for " + classname + " due to inability to get its class: " + t, t);
			return false;
		}
	...
	public static synchronized boolean removePlugIn(String classname, int type) {
		...
		final boolean result = classList.remove(classname) | (pluginMap.remove(classname) != null); // don't uses
																									// shortcut
																									// || because we
																									// want
																									// to remove from
	...
	/**
	 * Private constructor so that is can not be constructed. In JMF it is public,
	 * but this is an implementation detail that is not important for FMJ
	 * compatibility.
	 */
