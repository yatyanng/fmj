...
public abstract class ThreadedEventQueue extends MediaThread {
	...
	/**
	 * Wait until there is something in the event queue to process. Then dispatch
	 * the event to the listeners.The entire method does not need to be synchronized
	 * since this includes taking the event out from the queue and processing the
	 * event. We only need to provide exclusive access over the code where an event
