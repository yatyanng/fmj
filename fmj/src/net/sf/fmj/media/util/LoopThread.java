...
public abstract class LoopThread extends MediaThread {
	...
	/**
	 * Same as pause except that it blocks until the loop has actually paused (at
	 * waitHereIfPaused).
	 */
	...
	/**
	 * Set the paused state to true. The thread will halt at the beginning of the
	 * loop at the next iteration. i.e., unlike suspend(), the thread will NOT pause
	 * immediately. It will execute until the next waitHereIfPaused() is
	 * encountered.
	 */
	...
	/**
	 * Put the thread in a wait() if the paused state is on. Otherwise, it will just
	 * proceed.
	 */
