...
public class VideoCodecChain extends CodecChain {
	 ...
	 * Otherwise all frames must go to the decoder and the current decoder overloads
	 * the CPU.
	 */
	@Override
	boolean isRawFormat(Format format) {
		...
		return ((format instanceof RGBFormat || format instanceof YUVFormat
				|| (format.getEncoding() != null && (format.getEncoding().equalsIgnoreCase(VideoFormat.JPEG)
