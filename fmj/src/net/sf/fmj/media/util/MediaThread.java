...
public class MediaThread extends Thread {
	...
	static {
		try {
			...
			// System.err.println("Permission to manipulate threads and/or thread groups not
			// granted "
			// + e + " : " + e.getMessage());
	...
	private void checkPriority(String name, int ask, boolean priv, int got) {
		if (ask != got) {
			System.out.println(
					"MediaThread: " + name + " privilege? " + priv + "  ask pri: " + ask + " got pri:  " + got);
		}
	}
			...
			try {
				...
				if ((osName != null) && osName.startsWith("Linux")) {
					...
					if ((javaVmName != null) && javaVmName.equalsIgnoreCase("Dalvik")) {
						...
						androidOsProcess.getMethod("setThreadPriority", Integer.class).invoke(null,
								androidThreadPriority);

						/*
						 * Convert the android.os.Process thread priority to java.lang.Thread priority.
						 */
						int priority = 10 - Math.round(((androidThreadPriority + 20) / (float) 40) * 10);
	 ...
	 * <tt>android.os.Process#setThreadPriority(int)</tt>. Because the Android API
	 * in question pretty much needs to be called from the thread on which the
	 * priority is to be set, this <tt>MediaThread</tt> will attempt to apply the
	 * specified priority when its {@link #run()} method is called. Consequently,
	 * extenders overriding the <tt>run</tt> method will not have the Android thread
	 * priority applied until they call back to the <tt>MediaThread</tt>
	 * implementation.
	 *
	 * @param androidThreadPriority the name of the <tt>android.os.Process</tt>
	 *                              static final field which specifies the thread
	 *                              priority of this <tt>MediaThread</tt> to be set
	 *                              on Android via a call to
	...
	private void usePriority(int priority) {
		...
		if (debug) {
			checkPriority("priority", priority, securityPrivilege, getPriority());
		}
	}
