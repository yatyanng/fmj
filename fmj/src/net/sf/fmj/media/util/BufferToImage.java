...
public class BufferToImage {
	...
	public BufferToImage(VideoFormat format) {
		/*
		 * The code bellow throws a NullPointerException for the sake of compatibility
		 * with JMF. However, such behavior is wrong. Firstly, the format argument is
		 * not used. Secondly, Buffer instances have a format property. Thirdly,
		 * SimpleAWTRenderer#setInputFormat(Format) and
		 * SimpleSwingRenderer#setInputFormat(Format) may invoke the constructor with a
		 * VideoFormat which does not have a size and such an argument is perfectly
		 * valid for them.
		 */
		...
		if (format instanceof RGBFormat) {
			...
			if (dataType == Format.byteArray) {
				...
				if (bitsPerPixel == 24) {
					...
					final ComponentSampleModel sm = new ComponentSampleModel(DataBuffer.TYPE_BYTE, w, h, pixelStride,
							lineStride, new int[] { redMask - 1, greenMask - 1, blueMask - 1 });
					...
					int[] nBits = { 8, 8, 8 };
					// int[] bOffs = {2, 1, 0};
					final ColorModel colorModel = new ComponentColorModel(cs, nBits, false, false, Transparency.OPAQUE,
					...
					final DataBufferByte db = new DataBufferByte(new byte[][] { bytes }, bytes.length);
					final ComponentSampleModel sm = new ComponentSampleModel(DataBuffer.TYPE_BYTE, w, h, pixelStride,
							lineStride, new int[] { redMask - 1, greenMask - 1, blueMask - 1, 3 }); // TODO: what to do
																									// with
																									// alpha?
					final WritableRaster r = Raster.createWritableRaster(sm, db, new Point(0, 0));
					// construction borrowed from BufferedImage constructor, for
					...
					final DataBufferByte db = new DataBufferByte(new byte[][] { bytes }, bytes.length);
					final SampleModel sm = new SinglePixelPackedSampleModel(DataBuffer.TYPE_BYTE, w, h, lineStride,
				...
				if (bitsPerPixel == 16) {
					...
					final SampleModel sm = new SinglePixelPackedSampleModel(DataBuffer.TYPE_USHORT, w, h, lineStride,
							new int[] { redMask, greenMask, blueMask });
					final WritableRaster r = Raster.createWritableRaster(sm, db, new Point(0, 0));
					final ColorModel colorModel = new DirectColorModel(bitsPerPixel, redMask, greenMask, blueMask);
					final BufferedImage bi = new BufferedImage(colorModel, r, false, null);
				...
				// if (true)
				{
					...
					final SinglePixelPackedSampleModel sm = new SinglePixelPackedSampleModel(DataBuffer.TYPE_INT, w, h,
							new int[] { redMask, greenMask, blueMask });
					final WritableRaster r = Raster.createWritableRaster(sm, db, new Point(0, 0));

					final ColorModel colorModel = new DirectColorModel(24, redMask, // Red
							greenMask, // Green
							blueMask, // Blue
							0x0 // Alpha
					);
