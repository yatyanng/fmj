...
public class Handler extends BasicDataSink
	...
	protected final void close(String reason) {
		 ...
		 * buffer1Pending)) { try { bufferLock.wait(250); } catch (InterruptedException
		 * ie) { } } }
		 */
		...
		try {
			if (state == NOT_INITIALIZED) {
				if (locator != null) {
					...
					if (file.exists()) {
						if (!deleteFile(file)) {
							System.err.println("datasink open: Existing file " + pathName
									+ " cannot be deleted. Check if " + "some other process is using " + " this file");
							if (push)
					...
					try {
						...
						// throw it again
						System.err.println(
								"datasink open: IOException when creating RandomAccessFile " + pathName + " : " + e);
						if (push)
							((PushSourceStream) stream).setTransferHandler(null);
						throw e;
					}
	...
	/**
	 * Set the output <tt>MediaLocator</tt>. This method should only be called once;
	 * an error is thrown if the locator has already been set.
	 *
	 * @param output <tt>MediaLocator</tt> that describes where the output goes.
	 */
	@Override
		...
		while (spaceAvailable > 0) {
			try {
				...
				if (bytesRead > 16 * 1024 && WRITE_CHUNK_SIZE < 32 * 1024) {
					...
					else if (bytesRead > 32 * 1024 && WRITE_CHUNK_SIZE < 64 * 1024)
	...
	private void write(byte[] buffer, long location, int length) {
		...
		try {
			...
			while (length > 0) {
				...
				/*
				 * if (fileDescriptor != null) { // Sync'ing the file system at every 1 sec
				 * interval. if (lastSyncTime < 0) lastSyncTime = System.currentTimeMillis();
				 * else { long ts = System.currentTimeMillis(); if (ts - lastSyncTime > 1000L) {
				 * fileDescriptor.sync(); //System.err.println("sync: byte written: " +
				 * bytesWritten); bytesWritten = 0; lastSyncTime = ts; } } }
				 */
