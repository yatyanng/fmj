...
public abstract class BasicPlugIn implements PlugIn {
	...
	private static boolean checkIfJDK12() {
		...
		try {
			...
			Method getSystemClassLoaderM = ClassLoader.class.getMethod("getSystemClassLoader");

			// TODO May need to invoke RuntimePermission("getClassLoader") privilege
			systemClassLoader = (ClassLoader) getSystemClassLoaderM.invoke(ClassLoader.class);
	...
	/*
	 * Commented out the next 3 definitions because of a weird bug when running in
	 * jdk1.2 (No problem in jdk1.1.x). Reference to byte[].class in any file causes
	 * runtime error java.lang.IllegalAccessError: try to access field
	...
	static public Class<?> getClassForName(String className) throws ClassNotFoundException {
		/**
		...
		/**
		 * In jdk1.2 application, when you have FMJ in the ext directory and you want to
		 * access a class that is not in FMJ but is in the CLASSPATH, you have to load
		 * it using the the system class loader.
		 */
		try {
			return (Class<?>) forName3ArgsM.invoke(Class.class,
					new Object[] { className, new Boolean(true), systemClassLoader });
		} catch (Throwable e) {
		}

		/**
		 * In jdk1.2 applet, when you have FMJ in the ext directory and you want to
		 * access a class that is not in FMJ but applet codebase, you have to load it
		 * using the the context class loader.
		 */
		try {
			// TODO May need to invoke RuntimePermission("getClassLoader") privilege.
			ClassLoader contextClassLoader = (ClassLoader) getContextClassLoaderM.invoke(Thread.currentThread());
			return (Class<?>) forName3ArgsM.invoke(Class.class,
	...
	protected byte[] validateByteArraySize(Buffer buffer, int newSize) {
		...
		if (DEBUG)
			System.out.println(getClass().getName() + " : allocating byte[" + newSize + "] data");
	...
	protected int[] validateIntArraySize(Buffer buffer, int newSize) {
		...
		if (DEBUG)
			System.out.println(getClass().getName() + " : allocating int[" + newSize + "] data");
	...
	protected short[] validateShortArraySize(Buffer buffer, int newSize) {
		...
		if (DEBUG)
			System.out.println(getClass().getName() + " : allocating short[" + newSize + "] data");
