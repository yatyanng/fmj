...
public abstract class BasicProcessor extends BasicPlayer implements Processor {
	...
	/**
	 * Return the output DataSource of the Processor. The output DataSource is the
	 * output connection of the Processor through which it supplies the processed
	 * streams.
	 *
	 ...
	 * Processor builds the <tt>ContentDescriptor</tt> array according to the input
	 * DataSource attached to the Processor and according to the codecs and
	 * multiplexers installed in the system.
	 *
	 ...
	 *
	 * @return An array of the tracks in the media. An emtpy array is returned if
	 *         there is no <tt>TrackControl</tt> available for this Processor.
	 * @exception NotConfiguredError if the Processor is Unrealized.
	 */
	@Override
	...
	/**
	 * Set the output content-type. If <tt>setContentDescriptor</tt> is not called,
	 * the output DataSource's output will be on individual tracks.
	 *
	 * @param ocd the content-type of the output.
	 * @exception NotConfiguredError if the Processor is Unrealized. realized state
	 *                               and it does not support format changes after it
	 *                               has been realized.
	 */
	@Override
