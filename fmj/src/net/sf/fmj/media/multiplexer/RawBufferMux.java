...
public class RawBufferMux extends BasicPlugIn implements Multiplexer, Clock {
	...
	// ////////////////////////////////////////////////////////////////////////
	class RawBufferDataSource extends net.sf.fmj.media.protocol.BasicPushBufferDataSource {
	...
	class RawBufferSourceStream extends net.sf.fmj.media.protocol.BasicSourceStream
	 ...
	 * contentdescriptors given that the input data are of the formats specified by
	 * <tt>inputs</tt>. If <tt>inputs</tt> is null, then it lists all possible
	 * output content descriptors that this plug-in advertises.
	 */
	@Override
	...
	/**
	 * Initialize the tracks in the multiplexer with formats given in an array of
	 * track formats. The indexes used in the format array are used subsequently as
	 * keys to identify each individual track in the <tt>process</tt> method. This
	 * methods should be called only once. A java.lang.Error is thrown if it's
	 * called more than once.
	 *
	 * @param trackFormats an array for formats specifying the formats for each
	 *                     track in the multiplexer.
	 ...
	 *                belongs.
	 * @return BUFFER_PROCESSED_OK if the processing is successful. Other possible
	 *         return codes are defined in PlugIn.
	 * @see PlugIn
	 */
	@Override
	 ...
	 * @exception UnsupportedFormatException if the <tt>outputContentDescriptor</tt>
	 *                                       cannot be supported by the Multiple
	 *                                       xer.
			...
			if (mpegAudio.matches(buf.getFormat())) {
				if (buf.getTimeStamp() < 0) {
					if (systemStartTime >= 0)
						mediaTime[trackID] = (mediaStartTime + System.currentTimeMillis() - systemStartTime) * 1000000;
				} else
