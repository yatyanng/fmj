	...
	public int process(Buffer buffer, int trackID) {
		...
		if ((buffer.getFlags() & Buffer.FLAG_NO_WAIT) == 0) {
			if (buffer.getFormat() instanceof AudioFormat) {
				...
				if (mpegVideo.matches(buffer.getFormat()) && (buffer.getFlags() & Buffer.FLAG_RTP_MARKER) != 0) {
					...
					/*
					 * For MPEG the timestamp is the time of the frame but frames come out of order.
					 * Since a stream may not have all types of frames, have to allow for all mixes.
					 * If B frames are present, only wait on them. If no B frames, wait on P frames.
					 * If no B or P frames wait on I frames.
					 */
		...
		if (!(buffer.getFormat() instanceof AudioFormat) || mpegAudio.matches(buffer.getFormat())) {
			...
			if (monoIncrTime) {
				 ...
				 * System.err.println("monoStartTime = " + monoStartTime + " mediaStartTime = "
				 * + mediaStartTime + " TS = " + buffer.getTimeStamp() + " mono TS = " +
				 * monoTime);
				 */
	...
	private void waitForPT(long pt, int trackID) {
		...
		if (masterTrackID == -1 || trackID == masterTrackID) {
			if (systemStartTime < 0)
				...
				delay = (pt - mediaStartTime) - (System.currentTimeMillis() - systemStartTime);
		} else {
