...
public class XmlMovieMux extends AbstractInputStreamMux {
	...
	protected void doProcess(Buffer buffer, int trackID, OutputStream os) throws IOException {
		...
		if (buffer.getFormat() != null && !buffer.getFormat().equals(inputFormats[trackID])) // format
																								// of
																								// buffer
																								// should
																								// always
	...
	private void outputHeader(OutputStream os) throws IOException {
		...
		for (int i = 0; i < numTracks; ++i) {
			...
			os.write(("\t<Track index=\"" + i + "\" format=\""
					+ StringUtils.replaceSpecialXMLChars(FormatArgUtils.toString(inputFormats[i])) + "\"/>\n")
							.getBytes());
