...
public abstract class BasicMux extends BasicPlugIn implements javax.media.Multiplexer, Clock {
				...
				if (!firstBuffersDone[trackID]) {
					if (buffer.getTimeStamp() < masterTime) {
						...
						if (buffer.getFormat() instanceof VideoFormat) {
							...
							boolean isKey = (jpegFmt.matches(fmt) || mjpgFmt.matches(fmt) || rgbFmt.matches(fmt)
									|| yuvFmt.matches(fmt));
							if (isKey || (buffer.getFlags() & Buffer.FLAG_KEY_FRAME) != 0
	 ...
	 * required to create the media file. Two passes may be required for example to
	 * rearrange the media file so that the resultant file is Streamable
	 */
	...
	private void updateClock(Buffer buffer, int trackID) {
		...
		if (timestamp <= 0 && buffer.getFormat() instanceof AudioFormat) {
			...
			mediaTime[trackID] = System.currentTimeMillis() * 1000000 - systemStartTime;
		} else
