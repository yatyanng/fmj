...
public abstract class AbstractInputStreamMux extends AbstractMultiplexer {
	...
	public void close() {
		...
		// async
		/*
		 * if (pipedInputStream != null) { try { pipedInputStream.close(); } catch
		 * (IOException e) { logger.log(Level.WARNING, "" + e, e); } finally {
		 * pipedInputStream = null; } }
		 *
		 * if (pipedOutputStream != null) { try { pipedOutputStream.close(); } catch
		 * (IOException e) { logger.log(Level.WARNING, "" + e, e); } finally {
	...
	public DataSource getDataOutput() {
		if (dataOutput == null)
			dataOutput = createInputStreamPushDataSource(outputContentDescriptor, 1,
	...
	/**
	 * Not a JMF public API method, just a way for subclasses to get the data output
	 * to do notifications.
	 */
