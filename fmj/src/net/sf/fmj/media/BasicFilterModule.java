...
public class BasicFilterModule extends BasicModule {
	...
	public void process() {
		...
		do {
			if (readPendingFlag)
				...
				if (incomingFormat != ic.getFormat() && incomingFormat != null && !incomingFormat.equals(ic.getFormat())
					...
					if (!reinitCodec(inputBuffer.getFormat())) {
						...
						if (moduleListener != null)
							moduleListener.formatChangedFailure(this, ic.getFormat(), inputBuffer.getFormat());
						return;
					}
					...
					if (moduleListener != null)
						moduleListener.formatChanged(this, oldFormat, inputBuffer.getFormat());
				}
	...
	/**
	 * A new input format has been detected, we'll check if the existing codec can
	 * handle it. Otherwise, we'll try to re-create a new codec to handle it.
	 */
	...
	/*
	 * private Frame monitorFrame = null; private Panel monitorPanel = null; private
	 * long oldTime = (long) -1E+9; private Dimension monitorSize = null; private
	 * void monitorCheck(Buffer buffer) { if (buffer.getFormat() instanceof
	 * RGBFormat) { RGBFormat rgb = (RGBFormat) buffer.getFormat(); if (monitorFrame
	 * == null) { monitorSize = rgb.getSize(); monitorPanel = new Panel() { public
	 * void update(Graphics g) { }
	 *
	 * public void paint(Graphics g) { }
	 *
	 * public Dimension getPreferredSize() { return monitorSize; } }; monitorFrame =
	 * new Frame("Monitor"); monitorFrame.setLayout( new BorderLayout() );
	 * monitorFrame.add("Center", monitorPanel); monitorFrame.pack();
	 *
