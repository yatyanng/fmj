 ...
 * File renderer). VideoRenderer might be implemented as either Push or
 * Pull.<br>
...
public class BasicRendererModule extends BasicSinkModule implements RTPTimeReporter {
	...
	/**
	 * The loop to process the data. It handles the getting and putting back of the
	 * data buffers. It in turn calls scheduleBuffer(Buffer) to do the bulk of
	 * processing.
	 */
	protected boolean doProcess() {
		...
		if (storedBuffer != null)
			 ...
			 * buffer.getSequenceNumber() + " eom: " + buffer.isEOM() + " discard: " +
			 * buffer.isDiscard());
			 */
		}
		...
		if (storedBuffer == null && buffer.isEOM()) {
			...
			if ((buffer.getFlags() & Buffer.FLAG_NO_WAIT) == 0 && buffer.getTimeStamp() > 0 && buffer.getDuration() > 0
					&& buffer.getFormat() != null && !(buffer.getFormat() instanceof AudioFormat) && !noSync) {
	...
	public long getRTPTime() {
		if (ic.getFormat() instanceof AudioFormat) {
			if (renderer instanceof AudioRenderer) {
				/*
				 * System.err.println("rtpTime[audio]: " + lastTimeStamp + " latency: " +
				 * ((AudioRenderer)renderer).getLatency() + " TS: " + (lastTimeStamp -
	...
	private long getSyncTime(long pts) {
		if (rtpTimeBase != null) {
			...
			if (ts > pts + RTP_TIME_MARGIN || ts < pts - RTP_TIME_MARGIN) {
				/*
				 * System.err.println("pts and mts too different: ts = " + ts/1000000L +
				 * " pts = " + pts/1000000L + " diff = " + (ts - pts)/1000000L);
				 */
				if (!rtpErrMsg) {
					Log.comment(
							"Cannot perform RTP sync beyond a difference of: " + (ts - pts) / 1000000L + " msecs.\n");
					rtpErrMsg = true;
				}
	...
	/**
	 * Handle the prerolling a buffer. It will preroll until the media has reach the
	 * current media time before displaying.
	 */
	...
	/**
	 * Break down one larger buffer into smaller pieces so the processing won't take
	 * that long to block.
	 */
	 ...
	 * It in turn calls processBuffer to do the real processing.
	 */
	protected boolean scheduleBuffer(Buffer buf) {
		...
		if (prefetching || (format instanceof AudioFormat) || buf.getTimeStamp() <= 0
				|| (buf.getFlags() & Buffer.FLAG_NO_SYNC) == Buffer.FLAG_NO_SYNC || noSync) {
			 ...
			 * System.err.println("BRM: display on prefetch: " + buf.getSequenceNumber()); }
			 */
			...
			/*
			 * System.err.println("VR: PT = " + buf.getTimeStamp()/1000000L + " MT = " +
			 * mt/1000000L + " lateBy = " + lateBy);
			 */
	...
	/**
	 * If the presentation time has not been reached, this function will wait until
	 * that happens.
	 */
