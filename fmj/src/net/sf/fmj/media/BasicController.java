...
public abstract class BasicController implements Controller, Duration {
	 ...
	 * time-base-time will be computed with the given time base. The subclass should
	 * implement and further specialized this method to do the real work. But it
	 * should also invoke this method to maintain the correct states.
	 *
	 * @param tb the time base to set to.
	 * @exception IncompatibleTimeBaseException is thrown if the Controller cannot
	 *                                          accept the given time base.
	 */
	static String TimeBaseError = "Cannot set time base on an unrealized controller.";

	/**
	 * Start the controller. Invoke clock.start() to maintain the clock states. It
	 * starts a wait thread to wait for the given tbt. At tbt, it will wake up and
	 * call doStart(). A subclass should implement the doStart() method to do the
	 * real work.
	 *
	...
	/**
	 * Set the stop time. Invoke clock.setStopTime() to maintain the clock states.
	 * The subclass should implement and further specialized this method to do the
	 * real work. But it should also invoke this method to maintain the correct
	 * states.
	 *
	...
	/**
	 * Set the media time. Invoke clock.setMediaTime() to maintain the clock states.
	 * The subclass should implement and further specialized this method to do the
	 * real work. But it should also invoke this method to maintain the correct
	 * states.
	 *
	...
	/**
	 * Set the rate of presentation: 1.0: normal, 2.0: twice the speed. -2.0: twice
	 * the speed in reverse. Note that not all rates are supported. Invokes
	 * clock.setRate() to maintain the clock states. The subclass SHOULDN'T in
	 * general override this class. If necessary, it should override the behavior
	 * using the doSetRate method. By overriding the doSetRate method, subclass
	 * Conrollers can support negative rates, fractional rates etc., but they should
	 * guard against illegal rates from going into the clock calculations.
	 *
	 ...
	 * specified in the spec. Implement the abortRealize and abortPrefetch methods
	 ...
	 * outside of the controller. Take caution if this is being call from inside the
	 * Controller. It may cause deadlock.
	 */
	static String DeallocateError = "deallocate cannot be used on a started controller.";

	public BasicController() {
		sendEvtQueue = new SendEventQueue(this);
		sendEvtQueue.setName(sendEvtQueue.getName() + ": SendEventQueue: " + this.getClass().getName());
		sendEvtQueue.start();
		clock = new BasicClock();
	}

	/**
	 * Called when the configure() is aborted, i.e. deallocate() was called while
	 * realizing. Release all resources claimed previously by the configure() call.
	...
	/**
	 * Add a listener to the listenerList. This listener will be notified of this
	 * controller's event. This needs to be a synchronized method so as to maintain
	 * the integrity of the listenerList.
	 */
	@Override
	 ...
	 * ConfigureCompleteEvents from down stream Controllers have been received. If a
	 * subclass wants to override this method, it should still invoke this to ensure
	 * the correct events being sent to the upstream Controllers.
	 */
	protected synchronized void completeConfigure() {
		state = Configured;
		Log.comment(this.getClass().getName() + " " + this.hashCode() + " is sending ConfigureCompleteEvent");
	...
	/**
	 * Called when the controller is realized and when all the RealizeCompleteEvents
	 * from down stream Controllers have been received. If a subclass wants to
	 * override this method, it should still invoke this to ensure the correct
	 * events being sent to the upstream Controllers.
	 */
	...
	public synchronized void configure() {
		...
		case Unrealized:
			...
			Log.comment(this.getClass().getName() + " " + this.hashCode() + " is starting "
					+ configureThread.getClass().getName() + " " + configureThread.hashCode());
	...
	/**
	 * An internal function to notify the listeners on the listener list the given
	 * event. This gets called by the sendEvtQueue's processEvent() callback. This
	 * method updates a lock on the Vector listenerList before enumerating it.
	 */
	 ...
	 * thread. So do take necessary precautions to protect shared resources. It's OK
	 * to put an empty stub function body here.
	 * <p>
	 ...
	 * deallocate() and processEvent() will be blocked since they are synchronized
	 * methods. Override this to implement subclass behavior.
	 *
	...
	/**
	 * Start immediately. Invoked from start(tbt) when the scheduled start time is
	 * reached. Use the public start(tbt) method for the public interface. Override
	 * this to implement subclass behavior.
	 */
	...
	/**
	 * Get the <tt>Control</tt> that supports the class or interface specified. The
	 * full class or interface name should be specified. <tt>Null</tt> is returned
	 * if the <tt>Control</tt> is not supported.
	 *
	...
	/**
	 * Return a list of <b>Control</b> objects this <b>Controller</b> supports. If
	 * there are no controls, then an array of length zero is returned.
	 *
	 ...
	 * through the Configured state. For example, the Controller implementing the
	 * Processor should return true.
	 */
	 ...
	 * @exception ClockStoppedException thrown if the Controller has already been
	 *                                  stopped.
	 */
	@Override
	...
	/**
	 * Take the necessary steps to prefetch the controller. This is a non-blocking
	 * call. It starts a work thread to do the real work. The actual code to do the
	 * realizing should be written in doPrefetch(). The thread is also responsible
	 * for catching all the PrefetchCompleteEvents from the down stream nodes. When
	 * the steps to prefetch the controller are completed and when all the
	 * PrefetchCompleteEvents from down stream nodes have been received, the
	 * completePrefetch() call will be invoked.
	 */
	@Override
	...
	/**
	 * Take the necessary steps to realize the controller. This is a non-blocking
	 * call. It starts a work thread to do the real work. The actual code to do the
	 * realizing should be written in doRealize(). The thread is also responsible
	 * for catching all the RealizeCompleteEvents from the down stream nodes. When
	 * the steps to realize the controller are completed and when all the
	 * RealizeCompleteEvents from down stream nodes have been received, the
	 * completeRealize() call will be invoked.
	 */
	@Override
	...
	/**
	 * Remove a listener from the listener list. The listener will stop receiving
	 * notification from this controller. This needs to be a synchronized method so
	 * as to maintain the integrity of the listenerList.
	 */
	@Override
	...
	/**
	 * Send an event to the listeners listening to my events. The event is Queued in
	 * the sendEvtQueue which runs in a separate thread. This way, sendEvent() won't
	 * be blocked.
	 */
	...
	public void setStopTime(Time t) {
		...
		if (state == Started) {
			long timeToStop;
			if (((timeToStop = checkStopTime()) < 0) || (stopThreadEnabled && activateStopThread(timeToStop)))
	...
	/**
	 * Stop the controller. Invoke clock.stop() to maintain the clock states. The
	 * subclass should implement and further specialized this method to do the real
	 * work. But it should also invoke this method to maintain the correct states.
	 */
	@Override
	public void stop() {
	...
	public void syncStart(final Time tbt) {
		...
		sendEvent(new StartEvent(this, Prefetched, Started, Started, getMediaTime(), tbt));
		long timeToStop;
		if ((timeToStop = checkStopTime()) < 0 || (stopThreadEnabled && activateStopThread(timeToStop))) {
...
class ConfigureWorkThread extends StateTransitionWorkThread {
	...
	protected boolean process() {
		Log.comment(controller.getClass().getName() + " " + controller.hashCode() + " is about to do configure!");
...
class RealizeWorkThread extends StateTransitionWorkThread {
	...
	@Override
	protected boolean process() {
		Log.comment(controller.getClass().getSimpleName() + " " + controller.hashCode() + " is about to do realize!");
...
class SendEventQueue extends ThreadedEventQueue {
	 ...
	 * case, we call controller's dispatchEvent() to send the event to the listening
	 * controllers.
	 */
	@Override
...
abstract class StateTransitionWorkThread extends MediaThread {
	 ...
	 * completed and all the events from down stream nodes have been fully captured.
	 */
	...
	/**
	 * run() method for the thread. Do the work, wait for every expected events to
	 * arrive, signal the the completion.
	 */
	@Override
	public void run() {
		...
		try {
			Log.comment(getClass().getName() + " " + hashCode() + " is about to process!");
