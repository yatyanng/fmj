...
abstract public class BasicModule implements Module, StateTransistor {
	...
	class Registry extends Hashtable<String, Connector> {
		...
		public Connector put(String name, Connector connector) {
			if (containsKey(name))
				throw new RuntimeException("Connector '" + name + "' already exists in Module '"
	...
	/**
	 * return if data is available on all inputConnectors and there is room in all
	 * outputConnectors.
	 */
	...
	/**
	 * This function performs the steps to deallocate a module or Player, and return
	...
	/**
	 * This function performs the steps to stop a module or Player, and return to
	...
	/**
	 * For each of the outputConnectables from this node, it needs to be registered
	 * with this function.
	 */
	@Override
	...
	protected boolean verifyBuffer(Buffer buffer) {
		...
		if (data == null) {
			...
			if (buffer.getLength() > ((byte[]) data).length) {
				System.err.println("buffer advertized length = " + buffer.getLength() + " but actual length = "
						+ ((byte[]) data).length);
				return false;
			}
