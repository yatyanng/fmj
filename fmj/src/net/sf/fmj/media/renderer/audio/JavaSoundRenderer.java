...
public class JavaSoundRenderer implements Renderer {
	...
	private Format[] supportedInputFormats = new Format[] {
			...
			new AudioFormat(AudioFormat.ULAW, -1.0, -1, -1, -1, -1, -1, -1.0, Format.byteArray), // TODO: our codec
																									// doesn't support
																									// all
																									// ULAW input
																									// formats.
			new AudioFormat(AudioFormat.ALAW, -1.0, -1, -1, -1, -1, -1, -1.0, Format.byteArray), // TODO: our codec
																									// doesn't support
																									// all
																									// ALAW input
																									// formats.
	...
	/**
	 * Set supported input formats for the default or selected Mixer. Perhaps just
	 * list generic LINEAR, ALAW and ULAW. At the moment, we are returning all the
	 * formats handled by the current default mixer.
	 */
	@Override
	...
	/**
	 * Open the plugin. Must be called after the formats have been determined and
	...
	public void open() throws ResourceUnavailableException {
		...
		try {
			if (!inputFormat.getEncoding().equals(AudioFormat.LINEAR)) {
				...
				final AudioFormat codecOutputFormat = AudioFormatCompleter.complete((AudioFormat) outputFormats[0]); // TODO:
																														// choose
																														// the
																														// best
																														// quality
				...
				logger.info("JavaSoundRenderer: Audio format is not linear, created conversion from " + inputFormat
						+ " to " + codecOutputFormat);
	...
	public int process(Buffer buffer) {
		...
		while (length > 0) {
			...
			if (n >= length)
				...
				logger.warning("sourceLine.write returned 0, offset=" + offset + "; length=" + length + "; available="
						+ sourceLine.available() + "; frame size in bytes" + sourceLine.getFormat().getFrameSize()
						+ "; sourceLine.isActive() = " + sourceLine.isActive() + "; " + sourceLine.isOpen()
	...
	/**
	 * Reset the state of the plugin. The reset method is typically called if the
	 * end of media is reached or the media is repositioned.
	 */
	@Override
	public void reset() {
