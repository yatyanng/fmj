...
public abstract class AudioRenderer extends BasicPlugIn implements Renderer, Prefetchable, Drainable, Clock {
	...
	protected boolean initDevice(AudioFormat format) {
		...
		AudioFormat audioFormat = new AudioFormat(format.getEncoding(), format.getSampleRate(),
				format.getSampleSizeInBits(), format.getChannels(), format.getEndian(), format.getSigned());
	...
	protected void processByWaiting(Buffer buffer) {
		 ...
		 * sampleRate + " channels = " + channels + " length = " + buffer.getLength() +
		 * " offset = " + buffer.getOffset() + " timeToWait = " + timeToWait);
		 */
		try {
	...
	public void setTimeBase(TimeBase master) throws IncompatibleTimeBaseException {
		if (!(master instanceof AudioTimeBase)) {
			...
			/**
			 * Silently allows the time base to be set to make addController slightly more
			 * useful. --ivg throw new IncompatibleTimeBaseException();
			 */
		}
