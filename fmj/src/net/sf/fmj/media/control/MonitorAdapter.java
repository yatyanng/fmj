...
public class MonitorAdapter implements MonitorControl, Owned {
		...
		if (buffer.getFormat() instanceof AudioFormat) {
			...
			if (time >= lastPreviewTime + previewInterval || time <= lastPreviewTime) {
				if (mpegVideo.matches(format)) {
					/*
					 * Only set toDisplay true on MPEG I frames. This way VideoCodecChain can treat
					 * the stream as a raw format and avoid overloading the CPU by sending every
					 * frame to the decoder. Downside is it limits the framerate that can be set for
					 * the monitor.
					 */
	...
	public Component getControlComponent() {
		 ...
		 * getVisualComponent(); if (visualComponent == null) return null; southPanel =
		 * new Panel(); listBox = new List(6); for (int i = 0; i <= inFrameRate; i++) {
		 * listBox.add(i + " fps"); } southPanel.add(new
		 * Label("Monitor Frame Rate : ")); southPanel.add(listBox);
		 *
		 * preferredSize = visualComponent.getPreferredSize(); preferredSize.height +=
		 * southPanel.getPreferredSize().height;
		 *
	...
	public void process(Buffer input) {
		if (input == null || previewFrameRate <= 0 || format == null || input.isEOM() || input.isDiscard()
