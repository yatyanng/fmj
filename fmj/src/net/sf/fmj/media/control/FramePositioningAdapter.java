...
public class FramePositioningAdapter implements FramePositioningControl, Reparentable {
	static public Track getMasterTrack(Track tracks[]) {
		...
		for (int i = 0; i < tracks.length; i++) {
			...
			if ((rate = ((VideoFormat) f).getFrameRate()) != Format.NOT_SPECIFIED && rate != 0f) {
				return master;
			}
		}

		if (master != null && master.mapTimeToFrame(new Time(0)) != FramePositioningControl.FRAME_UNKNOWN)
			return master;
		else
			return null;
	}
	 ...
	 * <p>
	 * The frame returned is the nearest frame that has a media time less than or
	 * equal to the given media time.
	 * <p>
	 *
	 * @param mediaTime the input media time for the conversion.
	 * @return the converted frame number the given media time. If the conversion
	 *         fails, FRAME_UNKNOWN is returned.
	 */
	@Override
	 ...
	 *                     If framesToSkip is positive, it will seek forward by
	 *                     framesToSkip number of frames. If framesToSkip is
	 *                     negative, it will seek backward by framesToSkip number of
	 *                     frames. e.g. skip(-1) will step backward one frame.
	...
	public int skip(int framesToSkip) {
		if (frameStep != -1) {
			...
			if (currentFrame != 0 && currentFrame != FramePositioningControl.FRAME_UNKNOWN) {
