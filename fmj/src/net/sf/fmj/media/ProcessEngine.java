...
public class ProcessEngine extends PlaybackEngine {
	class ProcGraphBuilder extends SimpleGraphBuilder {
		...
		/**
		 * Overrides GraphBuider's buildGraph method. The major difference is, it pay
		 * attentions to the effects, codecs, renderers specified.
		 */
			...
			if (outputContentDes != null) {
				...
				if (rend != null) {
					...
					if ((node = buildGraph(candidates)) == null) {
						if (format != null)
							Log.error(
									"The customed transocoded format is not compatible with the given renderer plugin: "
											+ rend);
						else
					...
					if ((node = buildGraph(candidates)) == null) {
						if (format != null)
							...
							Log.error(
									"Failed to build a graph to render the input format with the given custom options.");
						indent = 0;
						Log.setIndent(indent);
						return null;
					}
		...
		/**
		 * Collect the supported output formats from the list of node candidates.
		 */
		void doGetSupportedOutputFormats(Vector candidates, Vector collected) {
			...
			for (int i = 0; i < outs.length; i++) {
				...
				if (node.plugin != null && verifyOutput(node.plugin, outs[i]) == null) {
					continue;
				}

				Vector cnames = PlugInManager.getPlugInList(outs[i], null, PlugInManager.CODEC);
				if (cnames == null || cnames.size() == 0)
					continue;

				for (int j = 0; j < cnames.size(); j++) {
					// Instantiate and verify the codec.
					if ((gn = getPlugInNode((String) cnames.elementAt(j), PlugInManager.CODEC, plugIns)) == null)
						continue;
		...
		/**
		 * This defines when the search ends. The "targets" array defines the nodes that
		 * are to be the "end points" (leaf nodes) of the graph. With the default graph
		 * builder, the targets array contains the list of sinks that can potentially
		...
		/**
		 * Given an input format, find out all the supported output formats by searching
		...
		public Format[] getSupportedOutputFormats(Format input) {
			...
			boolean mpegInput = (new AudioFormat(AudioFormat.MPEG)).matches(input)
		...
		boolean setDefaultTargetMux() {
			...
			targetMuxNames = PlugInManager.getPlugInList(null, outputContentDes, PlugInManager.MULTIPLEXER);
		...
		GraphNode verifyTargetMuxes(GraphNode node, Format outs[]) {
			...
			for (int i = 0; i < targetMuxes.length; i++) {
				if ((gn = targetMuxes[i]) == null) {
					...
					if ((gn = getPlugInNode(name, PlugInManager.MULTIPLEXER, plugIns)) == null) {
				...
				for (int j = 0; j < outs.length; j++) {
					if ((fmt = ((Multiplexer) gn.plugin).setInputFormat(outs[j], trackID)) == null)
						continue;
	...
	class ProcTControl extends BasicTrackControl implements Owned {
		...
		/**
		 * Check if the video size of the given format is valid. If not, return a format
		...
		private Format checkSize(Format fmt) {
			...
			if (fmt.matches(new VideoFormat(VideoFormat.JPEG_RTP)) || fmt.matches(new VideoFormat(VideoFormat.JPEG))) {
			...
			if (w != size.width || h != size.height) {
				...
				fmt = (new VideoFormat(null, new Dimension(w, h), Format.NOT_SPECIFIED, null, Format.NOT_SPECIFIED))
						.intersects(fmt);
			}
		...
		public Format[] getSupportedFormats() {
			...
			boolean renew = false;
			if (supportedFormats == null && (supportedFormats = Resource.getDB(track.getFormat())) == null) {
				if (gb == null)
					gb = new ProcGraphBuilder((ProcessEngine) engine);
				else
					gb.reset();
				supportedFormats = gb.getSupportedOutputFormats(track.getFormat());
				supportedFormats = Resource.putDB(track.getFormat(), supportedFormats);
				needSavingDB = true;
				renew = true;
			}
			...
			if (outputContentDes != null) {
				if (renew || !verifiedFormats.containsKey(outputContentDes.getEncoding())) {
					verifiedFormats.put(outputContentDes.getEncoding(),
							verifyMuxInputs(outputContentDes, supportedFormats));
				}
				return verifiedFormats.get(outputContentDes.getEncoding());
			} else {
		...
		public Format setFormat(Format format) {
			 ...
			 * VideoFormat newf = new VideoFormat(null, new Dimension(100, 100),
			 * Format.NOT_SPECIFIED, null, Format.NOT_SPECIFIED); format =
			...
			/*
			 * Format fmts[] = getSupportedFormats(); boolean good = false; for (int i = 0;
			 * i < fmts.length; i++) { if (formatWanted.matches(fmts[i])) {
			 * System.err.println("It's a good format"); good = true; } } if (!good) {
		...
		/**
		 * If a multiplexer (ContentDescriptor) is specified, we'll verify if the
		 * multiplexer supports the given input.
		 */
		Format[] verifyMuxInputs(ContentDescriptor cd, Format inputs[]) {
			...
			Vector cnames = PlugInManager.getPlugInList(null, cd, PlugInManager.MULTIPLEXER);
			if (cnames == null || cnames.size() == 0)
				return new Format[0];
			...
			// format.
			Log.comment("Query " + total + " multiplexers to see if they support the input format");

			java.util.List<Format> formatList = new ArrayList<>();

			Arrays.asList(inputs).parallelStream().forEach(format -> {
				Optional<Format> optional = Arrays.asList(mux).stream()
						.map(multiplexor -> multiplexor.setInputFormat(format, 0)).filter(Objects::nonNull).findFirst();
				if (optional.isPresent()) {
					formatList.add(optional.get());
				}
			});
	...
	protected static Map<String, Format[]> verifiedFormats = new HashMap<>();
	 ...
	 * formats; 2) Routines to build a default flow graph -- buildGraph; 3) Routines
	 * to build a custom flow graph -- buildCustomGraph.
	 *
	...
	protected boolean doConfigure() {
		...
		for (int i = 0; i < tracks.length; i++) {
			trackControls[i] = new ProcTControl(this, tracks[i], source.getOutputConnector(names[i]));
		}
	...
	public ContentDescriptor[] getSupportedContentDescriptors() throws NotConfiguredError {
		...
		Vector names = PlugInManager.getPlugInList(null, null, PlugInManager.MULTIPLEXER);
		...
		for (i = 0; i < names.size(); i++) {
			fs = PlugInManager.getSupportedOutputFormats((String) names.elementAt(i), PlugInManager.MULTIPLEXER);
			if (fs == null)
				continue;
	...
	boolean isRTPFormat(Format fmt) {
		return fmt != null && fmt.getEncoding() != null && fmt.getEncoding().endsWith("rtp")
	...
	public ContentDescriptor setContentDescriptor(ContentDescriptor ocd) throws NotConfiguredError {
		...
		if (ocd != null) {
			Vector cnames = PlugInManager.getPlugInList(null, ocd, PlugInManager.MULTIPLEXER);
