...
public abstract class BasicCodec extends BasicPlugIn implements Codec {
	...
	protected boolean checkInputBuffer(Buffer inputBuffer) {
		boolean fError = !isEOM(inputBuffer)
				&& (inputBuffer == null || inputBuffer.getFormat() == null || !checkFormat(inputBuffer.getFormat()));
	...
	protected RGBFormat updateRGBFormat(VideoFormat newFormat, RGBFormat outputFormat) {
		...
		RGBFormat newRGB = new RGBFormat(size, lineStride * size.height, oldFormat.getDataType(),
				newFormat.getFrameRate(), oldFormat.getBitsPerPixel(), oldFormat.getRedMask(), oldFormat.getGreenMask(),
				oldFormat.getBlueMask(), oldFormat.getPixelStride(), lineStride, oldFormat.getFlipped(),
				oldFormat.getEndian());
		return newRGB;
	}
}
