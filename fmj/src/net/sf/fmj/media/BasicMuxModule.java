...
public class BasicMuxModule extends BasicSinkModule {
	...
	public void connectorPushed(InputConnector ic) {
		...
		if (elapseTime[idx].update(buffer.getLength(), buffer.getTimeStamp(), format)) {
			...
			if (prerollTrack[idx]) {
				...
				if (elapseTime[idx].value > target) {
					...
					if (format instanceof AudioFormat && AudioFormat.LINEAR.equals(format.getEncoding())) {
						int remain = (int) ElapseTime.audioTimeToLen(elapseTime[idx].value - target,
								(AudioFormat) format);
			...
			if (stopTime > -1 && elapseTime[idx].value > stopTime && format instanceof AudioFormat) {
		...
		do {
			if (!dataPrerolled) {
				...
				if (rc == PlugIn.BUFFER_PROCESSED_OK && format == firstVideoFormat) {
			...
			if (prefetching
					&& (!(multiplexer instanceof Prefetchable) || ((Prefetchable) multiplexer).isPrefetched())) {
	...
	/**
	 * Internally, this pauses the processing thread from pushing more data into the
	 * multiplexer.
	 */
	void pause() {
