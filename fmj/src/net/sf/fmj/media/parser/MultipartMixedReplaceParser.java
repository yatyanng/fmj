...
public class MultipartMixedReplaceParser extends AbstractDemultiplexer {
	...
	private class VideoTrack extends PullSourceStreamTrack {
		...
		public VideoTrack(PullSourceStream stream) throws ResourceUnavailableException {
			...
			try {
				image = ImageIO.read(
						new ByteArrayInputStream((byte[]) buffer.getData(), buffer.getOffset(), buffer.getLength()));
			} catch (IOException e) {
			...
			if (frameContentType.equals("image/jpeg"))
				format = new JPEGFormat(new Dimension(image.getWidth(null), image.getHeight(null)),
						Format.NOT_SPECIFIED, Format.byteArray, -1.f, Format.NOT_SPECIFIED, Format.NOT_SPECIFIED);
			else if (frameContentType.equals("image/gif"))
				format = new GIFFormat(new Dimension(image.getWidth(null), image.getHeight(null)), Format.NOT_SPECIFIED,
						Format.byteArray, -1.f);
			else if (frameContentType.equals("image/png"))
				format = new PNGFormat(new Dimension(image.getWidth(null), image.getHeight(null)), Format.NOT_SPECIFIED,
						Format.byteArray, -1.f);
			else
		...
		/**
		 * Eat bytes until we see the specified boundary. Return -1 + -1 * num bytes
		 * eaten on eos, num bytes eaten otherwise.
		 */
		private int eatUntil(String boundary) throws IOException {
			...
			while (true) {
				...
				if (matchBuffer[matchOffset] == boundaryBytes[matchOffset]) {
					...
					if (matchOffset > 0) { // we had a partial, but not full match
						matchOffset = 0;
					} else { // completely nonmatching byte
			...
			try {
				...
				if (boundary == null) {
					...
					if (!line.trim().equals(boundary)) {
						...
						logger.info("Ignored bytes (eom after=" + (eatResult < 0) + "): "
				...
				if (frameContentType == null) {
					...
					if (!contentType.equals(frameContentType))
						throw new IOException(
								"Content type changed during stream from " + frameContentType + " to " + contentType);
				}
		...
		private String readLine(int max) throws IOException {
			...
			while (true) {
				if (offset >= max)
					throw new MaxLengthExceededException("No newline found in " + max + " bytes"); // no newline up to
																									// max.
				final int lenRead = read(buffer, offset, length);
				if (lenRead < 0)
					return null; // EOS
	...
	/**
	 * Allows us to dump out arbitrary, possible binary data, that we were expecting
