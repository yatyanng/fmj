...
public class RawPushBufferParser extends RawStreamParser {
	class FrameTrack implements Track, BufferTransferHandler {
		...
		public boolean findH261Key(Buffer b) {
			...
			if ((data[offset + skipBytes] != 0) || (data[offset + skipBytes + 1] != 1)
			...
			format = new VideoFormat(VideoFormat.H261_RTP, new Dimension(width, height),
					((VideoFormat) format).getMaxDataLength(), ((VideoFormat) format).getDataType(),
		...
		public boolean findH263_1998Key(Buffer b) {
			...
			if (payloadLen > 5) {
				...
				if (((data[offset] & 0x02) == 0x02) && ((data[offset + 3] & 0xfc) == 0x80)) {
					picOffset = offset + 3;
				} else if ((data[offset + 2] & 0xfc) == 0x80) {
					picOffset = offset + 2;
				}
			} else if (((data[offset] & 0x04) == 0x04) && ((data[offset + payloadLen] & 0xfc) == 0x80)) {
			...
			if (s == 7) {
				...
				if (((data[picOffset + 3] >> 1) & 0x07) == 1) {
					s = ((data[picOffset + 3] << 2) & 0x04) | ((data[picOffset + 4] >> 6) & 0x03);
				} else {
			...
			format = new VideoFormat(VideoFormat.H263_1998_RTP, new Dimension(width, height),
					((VideoFormat) format).getMaxDataLength(), ((VideoFormat) format).getDataType(),
					((VideoFormat) format).getFrameRate());
			b.setFormat(format);
			return true;
		...
		public boolean findH263Key(Buffer b) {
			...
			offset = b.getOffset();
			if ((data[offset + payloadLen] != 0) || (data[offset + payloadLen + 1] != 0)
			...
			format = new VideoFormat(VideoFormat.H263_RTP, new Dimension(width, height),
					((VideoFormat) format).getMaxDataLength(), ((VideoFormat) format).getDataType(),
					((VideoFormat) format).getFrameRate());
		...
		public boolean findJPEGKey(Buffer b) {
			...
			format = new VideoFormat(VideoFormat.JPEG_RTP, new Dimension(width, height),
					((VideoFormat) format).getMaxDataLength(), ((VideoFormat) format).getDataType(),
					((VideoFormat) format).getFrameRate());
		...
		private boolean findKeyFrame(Buffer buf) {
			if (!checkDepacketizer) {
				...
				Vector pnames = PlugInManager.getPlugInList(buf.getFormat(), null, Depacketizer.DEPACKETIZER);
				if (pnames.size() != 0) {
		...
		public boolean findMPAKey(Buffer b) {
			...
			if ((data[off] & 0xff) != 0xff || (data[off + 1] & 0xf6) <= 0xf0 || (data[off + 2] & 0xf0) == 0xf0
					|| (data[off + 2] & 0x0c) == 0x0c || (data[off + 3] & 0x03) == 0x02)
			...
			format = new AudioFormat(AudioFormat.MPEG_RTP, sampleRate, 16, channels, AudioFormat.LITTLE_ENDIAN,
					AudioFormat.SIGNED);
		...
		public boolean findMPEGKey(Buffer b) {
			...
			if (data[off + 4] != 0 || data[off + 5] != 0 || data[off + 6] != 1 || (data[off + 7] & 0xff) != 0xb3) {
			...
			width = ((data[off + 8] & 0xff) << 4) | ((data[off + 9] & 0xf0) >> 4);
			height = ((data[off + 9] & 0x0f) << 8) | (data[off + 10] & 0xff);
			frameRate = MPEGRateTbl[frix];

			format = new VideoFormat(VideoFormat.MPEG_RTP, new Dimension(width, height),
					((VideoFormat) format).getMaxDataLength(), ((VideoFormat) format).getDataType(), frameRate);
	...
	final float MPEGRateTbl[] = { 0.0f, 23.976f, 24.f, 25.f, 29.97f, 30.f, 50.f, 59.94f, 60.f };
	...
	boolean isRTPFormat(Format fmt) {
		return fmt != null && fmt.getEncoding() != null
				&& (fmt.getEncoding().endsWith("rtp") || fmt.getEncoding().endsWith("RTP"));
	}
