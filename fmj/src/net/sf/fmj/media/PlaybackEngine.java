...
public class PlaybackEngine extends BasicController implements ModuleListener {
	...
	/**
	 * This is the Graph builder extended from SimpleGraphBuilder to generate the
	 * data flow graph for the given track.
	 */
	...
	/**
	 * Construct a track (connected modules) from the specified node graph. Return a
	 * non-null GraphNode if the track cannot be built. The non-null node returned
	 * is the node that failed to be opened.
	 */
	protected GraphNode buildTrackFromGraph(BasicTrackControl tc, GraphNode node) {
		...
		do {
			...
			if (oc == null || (ic = oc.getInputConnector()) == null || (dst = (BasicModule) ic.getModule()) == null)
				break;
		} while (true);
	...
	public void createProgressControl() {
		...
		progressControl = new ProgressControlAdapter(frameRate, bitRate, videoProps, audioProps, videoCodec,
				audioCodec);
	}
	...
	public void dataBlocked(Module src, boolean blocked) {
		...
		if (blocked) {
			...
			setTargetState(Controller.Started); // NOTE: may cause race
												// condition
			sendEvent(new RestartingEvent(this, Controller.Started, Controller.Prefetching, Controller.Started,
					getMediaTime()));
		} else {
			sendEvent(new StartEvent(this, Controller.Prefetched, Controller.Started, Controller.Started,
	...
	protected boolean doConfigure() {
		...
		for (int i = 0; i < tracks.length; i++) {
			Log.comment("[PlaybackEngine " + hashCode() + "] Building Track Control " + (i + 1) + "/" + tracks.length);
	...
	protected boolean doConfigure1() {
		...
		// Realize the source.
		Log.comment("[PlaybackEngine " + hashCode() + "] Realize the source.");
		if (!source.doRealize()) {
		...
		Log.comment("[PlaybackEngine " + hashCode() + "] Check for interrupt after a critical section.");
		if (isInterrupted()) {
			Log.error(configError);
		...
		Log.comment("[PlaybackEngine " + hashCode() + "] Get Demultiplexer");
		...
		Log.comment("[PlaybackEngine " + hashCode() + "] Build the track controls.");
		try {
	...
	protected boolean doConfigure2() {
		Log.comment("[PlaybackEngine " + hashCode() + "] Demultiplexer Positionable: " + parser.isPositionable());
		Log.comment("[PlaybackEngine " + hashCode() + "] Demultiplexer Random Access: " + parser.isRandomAccess());
		...
		Log.comment("[PlaybackEngine " + hashCode() + "] Frame Positioning Control: " + framePositioningControl);
		return true;
	}
	...
	protected synchronized boolean doRealize() {
		Log.comment("About to do realize-1 and realize-2");
	...
	/**
	 * The real reset code. This is a blocking call and should only be called when
	...
	public void formatChanged(Module src, Format oldFormat, Format newFormat) {
		...
		if (src instanceof BasicRendererModule && oldFormat instanceof VideoFormat
	...
	public void formatChangedFailure(Module src, Format oldFormat, Format newFormat) {
		...
		if (!internalErrorOccurred) {
			sendEvent(new InternalErrorEvent(this,
					"Internal module " + src + ": failed to handle a data format change!"));
	...
	public String getCNAME() {
		if (rtpInfo == null) {
			if ((rtpInfo = (RTPInfo) dsource.getControl(RTPInfo.class.getName())) == null)
				return null;
		}
	...
	/**
	 * Return a list of <b>Control</b> objects this <b>Controller</b> supports. In
	 * this case, it is all the controls from all the modules controlled by this
	 * engine.
	 *
	...
	/**
	 * Return the run-time latency. It's the time it takes for a packet to travel
	 * from the first module to the last module. The time is computed in
	 * nanoseconds.
	 */
	...
	/**
	 * Override the parent's method to not check for realized state. There's no need
	 * to.
	 */
	@Override
	public TimeBase getTimeBase() {
	...
	/**
	 * Override BasicController's setMediaTime so as not to set the media time on
	 * the master clock twice.
	 */
	@Override
