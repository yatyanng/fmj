...
public class BasicSourceModule extends BasicModule implements Duration, Positionable {
	 ...
	 */
	static protected Demultiplexer createDemultiplexer(DataSource ds) throws IOException, IncompatibleSourceException {
	...
	static public BasicSourceModule createModule(DataSource ds) throws IOException, IncompatibleSourceException {
	...
	public void doStart() {
		...
		for (int i = 0; i < loops.length; i++) {
			...
			if (tracks[i].isEnabled()) {
				if (loops[i] == null && (loops[i] = createSourceThread(i)) == null) {
					continue;
				}
				loops[i].start();
			}
...
class SourceThread extends LoopThread implements TrackListener {
	...
	static int remapTimeFlag = Buffer.FLAG_SYSTEM_TIME | Buffer.FLAG_RELATIVE_TIME | Buffer.FLAG_RTP_TIME;
	...
	protected boolean process() {
		...
		if (buffer.getTimeStamp() != Buffer.TIME_UNKNOWN && (buffer.getFlags() & remapTimeFlag) != 0) {
	...
	private boolean remapRelativeTime(Buffer buffer) {
		 ...
		 *
		 * if( lastRelativeTime < 0) { //System.out.println( "relative time reset!"); //
		 * Shift the time by .01 sec to avoid generating the // same time stamp after
		 * retart. currentTime += 10000000; } else { //System.out.println( "else...");
		 ...
		 */
		buffer.setFlags((buffer.getFlags() & ~Buffer.FLAG_RELATIVE_TIME) | Buffer.FLAG_NO_SYNC);
	...
	private boolean remapSystemTime(Buffer buffer) {
		...
		buffer.setFlags((buffer.getFlags() & ~Buffer.FLAG_SYSTEM_TIME) | Buffer.FLAG_NO_SYNC);
