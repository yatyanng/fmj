...
public class InputStreamReader extends InputStream {
	private static class ReaderThread extends CloseableThread {
		...
		public ReaderThread(final ProducerConsumerQueue emptyQueue, final ProducerConsumerQueue fullQueue,
				final InputStream is, final int bufferSize) {
			super();
	...
	public int read(byte[] b, int off, int len) throws IOException {
		try {
			...
			final int lenToCopy = readBuffer.getLength() < len ? readBuffer.getLength() : len;
			System.arraycopy(readBufferData, readBuffer.getOffset(), b, off, lenToCopy);
