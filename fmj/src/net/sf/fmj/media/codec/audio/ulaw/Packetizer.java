...
public class Packetizer extends AbstractPacketizer {
	...
	protected Format[] outputFormats = new Format[] {
			new AudioFormat(AudioFormat.ULAW_RTP, -1.0, 8, 1, -1, -1, 8, -1.0, Format.byteArray) };

	public Packetizer() {
		super();
		this.inputFormats = new Format[] {
				new AudioFormat(AudioFormat.ULAW, -1.0, 8, 1, -1, -1, 8, -1.0, Format.byteArray) };
	...
	public Format[] getSupportedOutputFormats(Format input) {
		if (input == null)
			...
			final AudioFormat result = new AudioFormat(AudioFormat.ULAW_RTP, inputCast.getSampleRate(), 8, 1,
					inputCast.getEndian(), inputCast.getSigned(), 8, inputCast.getFrameRate(), inputCast.getDataType());
