...
public class MuLawDecoderUtil {
	/**
	 * An array where the index is the mu-law input, and the value is the 16-bit PCM
	 * result.
	 *
	 */
	...
	private static short decode(byte mulaw) {
		...
		data |= 0x10;
		/*
		 * Add a 1 to the end of the data by shifting over and adding one. Why? Mu-law
		 * is not a one-to-one function. There is a range of values that all map to the
		 * same mu-law byte. Adding a one to the end essentially adds a "half byte",
		 * which means that the decoding will return the value in the middle of that
		 * range. Otherwise, the mu-law decoding would always be less than the original
		 * data.
		 */
		data <<= 1;
		data += 1;
		/*
		 * Shift the five bits to where they need to be: left (exponent + 2) places Why
		 * (exponent + 2) ? 1 2 3 4 5 6 7 8 9 A B C D E F G . 7 6 5 4 3 2 1 0 . . . . .
		 * . . <-- starting bit (based on exponent) . . . . . . . . . . 1 x x x x 1 <--
		 * our data We need to move the one under the value of the exponent, which means
		 * it must move (exponent + 2) times
		 */
		data <<= exponent + 2;
	 ...
	 *
	 * @param data    An array of mu-law encoded bytes
	 * @param decoded An array of bytes in Little-Endian format containing the
