...
public class Encoder extends AbstractCodec {
	...
	public Format[] getSupportedOutputFormats(Format input) {
		if (input == null) {
			...
			if (!inputCast.getEncoding().equals(AudioFormat.LINEAR)
					|| (inputCast.getSampleSizeInBits() != 16
			...
			final AudioFormat result = new AudioFormat(AudioFormat.ALAW, inputCast.getSampleRate(), 8, 1,
					inputCast.getEndian(), AudioFormat.SIGNED, 8, inputCast.getFrameRate(), Format.byteArray);
	...
	public int process(Buffer inputBuffer, Buffer outputBuffer) {
		...
		final boolean bigEndian = ((AudioFormat) inputBuffer.getFormat()).getEndian() == AudioFormat.BIG_ENDIAN; // TODO:
																													// check
																													// for
																													// undefined
		ALawEncoderUtil.aLawEncode(bigEndian, (byte[]) inputBuffer.getData(), inputBuffer.getOffset(),
				inputBuffer.getLength(), outputBufferData);
