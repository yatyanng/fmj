...
public class Encoder extends AbstractCodec {
	...
	protected Format[] outputFormats = new Format[] { new AudioFormat(AudioFormat.GSM, 8000, 8, 1, Format.NOT_SPECIFIED,
	...
	public Encoder() {
		super();
		this.inputFormats = new Format[] { new AudioFormat(AudioFormat.LINEAR, 8000, 16, 1, AudioFormat.BIG_ENDIAN,
				AudioFormat.SIGNED, Format.NOT_SPECIFIED, Format.NOT_SPECIFIED, Format.byteArray) };
	...
	public Format[] getSupportedOutputFormats(Format input) {
		if (input == null) {
			...
			final AudioFormat result = new AudioFormat(AudioFormat.GSM, inputCast.getSampleRate(), 8, 1,
					inputCast.getEndian(), AudioFormat.SIGNED, 264, inputCast.getFrameRate(), Format.byteArray);
	...
	public int process(Buffer inputBuffer, Buffer outputBuffer) {
		...
		if (innerDataLength < PCM_BYTES) {
			...
			GSMEncoderUtil.gsmEncode(bigEndian, (byte[]) innerBuffer.getData(), innerBuffer.getOffset(),
					innerDataLength, outputBufferData);
