...
public class Decoder extends AbstractCodec {
	...
	protected Format[] outputFormats = new Format[] { new AudioFormat(AudioFormat.LINEAR, 8000, 16, 1,
			Format.NOT_SPECIFIED, AudioFormat.SIGNED, Format.NOT_SPECIFIED, Format.NOT_SPECIFIED, Format.byteArray) };
	...
	public Format[] getSupportedOutputFormats(Format input) {
		if (input == null)
			...
			final AudioFormat result = new AudioFormat(AudioFormat.LINEAR, inputCast.getSampleRate(), 16, 1,
					inputCast.getEndian(), AudioFormat.SIGNED, 16, Format.NOT_SPECIFIED, Format.byteArray);
	...
	public int process(Buffer inputBuffer, Buffer outputBuffer) {
		...
		if (innerBuffer.getLength() < GSM_BYTES) {
			...
			GSMDecoderUtil.gsmDecode(bigEndian, (byte[]) innerBuffer.getData(), inputBuffer.getOffset(),
					innerBuffer.getLength(), outputBufferData);
