...
public class JpegStripper {
	...
	private static int findNextMarker(int[] output) {
		for (int i = 0; i < output.length; i++) {
			if (output[i] == 0xFF && (output[i + 1] != 0 && (output[i + 1] < 0xD0 || output[i + 1] > 0xD7))) {
				STRIP = true;
				return i;
			}
		}
	 ...
	 * JpegStripper does not check if data represent valid Jpeg file.
	 *
	 * @param ba Byte array representing Jpeg file
	...
	private static int[] stripLeadingHeaders(int[] input) {
		...
		if (input[0] == 0xFF) {
			switch (input[1]) { // strip following markers: (to add new it is usually enough to
