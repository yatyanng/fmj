...
public class RGBScaler extends BasicCodec {
	...
	public RGBScaler(Dimension sizeOut) {
		inputFormats = new Format[] { new RGBFormat(null, Format.NOT_SPECIFIED, Format.byteArray, Format.NOT_SPECIFIED,
				24, 3, 2, 1, 3, Format.NOT_SPECIFIED, Format.FALSE, Format.NOT_SPECIFIED) };
	...
	public Format[] getSupportedOutputFormats(Format input) {
		...
		if (matches(input, inputFormats) != null) {
			...
			VideoFormat frameRateFormat = new VideoFormat(null, null, Format.NOT_SPECIFIED, null, frameRate);
	...
	private native void nativeScale(Object inData, long inBytes, Object outData, long outBytes, int psIn, int lsIn,
			int wIn, int hIn, int psOut, int lsOut, int wOut, int hOut);
	...
	protected void nearestNeighbour(Buffer inBuffer, Buffer outBuffer) {
		...
		if (nativeAvailable) {
			try {
				nativeScale(inObj, inBytes, outObj, outBytes, pixStrideIn, lineStrideIn, sizeIn.width, sizeIn.height,
						pixStrideOut, lineStrideOut, sizeOut.width, sizeOut.height);
			} catch (Throwable t) {
	...
	public Format setOutputFormat(Format output) {
		...
		outputFormat = outputFormats[0].intersects(new RGBFormat(size, maxDataLength, null, frameRate,
				Format.NOT_SPECIFIED, Format.NOT_SPECIFIED, Format.NOT_SPECIFIED, Format.NOT_SPECIFIED,
				Format.NOT_SPECIFIED, lineStride, Format.NOT_SPECIFIED, Format.NOT_SPECIFIED));
		return outputFormat;
	}

	public void setOutputSize(Dimension sizeOut) {
		outputFormats = new Format[] { new RGBFormat(sizeOut, sizeOut.width * sizeOut.height * 3, Format.byteArray,
				Format.NOT_SPECIFIED, 24, 3, 2, 1, 3, sizeOut.width * 3, Format.FALSE, Format.NOT_SPECIFIED) };
	}
}
