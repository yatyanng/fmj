...
public class JpegRTPHeader {
	...
	/**
	 * Creates header containing Quantization tables (used when Q is set to values
	 * greater than 127).
	 *
	 * @param length length for the headers (usually 128 - two 64 bytes long tables)
	...
	public static byte[] createQHeader(int length, int[] lqt, int[] cqt) {
		...
		if (length != 0) {
			...
			i += lqt.length;
			System.arraycopy(ArrayUtility.intArrayToByteArray(zzCqt), 0, data, i, cqt.length);
			i += cqt.length;
		}
		return data;
	}
	 ...
	 *
	 * @param dri   Restart interval - number of MCUs between restart markers
	 * @param f     first bit (should be set to 1)
	 * @param l     last bit (should be set to 1)
	 * @param count number of restart markers (should be set to 3FFF)
	 * @return RST header
	 */
	...
	public JpegRTPHeader(final byte typeSpecific, final int fragmentOffset, final byte type, final byte q,
			final byte width, final byte height) {
		super();
	...
	public boolean equals(Object o) {
		...
		return this.typeSpecific == oCast.typeSpecific && this.fragmentOffset == oCast.fragmentOffset
				&& this.type == oCast.type && this.q == oCast.q && this.width == oCast.width
				&& this.height == oCast.height;
	...
	public String toString() {
		return "typeSpecific=" + getTypeSpecific() + " fragmentOffset=" + getFragmentOffset() + " type=" + getType()
				+ " q=" + getQ() + " w=" + getWidthInPixels() + " h=" + getHeightInPixels();
	}
}
