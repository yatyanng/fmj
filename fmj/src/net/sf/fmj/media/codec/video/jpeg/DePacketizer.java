...
public class DePacketizer extends AbstractCodec implements Codec {
	/**
	 * Compares buffers by the fragment offset. Assumes buffers have enough data in
	 * them for a JpegRTPHeader.
	 *
	 * @author Ken Larson
	 */
	private static class BufferFragmentOffsetComparator implements Comparator<Buffer> {
		...
		public int compare(Buffer a, Buffer b) {
			...
			final JpegRTPHeader jpegRtpHeaderA = JpegRTPHeader.parse((byte[]) a.getData(), a.getOffset());
			final JpegRTPHeader jpegRtpHeaderB = JpegRTPHeader.parse((byte[]) b.getData(), b.getOffset());
	...
	static class FrameAssembler {
		...
		/**
		 * @return false if any fragments are missing. Does not detect fragments missing
		 *         at the end.
		 */
		private boolean contiguous() {
			...
			for (Buffer b : list) {
				...
				if (jpegRtpHeader.getQ() >= 128) {
					int length = 0;
					int j = b.getOffset() + JpegRTPHeader.HEADER_SIZE + otherOffset + 2; // the size of the offset to
																							// the
		...
		public int copyToBuffer(Buffer bDest) {
			...
			final boolean prependHeader = !hasJPEGHeaders((byte[]) bFirst.getData(),
					bFirst.getOffset() + JpegRTPHeader.HEADER_SIZE, bFirst.getLength() - JpegRTPHeader.HEADER_SIZE);
			...
			if (bDest.getData() != null
					&& ((byte[]) bDest.getData()).length >= (frameLength + MAX_HEADER + MAX_TRAILER)) {
			 ...
			 * Integer.toHexString(b&0xFF); System.out.print((s.length()<2?"0"+s:s)+"-"); }
			 * System.out.println("");
			 */
			...
			if (prependHeader) {
				...
				if (jpegRtpHeaderFirst.getType() >= 64 && jpegRtpHeaderFirst.getType() <= 127) { // For types 64-127
																									// there has to be
																									// restart header
				...
				// handle Reset Header
				offsetAfterHeaders = RFC2035.MakeHeaders(false, data, offsetAfterHeaders, jpegRtpHeaderFirst.getType(),
						jpegRtpHeaderFirst.getQ(), jpegRtpHeaderFirst.getWidthInBlocks(),
			...
			for (Buffer b : list) {
				...
				System.arraycopy(b.getData(), b.getOffset() + JpegRTPHeader.HEADER_SIZE, data,
						offsetAfterHeaders + jpegRtpHeader.getFragmentOffset(),
						b.getLength() - JpegRTPHeader.HEADER_SIZE);
			}

			final boolean appendEOI = !hasJPEGTrailer(data, offsetAfterHeaders + frameLength, MAX_TRAILER); // no need
																											// to append
																											// if it is
																											// already
																											// there.
		...
		/**
		 * Total length of all fragments. Does not include JPEG header. Assumes that
		 * complete() has been called and returns true.
		 */
		...
		/**
		 * Add the buffer (which contains a fragment) to the assembler. Should be a
		 * clone of a real buffer, since the buffer will be kept around.
		 *
		...
		public void put(Buffer buffer) {
			...
			if (TRACE)
				System.out.println("adding buffer seq=" + buffer.getSequenceNumber() + " ts=" + buffer.getTimeStamp());
	...
	private static class FrameAssemblerCollection {
		private Map<Long /* timestamp */, FrameAssembler> frameAssemblers = new HashMap<Long, FrameAssembler>();
		...
		public void removeAllButNewestN(int n) {
			while (frameAssemblers.size() > n) {
				...
				if (TRACE) {
					System.out.println("Discarding " + completeIncomplete + " frame (not in newest " + n + ", ts="
							+ oldestTimestamp);
				}
		...
		public void removeOlderThan(long timestamp) {
			final Iterator<Entry<Long, FrameAssembler>> i = frameAssemblers.entrySet().iterator();
			while (i.hasNext()) {
				...
				if (entryTimestamp.longValue() < timestamp) {
					if (TRACE) {
						System.out.println(
								"Discarding incomplete frame older than " + timestamp + ", ts=" + entryTimestamp);
					}
					i.remove();
				}
			}
	...
	private static void dump(Buffer b, String name) {
		...
		if (TRACE)
			System.out.println(dump((byte[]) b.getData(), b.getOffset(),
					b.getLength() > MAX_DUMP_SIZE ? MAX_DUMP_SIZE : b.getLength()));

	}

	/**
	 * Debugging only. In this version, len is the length to dump to the string, not
	 * the length minus offset as in StringUtils. Returns the string instead of
	 * pringing to std out.
	 */
	...
	/**
	 * Because packets can come out of order, it is possible that some packets for a
	 * newer frame may arrive while an older frame is still incomplete. However, in
	 * the case where we get nothing but incomplete frames, we don't want to keep
	 * all of them around forever.
	 */
	...
	public DePacketizer() {
		if (COMPARE_WITH_BASELINE) {
			try {
				...
				System.out.println("Unable to instantiate com.sun.media.codec.video.jpeg.DePacketizer"); // will
																											// happen
																											// if
																											// JMF
																											// not
	...
	public Format[] getSupportedOutputFormats(Format input) {
		...
		final Format[] result = new Format[] {
				new JPEGFormat(inputCast.getSize() != null ? inputCast.getSize() : HARD_CODED_SIZE, -1,
						Format.byteArray, -1.0f, -1, -1) };
	...
	public int process(Buffer input, Buffer output) {
		...
		if (!input.isDiscard()) {
			...
			final JpegRTPHeader jpegRtpHeader = input.getLength() >= JpegRTPHeader.HEADER_SIZE
					? JpegRTPHeader.parse((byte[]) input.getData(), input.getOffset())
					: null;
			...
			if (assembler.complete()) {
				...
				if (TRACE)
					System.out.println("### " + ((System.nanoTime() - lastTimestamp) / 1000000L) + " "
				...
				frameAssemblers.removeAllButNewestN(MAX_ACTIVE_FRAME_ASSEMBLERS); // weed
																					// out
																					// incomplete
																					// frames
																					// that
