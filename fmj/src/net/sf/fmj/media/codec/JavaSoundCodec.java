...
public class JavaSoundCodec extends AbstractCodec {
	 ...
	 * http://en.wikipedia.org/wiki/Au_file_format. The Au file format is a simple
	 * audio file format that consists of a header of 6 32-bit words and then the
	 * data (high-order byte comes first). The format was introduced by Sun
	 * Microsystems.
	 */
	...
	public static byte[] createWavHeader(javax.sound.sampled.AudioFormat f) {
		...
		encodeIntLE((((int) f.getSampleRate()) * f.getChannels() * f.getSampleSizeInBits()) / 8, result, 28); // ByteRate
																												// ==
																												// SampleRate
																												// *
																												// NumChannels
		...
		encodeShortLE((short) ((f.getChannels() * f.getSampleSizeInBits()) / 8), result, 32); // BlockAlign ==
																								// NumChannels *
																								// BitsPerSample/8 The
																								// number of bytes for
																								// one sample including
																								// all
																								// channels. I wonder
																								// what happens when
																								// this number
	 ...
	 * does not let us specify the input format. It gets the input format by reading
	 * the stream. However, the parser has already stripped off the headers, and the
	 * data we are getting is pure. To use JavaSound, we have to create a fake
	 * header based on the input format information. It does not matter whether this
	 * header matches the original header, it just has to get
	 * AudioSystem.getAudioInputStream To figure out what kind of format it is.
	 *
	 */
	...
	public JavaSoundCodec() {
		...
		if (!JavaSoundUtils.onlyStandardFormats) {
			...
			try {
				...
				final String[] mpegEncodingStrings = { "MPEG1L1", "MPEG1L2", "MPEG1L3", "MPEG2DOT5L1", "MPEG2DOT5L2",
						"MPEG2DOT5L3", "MPEG2L1", "MPEG2L2", "MPEG2L3" };
	...
	public Format[] getSupportedOutputFormats(Format input) {
		...
		final javax.sound.sampled.AudioFormat[] targets1 = AudioSystem.getTargetFormats(Encoding.PCM_UNSIGNED,
				javaSoundFormat);
		final javax.sound.sampled.AudioFormat[] targets2 = AudioSystem.getTargetFormats(Encoding.PCM_SIGNED,
				javaSoundFormat);
		...
		if ((null != classMpegAudioFormatt) && classMpegAudioFormatt.isInstance(javaSoundFormat)) {
			javax.sound.sampled.AudioFormat decodedFormat = new javax.sound.sampled.AudioFormat(
					javax.sound.sampled.AudioFormat.Encoding.PCM_SIGNED, javaSoundFormat.getSampleRate(), 16,
					javaSoundFormat.getChannels(), javaSoundFormat.getChannels() * 2, javaSoundFormat.getSampleRate(),
					false);
		...
		try {
			...
			if (audioInputStreamConverted == null) {
				if (noRoomInBufferQueue) {
					...
					logger.fine(
							"JavaSoundCodec: audioInputStreamConverted == null, returning OUTPUT_BUFFER_NOT_FILLED");
			...
			if (trace)
				logger.fine("audioInputStreamConverted.available() == " + avail
						+ ", bufferQueueInputStream.available() = " + bufferQueueInputStream.available());
			...
			totalOut += lenRead;
			return (noRoomInBufferQueue || input.isEOM()) ? INPUT_BUFFER_NOT_CONSUMED : BUFFER_PROCESSED_OK; // if input
																												// is
																												// EOM,
																												// then
																												// they
