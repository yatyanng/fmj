 ...
 */
public class CloneableCaptureDevicePushBufferDataSource extends PushBufferDataSource
	 ...
	 *
	 * @exception IOException Thrown if there are IO problems when <tt>connect</tt>
	 *                        is called.
	 */
	@Override
	 ...
	 * data source was a PullDataSource, then this will be a PushDataSource which
	 * pushes at the same rate at which the CloneableDataSource is being pulled.
	 *
	 ...
	 * <p>
	 * The <tt>disconnect</tt> method frees resources used to maintain a connection
	 * to the source. If no resources are in use, <tt>disconnect</tt> is ignored. If
	 * <tt>stop</tt> hasn't already been called, calling <tt>disconnect</tt> implies
	 * a stop.
	 *
	 */
	@Override
	...
	/**
	 * Get a string that describes the content-type of the media that the source is
	 * providing.
	 * <p>
	...
	/**
	 * Get the duration of the media represented by this object. The value returned
	 * is the media's duration when played at the default rate. If the duration
	 * can't be determined (for example, the media object is presenting live video)
	 * <tt>getDuration</tt> returns <tt>DURATION_UNKNOWN</tt>.
	 *
	 * @return A <tt>Time</tt> object representing the duration or DURATION_UNKNOWN.
	 */
	@Override
	...
	/**
	 * Returns an array of <tt>FormatControl</tt> objects. Each of them can be used
	 * to set and get the format of each capture stream. This method can be used
	 ...
	 * streams is entirely content dependent. The <tt>ContentDescriptor</tt> of this
	 * <tt>DataSource</tt> provides the only indication of what streams can be
	...
	public PushBufferStream[] getStreams() {
		if (superClass.streams == null) {
			...
			for (int i = 0; i < superClass.streamsAdapters.length; i++)
				superClass.streams[i] = superClass.streamsAdapters[i].getAdapter();
		}
	...
	/**
	 * Initiate data-transfer. The <tt>start</tt> method must be called before data
	 * is available. (You must call <tt>connect</tt> before calling <tt>start</tt>.)
	 *
	 * @exception IOException Thrown if there are IO problems with the source when
