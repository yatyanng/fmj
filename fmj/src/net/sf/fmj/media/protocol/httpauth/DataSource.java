...
public class DataSource extends PullDataSource implements SourceCloneable {
	class URLSourceStream implements PullSourceStream {
		...
		public int read(byte[] buffer, int offset, int length) throws IOException {
			final int result = conn.getInputStream().read(buffer, offset, length); // TODO: does this handle the
																					// requirement of not
	...
	public void connect() throws IOException {
		...
		if (conn instanceof HttpURLConnection) { // TODO: this is probably why JMF has explicit HTTP and FTP data
			...
			if (user != null && !user.equals("")) {
				huc.setRequestProperty("Authorization",
						"Basic " + StringUtils.byteArrayToBase64String((user + ":" + pass).getBytes()));
			}
			huc.connect();
