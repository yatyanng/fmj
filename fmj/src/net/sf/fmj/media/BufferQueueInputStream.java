...
public class BufferQueueInputStream extends InputStream {
		...
		try {
			synchronized (q) {
				do {
					...
					buffer = (Buffer) q.get();
					if (trace)
						logger.fine(this + " Getting buffer: " + buffer.getLength());
	...
	private boolean put(Buffer b, boolean block, boolean clone) {
		...
		if (trace)
			logger.fine(this + " BufferQueueInputStream.put: Putting buffer, length=" + b.getLength() + " eom="
					+ b.isEOM());
		try {
			synchronized (q) {
				if (!block && q.isFull())
	...
	public int read(byte[] b, int off, int len) throws IOException {
		...
		if (data == null)
			throw new NullPointerException(
					"Buffer has null data.  length=" + buffer.getLength() + " offset=" + buffer.getOffset());

		int lengthToCopy = buffer.getLength() < len ? buffer.getLength() : len;
		if (trace)
			logger.fine(this + " BufferQueueInputStream.read: lengthToCopy=" + lengthToCopy + " buffer.getLength()="
					+ buffer.getLength() + " buffer.getOffset()=" + buffer.getOffset() + " b.length=" + b.length
					+ " len=" + len + " off=" + off);
