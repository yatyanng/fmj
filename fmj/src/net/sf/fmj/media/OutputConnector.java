...
public interface OutputConnector extends Connector {
	 ...
	 *
	 * @param inputConnector input connector to check connection to.
	 * @param useThisFormat  states the format of the connection. If it is null the
	 *                       format would be negotiated.
	...
	/**
	 * Connects an InputConnector to this OutputConnector. This method should only
	 * be called by the Manager when it is connecting this OutputConnector.<br>
	 * The protocol of the two connectors must match. <br>
	 * the number of buffer objects in the created connection should be at least the
	 * size requested by both the input and the output connector
	 *
	 * @param inputConnector input connector to connect to.
	 * @param useThisFormat  states the format of the connection. If it is null the
	 *                       format would be negotiated.
	...
	/**
	 * Indicates the oldest Buffer object got from this Connector (by calling the
	 * getEmptyBuffer method) now contains valid buffer object, which can be used by
	 * the downstream Module.<br>
	 *
	 * if such buffer Objects does not exists throws RuntimeException.
	 *
	 * The exact behavior depands on the protocol:
	 * <ul>
	 * <li><b>ProtocolPush</b> - if bufferValid call the connected InputConnector
	 * Module connectorPushed() method in the calling thread. method blocks until
	 * the downstream module finishes processing the buffer.</li>
	 * <li><b>ProtocolSafe</b> - perform notify() on the connection lock object.
	 * <br>
	 * </ul>
	 *
	 * <i>The bufferValid flag was removed. If the buffer Object is just returned,
	 * it should be marked in the Buffer itself, by specifying this is a discarded
	 * Buffer chunk. </i>
	 *
