...
public abstract class BasicPlayer extends BasicController
	 ...
	 * prefetching. Release all resources claimed previously by the prefetch call.
	 */
	@Override
	 ...
	 * realizing. Release all resources claimed previously by the realize() call.
	 */
	@Override
	...
	/**
	 * Assume control of another Controller. A Player can accept responsibility for
	 * controlling another Controller. Once a Controller has been added this Player
	 * will:
	 * <ul>
	 * <li>Slave the Controller to the Player's time-base.
	 * <li>Use the Controller in the Player's computation of start latency. The
	 * value the Player returns in its <b>getStartLatency</b> method is the larger
	 ...
	 * <li>For all asynchronous methods (realize, prefetch) a completion event will
	 * not be generated until all added Controllers have generated completion
	 * events.
	 * </ul>
	 * <p>
	 *
	 * <b>Note:</b> It is undefined what will happen if a Controller is under the
	 * control of a Player and any of the Controller's methods are called outside of
	 * the controlling Player.
	 *
	 * @param newController the Controller this Player will control.
	 * @exception IncompatibleTimeBaseException thrown if the new controller will
	 *                                          not accept the player's timebase.
	 */
	@Override
	public synchronized void addController(Controller newController) throws IncompatibleTimeBaseException {
		...
		if (playerState == Prefetched) {
			if ((controllerState == Realized) || (controllerState == Prefetching)) {
	...
	/**
	 * Check the given controller to see if it's busy or not. Needs to be overridden
	 * by subclass. The subclass method should change the master timebase if
	 * necessary. It should handle audio only or video only tracks properly when the
	 * device is busy.
	 *
	...
	/**
	 * This is called when close() is invoked on the Player. close() takes care of
	 * the general behavior before invoking doClose(). Subclasses should implement
	 * this only if it needs to do something specific to close the player.
	 */
	@Override
	 ...
	 * </ul>
	 * Subclasses are allowed to override doConfigure(). But this should be done in
	 * caution. Subclass should also invoke super.doConfigure(). This is called from
	 * a separately running thread.
	 *
	...
	protected synchronized boolean doConfigure() {
		...
		while (--i >= 0) {
			Controller c = (Controller) controllerList.elementAt(i);
			if (c.getState() == Unrealized && (c instanceof Processor || c instanceof BasicController)) {
		...
		while (--i >= 0) {
			Controller c = (Controller) controllerList.elementAt(i);
			if ((c instanceof Processor || c instanceof BasicController) && c.getState() < Configured) {
	...
	/**
	 * The stub function to perform the steps to prefetch the controller. This will
	 * call prefetch() on every controller in the controller list and wait for their
	 ...
	 * </ul>
	 * Subclasses are allowed to override doRealize(). But this should be done in
	 * caution. Subclass should also invoke super.doRealize(). This is called from a
	...
	public void downloadUpdate() {
		...
		int maxValuePercent;
		if ((contentLength == javax.media.protocol.SourceStream.LENGTH_UNKNOWN) || (contentLength <= 0)) {
			maxValuePercent = 0;
		} else {
	 ...
	 * Subclasses should override this method and return the control panel component
	 * but call this method first to ensure that the restrictions on player methods
	 * are enforced.
	 *
	...
	/**
	 * Get the object for controlling audio gain. Return null if this player does
	 * not have a GainControl (e.g. no audio).
	 *
	...
	/**
	 * This should be implemented by the subclass. The subclass method should return
	 * the master TimeBase -- the TimeBase that all other controllers slave to. Use
	 * SystemTimeBase if unsure.
	 *
	...
	/**
	 * Get the Component this player will output its visual media to. If this player
	 * has no visual component (e.g. audio only) getVisualComponent() will return
	 * null. Subclasses should override this method and return the visual component
	 * but call this method first to ensure that the restrictions on player methods
	 * are enforced.
	 *
	...
	/**
	 * Will return true if the player can do frame positioning. Hack for now, should
	 * be removed when players actually implement the framePositioning control.
	 */
	 ...
	 * This is a protected method use only by subclasses. Use addController() for
	 * public access.
	 */
	...
	private/* synchronized */void notifyIfAllEventsArrived(Vector controllerList, Vector receivedEventList) {
	...
	final synchronized void play() {
		...
		while (!closing && !prefetchFailed && (getState() == Configuring || getState() == Realizing
		...
		if (getState() != Started && getTargetState() == Started && getState() == Prefetched) {
	...
	protected/* synchronized */void processEvent(ControllerEvent evt) {
		...
		if (evt instanceof ControllerClosedEvent && !closing && controllerList.contains(source)
		...
		if ((evt instanceof SizeChangeEvent) && controllerList.contains(source)) {
			...
			sendEvent(new SizeChangeEvent(this, ((SizeChangeEvent) evt).getWidth(), ((SizeChangeEvent) evt).getHeight(),
		 ...
		 * UnsupportedFormatEvent(this, ((UnsupportedFormatEvent)evt).getFormat()));
		 * return; }
		 */
		...
		if ((evt instanceof RestartingEvent) && controllerList.contains(source)) {
			...
			sendEvent(new RestartingEvent(this, Started, Prefetching, Started, getMediaTime()));
		}
		...
		if ((evt instanceof SeekFailedEvent) && controllerList.contains(source)) {
			 ...
			 */
			sendEvent(new SeekFailedEvent(this, Started, Prefetched, Prefetched, getMediaTime()));
		}
		...
		if ((evt instanceof StopAtTimeEvent) && controllerList.contains(source) && (getState() == Started)) {
		...
		if (controllerList != null && controllerList.contains(source) && eventList != null
	...
	/**
	 * Loops through the list of controllers maintained by this player and invoke
	 * setMediaTime on each of them. This is a "final" method and cannot be
	...
	/**
	 * Set the playback rate on the player. It loops through its list of controllers
	 * and invoke setRate on each of them.
	 */
	@Override
	...
	/**
	 * Set the DataSource that provides the media for this player. BasicPlayer only
	 * supports PullDataSource by default. Subclasses can override this method to
	 * support other DataSources.
	 *
	 * @param source of media for this player.
	 * @exception IOException                 thrown when an i/o error occurs in
	 *                                        reading information from the data
	 *                                        source.
	 * @exception IncompatibleSourceException thrown if the Player can't use this
	 *                                        source.
	 */
	@Override
	public void setSource(javax.media.protocol.DataSource source) throws IOException, IncompatibleSourceException {
		...
		try {
			cachingControl = (CachingControl) source.getControl(CachingControl.class.getName());
	...
	/**
	 * Assigns a timebase for the player. If the BasicPlayer plays back audio, the
	 * timebase can be none other than the master timebase as returned by
	 * getMasterTimeBase(). This is to ensure that we won't set to a timebase the
	 * audio cannot handle at this point. If the playback is video only, the
	 * timebase can be set to any timebase desired.
	 *
	 * @param tb time base to be used by the Player.
	 * @exception IncompatibleTimeBaseException thrown when a time base other than
	 *                                          the master time base is set when
	 *                                          audio is enabled.
	 */
	@Override
	...
	/**
	 * Slave all the controllers to the master time base. The controllers should be
	 * in realized or greater state This differs from the setTimeBase() as it loops
	 * through each controllers and call setTimeBase on each of them.
	 *
	 * @param tb the time base to be used by all controllers.
	 * @exception IncompatibleTimeBaseException thrown if any controller will not
	 *                                          accept the player's timebase.
	 */
	protected void slaveToMasterTimeBase(TimeBase tb) throws javax.media.IncompatibleTimeBaseException {
	...
	/**
	 * Start the Player as soon as possible. Start attempts to transition the player
	 * into the started state. If the player has not been realized, or prefetched,
	 * then the equivalent of those actions will occur, and the appropriate events
	 * will be generated. If the implied realize or prefetch fail, a failure event
	 * will be generated and the Player will remain in one of the non-started
	 * states.
	 * <p>
	 * This is a "final" method. Subclasses should override doStart() to implement
	...
	public final void start() {
		synchronized (startSync) {
			...
			if (getState() == Started) {
				sendEvent(new StartEvent(this, Started, Started, Started, mediaTimeAtStart, startTime));
	...
	/**
	 * Stop the player. If current state is Started, sends stop() to all the managed
	 * controllers, and waits for a StopEvent from all of them. It then sends a
	 * StopEvent to any listener(s).
	 */
	@Override
	...
	private/* synchronized */void stop(int stopType) {
		...
		if (getState() != Started) {
			...
			case STOP_BY_REQUEST:
				...
				break;
			case RESTARTING:
				sendEvent(new RestartingEvent(this, getState(), getState(), Started, getMediaTime()));
				break;
			default:
			...
			synchronized (this) {
				...
				switch (stopType) {
				case STOP_BY_REQUEST:
					sendEvent(new StopByRequestEvent(this, Started, getState(), getTargetState(), getMediaTime()));
					break;
				case RESTARTING:
					sendEvent(new RestartingEvent(this, Started, getState(), Started, getMediaTime()));
					break;
				default:
					sendEvent(new StopEvent(this, Started, getState(), getTargetState(), getMediaTime()));
					break;
				}
			}
		}
	...
	/**
	 * Start at the given time base time. This overrides Clock.syncStart() and obeys
	 * all its semantics.
	 * <p>
	 * This is a "final" method. Subclasses should override doStart() to implement
	 * its own specific behavior.
