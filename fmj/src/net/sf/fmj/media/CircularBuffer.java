...
public class CircularBuffer {
	/**
	 * buffer to hold the references. Would be changed to buffer array, when Buffer
	 * stabalize
	 */
	private Buffer buf[];
	/** write pointer */
	...
	public void print() {
		System.err.println("CircularQueue : head=" + head + " tail=" + tail + " canRead=" + availableFramesForReading
				+ " canWrite=" + availableFramesForWriting + " lockedRead=" + lockedFramesForReading + " lockedWrite="
				+ lockedFramesForWriting);
	}
	...
	public synchronized void writeReport() {
		 ...
		 * { availableFramesForWriting++; // buffer not written tail--; if (tail<0) tail
		 * += size; }
		 */
	}
}
