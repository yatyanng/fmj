...
class XMLRegistryIO implements RegistryIO {
	...
	private Element getCaptureDeviceElement(Document document) throws IOException {
		final Element captureDeviceElement = document.createElement(ELEMENT_CAPTURE_DEVICES);
		...
		while (iter.hasNext()) {
			...
			final Element deviceElement = document.createElement(ELEMENT_DEVICE);
			{
				final Element deviceNameElement = document.createElement(ELEMENT_DEVICE_NAME);
				deviceNameElement.setTextContent(info.getName());
				deviceElement.appendChild(deviceNameElement);
			}

			{
				final Element e = document.createElement(ELEMENT_DEVICE_LOCATOR);
			...
			}
			{
				...
				for (int i = 0; i < formats.length; ++i) {
					final Element formatElement = document.createElement(ELEMENT_DEVICE_FORMAT);

					{
						final Element e2 = document.createElement(ELEMENT_DEVICE_FORMAT_CLASS); // for
																								// XML
																								// readability
																								// only
					...
					{
						final Element e2 = document.createElement(ELEMENT_DEVICE_FORMAT_DESCRIPTION); // for
																										// XML
																										// readability
																										// only
					...
					// that are not in JMF.
					{
						final Element e2 = document.createElement(ELEMENT_DEVICE_FORMAT_SERIALIZED);
	...
	private Element getContentElement(Document document) {
		...
		Iterator<String> prefixIter = registryContents.contentPrefixList.iterator();
	...
	private Element getDemuxElement(Document document) {
		return getPluginElement(PlugInManager.DEMULTIPLEXER, ELEMENT_DEMUXES, document);
	}
	...
	private Element getMimeElement(Document document) {
		...
		Iterator<String> typesIterator = registryContents.mimeTable.getMimeTypes().iterator();
		while (typesIterator.hasNext()) {
			String type = typesIterator.next();
			List<String> extensions = registryContents.mimeTable.getExtensions(type);

			Element typeElement = document.createElement(ELEMENT_MIMETYPE);
			typeElement.setAttribute("value", type);
			typeElement.setAttribute("default-ext", registryContents.mimeTable.getDefaultExtension(type));
	...
	private Element getMuxElement(Document document) {
		return getPluginElement(PlugInManager.MULTIPLEXER, ELEMENT_MUXES, document);
	}
	...
	private Element getProtocolElement(Document document) {
		...
		Iterator<String> prefixIter = registryContents.protocolPrefixList.iterator();
		while (prefixIter.hasNext()) {
			String prefix = prefixIter.next();
			Element prefixElement = document.createElement(ELEMENT_PREFIX);
			prefixElement.setTextContent(prefix);
	...
	private Element getRendererElement(Document document) {
		return getPluginElement(PlugInManager.RENDERER, ELEMENT_RENDERERS, document);
	}
	...
	private void loadCaptureDevices(Element element) throws IOException, ClassNotFoundException {
		...
		for (int i = 0; i < list.size(); i++) {
			...
			List<Element> formatElementsList = getChildren(deviceElement, ELEMENT_DEVICE_FORMAT);
			javax.media.Format[] formats = new javax.media.Format[formatElementsList.size()];
			for (int j = 0; j < formatElementsList.size(); ++j) {
				Element formatElement = formatElementsList.get(j);
				Element serializedElement = getChild(formatElement, ELEMENT_DEVICE_FORMAT_SERIALIZED);
				formats[j] = SerializationUtils.deserialize(getTextTrim(serializedElement));
			}

			CaptureDeviceInfo info = new CaptureDeviceInfo(getTextTrim(deviceNameElement),
					new MediaLocator(getTextTrim(deviceLocatorElement)), formats);
	...
	private void loadDocument(Document document) throws IOException {
		...
		Element contentPrefixesElement = getChild(rootElement, ELEMENT_CONTENT_PREFIX);
		loadContentPrefixes(contentPrefixesElement);

		Element protocolPrefixesElement = getChild(rootElement, ELEMENT_PROTO_PREFIX);
		...
		Element captureDevicesElement = getChild(rootElement, ELEMENT_CAPTURE_DEVICES);
		try {
