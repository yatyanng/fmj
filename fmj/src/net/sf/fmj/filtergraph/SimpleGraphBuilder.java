...
public class SimpleGraphBuilder {
	...
	/**
	 * Find a codec that can handle the given input and output. The output argument
	 * can be null if no specific output format is required.
	 */
	static public Codec findCodec(Format in, Format out, Format selectedIn[], Format selectedOut[]) {
		Vector cnames = PlugInManager.getPlugInList(in, out, PlugInManager.CODEC);
		if (cnames == null) {
		...
		for (int i = 0; i < cnames.size(); i++) {
			if ((c = (Codec) createPlugIn((String) cnames.elementAt(i), PlugInManager.CODEC)) == null)
				continue;
	...
	static public Renderer findRenderer(Format in) {
		Vector names = PlugInManager.getPlugInList(in, null, PlugInManager.RENDERER);
		if (names == null) {
		...
		for (int i = 0; i < names.size(); i++) {
			if ((r = (Renderer) createPlugIn((String) names.elementAt(i), PlugInManager.RENDERER)) == null)
				continue;
	 ...
	 * findCodec and findRenderer, it uses the same graph building algorithm that
	 * the media engine uses to determine the best rendering path for a particular
	 * input format. The return value is a vector of plugins of all the codecs and
	 * the renderer. The plugin list is in reverse order starting from the renderer.
	 * The list of the corresponding input formats for each codec is also returned
	...
	/**
	 * Given a codec class name, instantiate the codec and query it dynamically to
	 * see if it supports the given input and output formats.
	 */
	static public GraphNode getPlugInNode(String name, int type, Map<String, GraphNode> plugIns) {
		GraphNode gn = null;
	...
	/**
	 * Choose a format among the two input arrays that matches and verify that if
	 * the given upstream and downstream plugins accept the matched format as output
	 * (for the upstream) or as input (for the downstream). Either of the plugin
	 * arguments can be null. In which case the verification step will be skipped
	 * accordingly.
	 *
	...
	protected Hashtable<String, GraphNode> plugIns = new Hashtable<String, GraphNode>(40);
	...
	/**
	 * Given the intermediate search candidates, build a graph until it reaches a
	 * target.
	 */
	 ...
	 * invoked to see if the graph can actually be built. Subclass should implement
	 * this.
	 */
...
//        }
	 ...
	 * candiates list and mark it in the registry as as failed node so it won't be
	 * attempted again. This method is upated by hsy on 10/10/2000. If we just
	 * remove the failed node from candidates and mark it in the registy, we can't
	 * guarantee we could roll back to the exact/desired node/state to re-start
	 * searching, since this node might have already been removed from candidates.
	 * The simple/straightforward fix is to clear up candidates and plugIns and
	 * start allover again.
	 */
	...
	protected boolean setDefaultTargetRenderer(Format in) {
		...
		if (in instanceof AudioFormat) {
			targetPluginNames = PlugInManager.getPlugInList(new AudioFormat(null, Format.NOT_SPECIFIED,
					Format.NOT_SPECIFIED, Format.NOT_SPECIFIED, Format.NOT_SPECIFIED, Format.NOT_SPECIFIED,
					Format.NOT_SPECIFIED, Format.NOT_SPECIFIED, null), null, PlugInManager.RENDERER);
		} else if (in instanceof VideoFormat) {
			targetPluginNames = PlugInManager
					.getPlugInList(new VideoFormat(null, null, Format.NOT_SPECIFIED, null, Format.NOT_SPECIFIED // frameRate
																												// ???
					), null, PlugInManager.RENDERER);
		} else {
			targetPluginNames = PlugInManager.getPlugInList(null, null, PlugInManager.RENDERER);
		}
	...
	protected GraphNode verifyGraph(GraphNode node) {
		...
		while (node != null && node.plugin != null) {
			...
			} else if ((node.type == -1 || node.type == PlugInManager.CODEC) && node.plugin instanceof Codec) {
			...
			if ((fmt = matches(outs, gn.getSupportedInputs(), node.plugin, gn.plugin)) != null) {
				...
				if (inspector != null) {
					if (node.plugin != null && !inspector.verify((Codec) node.plugin, node.input, fmt))
						continue;
					if ((gn.type == -1 || gn.type == PlugInManager.CODEC) && gn.plugin instanceof Codec) {
						if (!inspector.verify((Codec) gn.plugin, fmt, null))
							continue;
					} else if ((gn.type == -1 || gn.type == PlugInManager.RENDERER) && gn.plugin instanceof Renderer) {
