...
public final class ClasspathChecker {
	...
	public static boolean check() {
		...
		if (!checkFMJPrefixInPackageManager()) {
			// logger.warning("net.sf.fmj not found in PackageManager.getContentPrefixList()
			// and PackageManager.getProtocolPrefixList(); is JMF ahead of FMJ in the
			// classpath?");
			result = false;
		}

		if (!checkManagerImplementation()) {
			// logger.warning("javax.media.Manager is JMF's implementation, not FMJ's; is
			// JMF ahead of FMJ in the classpath?");
			result = false;
		}
	...
	public static boolean checkAndWarn() {
		...
		if (!checkFMJPrefixInPackageManager()) {
			logger.warning(
					"net.sf.fmj not found in PackageManager.getContentPrefixList() and PackageManager.getProtocolPrefixList(); is JMF ahead of FMJ in the classpath?");
			result = false;
		}
		...
		if (!checkManagerImplementation()) {
			logger.warning(
					"javax.media.Manager is JMF's implementation, not FMJ's; is JMF ahead of FMJ in the classpath?");
			result = false;
		}
