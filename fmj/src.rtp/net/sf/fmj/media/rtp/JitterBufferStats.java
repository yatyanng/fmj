...
class JitterBufferStats implements JitterBufferControl {
	/**
	 * The number of RTP packets that the associated queue has discarded because it
	 * was full.
	 */
	 ...
	 * adaptive behavior, it has taken into account the fact that the packets in
	 ...
	 * The number of RTP packets that the associated queue has discarded because
	 * they arrived too late to be added to the queue. If the queue exhibits
	 * adaptive behavior, it has NOT (in contrast to {@link #discardedLate} taken
	 * into account the fact that the packets in question have arrived too late.
	 */
	private int discardedVeryLate;

	/**
	 * The maximum size/capacity in number of RTP packets that the associated queue
	 * has ever reached.
	 */
	...
	/**
	 * Initializes a new <tt>JitterBufferStats</tt> instance which is to implement
	 * {@link PacketQueueControl} for a specific {@link RTPSourceStream}.
	 *
	 * @param stream the <tt>RTPSourceStream</tt> for which the new instance is to
	 *               implement {@link PacketQueueControl}
	 */
	 ...
	 * {@inheritDoc}
	 *
	 * The <tt>Control</tt> implementation of <tt>JitterBufferStats</tt> does not
	 * provide any user interface of its own and, consequently, always returns
	 * <tt>null</tt>.
	 */
	@Override
	 ...
	 * have a notion of packet duration and, consequently, it may be inaccurate. The
	 * method {@link #getNominalDelay()} delegates to <tt>JitterBufferBehaviour</tt>
	 * which is more likely to have a notion of packet duration and, consequently,
	 * it likely to be more accurate.
	 */
	@Override
	 ...
	 * {@inheritDoc}
	 *
	 * Returns the sum of the values of the other <tt>discardedXXX</tt> properties
	 * of this instance such as <tt>discardedFull</tt>, <tt>discaredLate</tt>,
	 * <tt>discaredReset</tt>, etc.
	 */
	@Override
	public int getDiscarded() {
		return getDiscardedFull() + getDiscardedLate() + getDiscardedReset() + getDiscardedShrink()
	...
	/**
	 * Increments the number of RTP packets that the associated queue has discarded
	 * because it was full.
	 */
	...
	/**
	 * Increments the number of RTP packets that the associated queue has discarded
	 * because they arrived too late to be added to the queue. If the queue exhibits
	 * adaptive behavior, it has taken into account the fact that the packets in
	 * question have arrived too late.
	 */
	...
	/**
	 * Updates the {@link RTPStats#PDUDROP} of the <tt>RTPStats</tt> associated with
	 * this <tt>JitterBufferStats</tt> because the two classes maintain
	 * discard-related statistics and <tt>RTPStats</tt> provides them to the public
	 * through the interface {@link ReceptionStats}.
	 */
	private void incrementRTPStatsPDUDrop() {
		/*
		 * There is no direct chain of references from JitterBufferStats to RTPStats.
		 * Walk through an indirect chain of references then but be careful. Start by
		 * making sure that the RTPSourceStream associated with this JitterBufferStats
		 * is still associated with the DataSource which initialized it.
		 */
			...
			if (datasourceStreams != null) {
				for (PushBufferStream datasourceStream : datasourceStreams) {
					if (datasourceStream == stream) {
						/*
						 * The DataSource which initialized the RTPSourceStream associated with this
						 * JitterBufferStats is still associated with it. Continue by finding an
						 * SSRCInfo which is associated with the DataSource/RTPSourceStream i.e. this
						 * JitterBufferStats.
						 */
						RTPSessionMgr mgr = datasource.getMgr();

						if (mgr != null) {
							SSRCInfo ssrcinfo = mgr.getSSRCInfo(datasource.getSSRC());

							if ((ssrcinfo != null) && (ssrcinfo.dsource == datasource)
									&& (ssrcinfo.dstream == stream)) {
								/*
								 * We've located the RTPStats associated with this JitterBufferStats.
								 */
	...
	/**
	 * Notifies this instance that the size/capacity of the associated queue of RTP
	 * packets may have changed and that it may be time for this instance to update
	 * the value to be returned by {@link #getMaxSizeReached()}.
	 */
	 ...
	 *
	 * @param buffer the <tt>Buffer</tt> which is to be taken into account for the
	 *               purposes of calculating and maintaining an average
	 *               approximation of the size in bytes of an RTP packet
	 */
	void updateSizePerPacket(Buffer buffer) {
		int bufferLength = buffer.getLength();

		sizePerPacket = (sizePerPacket == 0) ? bufferLength : ((sizePerPacket + bufferLength) / 2);
	}
}
