...
/**
 * Implements <tt>JitterBufferBehaviour</tt> for audio media data. It realizes a
 * jitter buffer which is either fixed or adaptive.
 *
...
class AudioJitterBufferBehaviour extends BasicJitterBufferBehaviour {
	...
	/**
	 * The default duration in milliseconds of an audio RTP packet. The default
	 * value expresses an expectation only. A value of <tt>20</tt> seems more
	 * reasonable than, for example, <tt>30</tt> because it is more commonly used in
	 * specifications.
	 */
	private static final int DEFAULT_MS_PER_PKT = 20;

	/**
	 * The number of initial RTP packets <tt>AudioJitterBufferBehaviour</tt> is to
	 * ignore before commencing the procedure of adapting.
	 */
	...
	/**
	 * Grow the queue if there are at least that many late packets (in the last
	 * AJB_GROW_INTERVAL packets)
	 */
	private final int AJB_GROW_THRESHOLD;

	/**
	 * The maximum size/capacity to which this instance will grow the associated RTP
	 * packet queue/jitter buffer.
	 */
	private final int AJB_MAX_SIZE;

	/**
	 * The minimum size/capacity to which this instance will shrink the associated
	 * RTP packet queue/jitter buffer.
	 */
	private final int AJB_MIN_SIZE;

	/**
	 * The number of packets by which this instance is to shrink the associated RTP
	 * packet queue/jitter buffer at a time.
	 */
	 ...
	 * latency so that a decision to shrink by {@link #AJB_SHRINK_DECREMENT} may be
	 * made.
	 */
	...
	/**
	 * Contains information about the recently received packets. A <tt>0</tt>
	 * indicates that the respective packet was accepted normally, a <tt>1</tt>
	 * indicates that it was dropped because it was received too late. The storage
	 * of the <tt>history</tt> is circular and {@link #historyPointer} always points
	 * to the last packet added.
	 */
	private byte[] history;

	/**
	 * The number of packets for which <tt>history</tt> is valid. Used in order to
	 * avoid filling <tt>history</tt> with zeroes when it needs to be reset.
	 */
	private int historyLength;

	/**
	 * Points to the place in <tt>history</tt> corresponding to the last packet
	 * added
	 */
	private int historyTail;

	/**
	 * The average approximation of the duration in milliseconds of an RTP packet.
	 * Used for audio only at the time of this writing. It sounds reasonable to
	 * introduce such a value for the duration since there is one for the size in
	 * bytes already (i.e. <tt>sizePerPkt</tt>).
	 */
	 ...
	 * {@link #AJB_SHRINK_INTERVAL} has elapsed i.e. the time expressed in number of
	 * packets which has elapsed from the current <tt>AJB_SHRINK_INTERVAL</tt>. At
	 * the end of the interval in question, a decision will be made whether the
	 * associated queue will be shrunk.
	 */
	private int shrinkCount = 0;

	/**
	 * The indicator which determines whether the <tt>Buffer.FLAG_SKIP_FEC</tt> flag
	 * should be set on the next packet read from/out of the RTP packet queue.
	 */
	...
	public AudioJitterBufferBehaviour(RTPSourceStream stream) {
		...
		AJB_ENABLED = com.sun.media.util.Registry.getBoolean("adaptive_jitter_buffer_ENABLE", true);
		AJB_GROW_INCREMENT = com.sun.media.util.Registry.getInt("adaptive_jitter_buffer_GROW_INCREMENT", 2);
		AJB_GROW_INTERVAL = com.sun.media.util.Registry.getInt("adaptive_jitter_buffer_GROW_INTERVAL", 30);
		AJB_GROW_THRESHOLD = com.sun.media.util.Registry.getInt("adaptive_jitter_buffer_GROW_THRESHOLD", 3);
		AJB_MAX_SIZE = com.sun.media.util.Registry.getInt("adaptive_jitter_buffer_MAX_SIZE", 16);
		AJB_MIN_SIZE = com.sun.media.util.Registry.getInt("adaptive_jitter_buffer_MIN_SIZE", 4);
		AJB_SHRINK_DECREMENT = com.sun.media.util.Registry.getInt("adaptive_jitter_buffer_SHRINK_DECREMENT", 1);
		AJB_SHRINK_INTERVAL = com.sun.media.util.Registry.getInt("adaptive_jitter_buffer_SHRINK_INTERVAL", 120);
		AJB_SHRINK_THRESHOLD = com.sun.media.util.Registry.getInt("adaptive_jitter_buffer_SHRINK_THRESHOLD", 1);
	...
	public int getAbsoluteMaximumDelay() {
		...
		return (absoluteMaximumDelay > 65535) ? 65535 : (int) absoluteMaximumDelay;
	}
	...
	protected int monitorQSize(Buffer buffer) {
		...
		if (AJB_ENABLED) {
			...
			if ((historyLength >= AJB_GROW_INTERVAL) && (growCount >= AJB_GROW_THRESHOLD) && (size < AJB_MAX_SIZE)) {
			...
			if ((shrinkCount >= AJB_SHRINK_INTERVAL) && (size > AJB_MIN_SIZE) && q.freeNotEmpty()) {
	...
	public boolean preAdd(Buffer buffer, RTPRawReceiver rtprawreceiver) {
		...
		if ((lastSeqSent != Buffer.SEQUENCE_UNKNOWN) && ((bufferSN = buffer.getSequenceNumber()) < lastSeqSent)) {
		...
		if (AJB_ENABLED && q.noMoreFree() && (stats.getNbAdd() > INITIAL_PACKETS)) {
			int size = q.getCapacity();
	...
	/**
	 * Resizes the queue to <tt>capacity</tt>. Assumes <tt>capacity</tt> is not more
	 * than the current capacity of the queue. Drops packets if necessary.
	 */
