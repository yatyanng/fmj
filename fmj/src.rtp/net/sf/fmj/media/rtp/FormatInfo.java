...
public class FormatInfo {
	...
	public void add(int i, Format format) {
		...
		if (existingFormat != null) {
			/*
			 * XXX If the specified format is matches-equivalent to the existingFormat, it
			 * shouldn't disturb the rest of the code/logic. However, the specified format
			 * may contain additional and/or more recent information (e.g. format parameters
			 * which are not differentiating) which may be vital to the application.
			 */
			if ((format == null) || !existingFormat.matches(format) || !format.matches(existingFormat))
				return;
		}
