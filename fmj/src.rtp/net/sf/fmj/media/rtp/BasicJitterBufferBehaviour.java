...
class BasicJitterBufferBehaviour implements JitterBufferBehaviour {
	 ...
	 * packets added to and read from {@link #stream}.
	 */
	 ...
	 * @throws IllegalArgumentException if the specified <tt>capacity</tt> is less
	 *                                  than the capacity of <tt>q</tt>
	 */
	 ...
	 * {@inheritDoc}
	 *
	 * <tt>BasicJitterBufferBehaviour</tt> always returns <tt>false</tt> to indicate
	 * that it implements a fixed jitter buffer/RTP packet queue.
	 */
	@Override
	 ...
	 * queue/<tt>JitterBuffer</tt> after a specific <tt>Buffer</tt> is received and
	 * before it is added to the <tt>JitterBuffer</tt>.
	 *
	 * @param buffer the <tt>Buffer</tt> which has been received and is to be added
	 *               (after the method returns)
	 * @return the approximate length in packets of the buffering performed by this
	 *         <tt>JitterBufferBehaviour</tt> and the associated
	 *         <tt>JitterBuffer</tt>. <tt>BasicJitterBufferBehaviour</tt> always
	 *         returns <tt>0</tt>.
	 */
	 ...
	 * {@inheritDoc}
	 *
	 * Maintains an average approximation of the size in bytes of an RTP packet in
	 * the <tt>JitterBufferStats</tt> of the associated <tt>RTPSourceStream</tt> and
	 * updates the <tt>recvBufSize</tt> of the specified <tt>rtprawreceiver</tt>.
	 */
	@Override
	...
	protected void setRecvBufSize(RTPRawReceiver rtprawreceiver, int aprxBufferLengthInPkts) {
		...
		if ((rtprawreceiver != null) && (aprxThresholdInBytes > this.recvBufSize)) {
			...
			this.recvBufSize = (recvBufSize < aprxThresholdInBytes) ? 0x7fffffff /* BufferControlImpl.NOT_SPECIFIED? */
	 ...
	 * {@inheritDoc}
	 *
	 * <tt>BasicJitterBufferBehaviour</tt> returns <tt>true</tt> if the associated
	 * RTP packet queue/jitter buffer is empty; otherwise, <tt>false</tt>
	 */
	@Override
