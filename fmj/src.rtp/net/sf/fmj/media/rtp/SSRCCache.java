...
public class SSRCCache {
	...
	public double calcReportInterval(boolean sender, boolean recvfromothers) {
		/*
		 * Minimum average time between RTCP packets from this site (in milliseconds).
		...
		/*
		 * Dedicate a fraction of the RTCP bandwidth to senders unless the number of
		 * senders is large enough that their share is more than that fraction.
		 */
		...
		if (recvfromothers)
			 ...
			 * distributed between 0.5*time and 1.5*time.
			 */
	...
	SSRCInfo get(int ssrc, InetAddress address, int port, int mode) {
		...
		synchronized (this) {
			if (ourssrc != null && ourssrc.ssrc == ssrc && ourssrc.address != null
