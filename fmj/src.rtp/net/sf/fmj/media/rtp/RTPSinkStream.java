...
public class RTPSinkStream implements BufferTransferHandler {
	...
	public void transferData(PushBufferStream stream) {
		try {
			...
			if (!current.getFormat().matches(info.myformat)) {
				...
				if (payload == -1)
					return;
				LocalPayloadChangeEvent evt = new LocalPayloadChangeEvent(transmitter.cache.sm, info,
	...
	private void waitForPT(long start, long startPT, long pt) {
		for (long delay = pt - startPT - (System.currentTimeMillis() - start); delay > LEEWAY; delay = pt - startPT
