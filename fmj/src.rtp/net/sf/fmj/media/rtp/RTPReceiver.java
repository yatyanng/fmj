...
public class RTPReceiver extends PacketFilter {
	...
	/**
	 * Gets an {@link RTPControlImpl} control over the {@code dsource} of a specific
	 * {@link SSRCInfo}.
	 *
	 * @param ssrcinfo the {@code SSRCInfo} whose {@code dsource} is to be queried
	 *                 for a {@code RTPControlImpl} control
	...
	public RTPReceiver(SSRCCache ssrccache, RTPDemultiplexer rtpdemultiplexer, int i, String s) throws IOException {
		this(ssrccache, rtpdemultiplexer, new RTPRawReceiver(i & -2, s, ssrccache.sm.defaultstats));
	}
	...
	public Packet handlePacket(RTPPacket rtppacket) {
		...
		if (rtppacket.base instanceof UDPPacket) {
			...
			if (cache.sm.bindtome && !cache.sm.isBroadcast(cache.sm.dataaddress)
		...
		if (!initBC) {
			((BufferControlImpl) cache.sm.buffercontrol).initBufferControl(ssrcinfo.currentformat);
			initBC = true;
		}
		...
		if (!ssrcinfo.newrecvstream) {
			...
			cache.eventhandler.postEvent(new NewReceiveStreamEvent(cache.sm, (ReceiveStream) ssrcinfo));
		}
		...
		if (ssrcinfo.quiet) {
			...
			cache.eventhandler.postEvent(new ActiveReceiveStreamEvent(cache.sm, ssrcinfo.sourceInfo,
					(ssrcinfo instanceof ReceiveStream) ? (ReceiveStream) ssrcinfo : null));
		}

		demuxpayload(ssrcinfo, /* wasOnProbation */ oldprobation > 0 && newprobation == 0, rtppacket);
	...
	private void demuxpayload(SSRCInfo ssrcinfo, boolean wasOnProbation, RTPPacket rtppacket) {
		if (ssrcinfo.dsource != null) {
			if (wasOnProbation) {
				...
				if (rtppacket1 != null)
					rtpdemultiplexer.demuxpayload(new SourceRTPPacket(rtppacket1, ssrcinfo));
			}
			rtpdemultiplexer.demuxpayload(new SourceRTPPacket(rtppacket, ssrcinfo));
		}
	}

	/**
	 * Updates the value of the field {@code lastPayloadType} of {@code ssrcinfo}
	 * based on the information carried by {@code rtppacket}.
	 *
	 * @param ssrcinfo  the {@code SSRCInfo} to update
	 * @param rtppacket the {@code RTPPacket} with which {@code ssrcinfo} is to be
	 *                  updated
	 * @return the value of the field {@code lastPayloadType} of {@code ssrcinfo}
	 *         before the update
	 */
	private int updateLastPayloadType(SSRCInfo ssrcinfo, RTPPacket rtppacket) {
		...
		if (oldpayload != -1 && oldpayload != newpayload) {
			...
			if (ssrcinfo.dsource != null) {
				...
				try {
					Log.warning("Stopping stream because of payload type mismatch:" + " expecting pt=" + oldpayload
							+ ", got pt=" + newpayload);
			...
			cache.eventhandler.postEvent(
					new RemotePayloadChangeEvent(cache.sm, (ReceiveStream) ssrcinfo, oldpayload, newpayload));
		} else {
			ssrcinfo.lastPayloadType = newpayload;
		}
	...
	/**
	 * Updates the value of the field {@code payloadType} of {@code ssrcinfo} based
	 * on the information carried by {@code rtppacket}.
	 *
	 * @param ssrcinfo  the {@code SSRCInfo} to update
	 * @param rtppacket the {@code RTPPacket} with which {@code ssrcinfo} is to be
	 *                  updated
	 * @return the value of the field {@code payloadType} of {@code ssrcinfo} before
	 *         the update
	 */
	...
	private void streamconnect(SSRCInfo ssrcinfo) {
		if (!ssrcinfo.streamconnect) {
			...
			javax.media.protocol.PushBufferStream[] pushbufferstreams = datasource.getStreams();
