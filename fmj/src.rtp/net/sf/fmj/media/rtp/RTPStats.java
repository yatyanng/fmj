...
public class RTPStats implements ReceptionStats {
	...
	public synchronized void update(int which, int amount) {
		...
		case PDULOST:
			if (amount > 0) {
				/*
				 * The incrementing of PDULOST is already handled by the method update(int). Do
				 * not duplicate the source code for the sake of maintenance.
				 */
				while (amount-- > 0)
					update(which);
			} else {
	...
	/**
	 * Gets the burst characteristics/metrics and the algorithm for measuring them.
	 *
