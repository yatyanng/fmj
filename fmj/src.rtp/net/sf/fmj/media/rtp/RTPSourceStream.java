...
public class RTPSourceStream extends BasicSourceStream implements PushBufferStream {
	...
	/**
	 * The jitter buffer associated with this instance in terms of behaviour, logic
	 * agnostic of the very storage-related details and the simplest of RTP packet
	 * queuing specifics which are abstracted by {@link #q}.
	 */
	...
	/**
	 * The <tt>DataSource</tt> which has initialized and has this instance as its
	 * <tt>sourceStream</tt>.
	 */
	...
	/**
	 * The RTP packet queue/jitter buffer which implements the storage of the RTP
	 * packets added to and read from this <tt>RTPSourceStream</tt>.
	 */
	final JitterBuffer q;

	/**
	 * The <tt>Condition</tt> which is used for synchronization purposes instead of
	 * synchronizing a block on {@link #q} because the latter is not flexible enough
	 * for the thread complexity of <tt>JitterBuffer</tt>.
	 */
	...
	/**
	 * The statistics related to the RTP packet queue/jitter buffer associated with
	 * this <tt>RTPSourceStream</tt>. Implements {@link PacketQueueControl} on
	 ...
	 * there are enough packets to be read without blocking but no read is actually
	 * performed.
	 */
	 ...
	 *
	 * In case the queue is full: if <tt>buffer</tt>'s sequence number comes before
	 * the sequence numbers of the <tt>Buffer</tt>s in the queue, nothing is done.
	 * Otherwise, a packet is dropped using PktQue.dropPkt()
	 *
	...
	public void add(Buffer buffer, boolean flag, RTPRawReceiver rtprawreceiver) {
		...
		try {
			...
			if (lastSeqRecv - bufferSN > 256L) {
				Log.info("Resetting queue, last seq added: " + lastSeqRecv + ", current seq: " + bufferSN);
				reset();
			...
			try {
				...
				{
					qBuffer.setFlags(qBuffer.getFlags() | Buffer.FLAG_BUF_OVERFLOWN | Buffer.FLAG_NO_DROP);
				} else {
					qBuffer.setFlags(qBuffer.getFlags() | Buffer.FLAG_NO_DROP);
				}
	 ...
	 * exhibited by/the logic of the jitter buffer/RTP packet queue associated with
	 * this instance.
	 *
	 * @return the <tt>JitterBufferBehaviour</tt> which represents the behaviour
	 *         exhibited by/the logic of the jitter buffer/RTP packet queue
	...
	public Object getControl(String controlType) {
		return JitterBufferControl.class.getName().equals(controlType) ? stats : super.getControl(controlType);
	}
	...
	/**
	 * Gets the (RTP) sequence number of the last <tt>Buffer</tt> read out of this
	 * <tt>SourceStream</tt>.
	 *
	...
	/**
	 * Pops an element off the queue and copies it to <tt>buffer</tt>. The data and
	 * header arrays of <tt>buffer</tt> are reused.
	 *
	 * @param buffer The <tt>Buffer</tt> object to copy an element of the queue to.
	 */
	@Override
	 ...
	 * Runs in {@link #thread}.
	 *
	 * @param runnable the <tt>TransferDataRunnable</tt> which is running in the
	 *                 current thread
	...
	/**
	 * Sets a <tt>JitterBufferBehaviour</tt> which represents the behaviour to be
	 * exhibited by/the logic of the jitter buffer/RTP packet queue associated with
	 * this instance.
	 *
	 * @param behaviour the <tt>JitterBufferBehaviour</tt> which represents the
	 *                  behaviour to be exhibited by the jitter buffer/RTP packet
	 *                  queue associated with this instance. If <tt>null</tt>, the
	 *                  implementation defaults to
	 *                  <tt>BasicJitterBufferBehaviour</tt>.
	 */
	...
	private void startThread() {
		synchronized (startSyncRoot) {
			...
			if ((this.thread == null) && !closed) {
				RTPMediaThread thread = new RTPMediaThread(new TransferDataRunnable(this),
	...
	/**
	 * Wait on {@link #startSyncRoot} while {@link #closing} equals <tt>true</tt>
	 * i.e. wait on <tt>startSyncRoot</tt> until <tt>false</tt> is set on
	 * <tt>closing</tt>.
	 */
	 ...
	 * <tt>RTPSourceStream</tt> while, optionally, keeping a <tt>WeakReference</tt>
	 * to the <tt>RTPSourceStream</tt>.
	 *
	 * @author Lyubomir Marinov
	 */
	private static class TransferDataRunnable implements Runnable {
		/**
		 * The indicator which determines whether <tt>TransferDataRunnable</tt> keeps a
		 * <tt>WeakReference</tt> to the associated <tt>RTPSourceStream</tt>.
		 */
		private static final boolean WEAK_REFERENCE = false;

		/**
		 * The <tt>RTPSourceStream</tt> which has initialized and owns this instance.
		 */
		 ...
		 * The (unique) reason for invoking
		 * {@link BufferTransferHandler#transferData(PushBufferStream)} on
		 * {@link #owner}. Introduced in order to prevent busy waits when there are
		 * enough packets to be read without blocking but no read is actually performed.
		 */
		private long transferDataReason;
		...
		/**
		 * Initializes a new <tt>TransferDataRunnable</tt> instance which is to transfer
		 * data out of a specific <tt>RTPSourceStream</tt>.
		 *
		 * @param owner the <tt>RTPSourceStream</tt> which is initializing the new
		 *              instance
		 */
		 ...
		 * instance.
		 *
		 * @return the <tt>RTPSourceStream</tt> which has initialized and owns this
		 *         instance
		 */
