...
public class DefaultRTCPTransmitterImpl implements RTCPTransmitter {
	...
	public DefaultRTCPTransmitterImpl(SSRCCache cache, int port, String address, UDPPacketSender sender)
			throws UnknownHostException, IOException {
	...
	protected List<RTCPPacket> makereports() {
		...
		sdesItems.add(new RTCPSDESItem(RTCPSDESItem.CNAME, ourinfo.sourceInfo.getCNAME()));
		...
		if (sdescounter % 3 == 0) {
			...
			// NAME
			if ((sd = ourinfo.name) != null && (s = sd.getDescription()) != null)
