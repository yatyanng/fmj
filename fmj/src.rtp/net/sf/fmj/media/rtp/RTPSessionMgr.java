...
public class RTPSessionMgr extends RTPManager implements SessionManager {
	 ...
	 * <tt>RTPSessionMgr</tt>.
	 *
	 * @return the <tt>RTCPTransmitterFactory</tt> of this <tt>RTPSessionMgr</tt>.
	 */
	 ...
	 * <tt>RTPSessionMgr</tt>.
	 *
	 * @param rtcpTransmitterFactory the <tt>RTCPTransmitterFactory</tt> to use with
	 *                               this <tt>RTPSessionMgr</tt>.
	 */
	...
	RTCPRawSender sender = null; // it seems like this is never used in a meaningful way. The variable name is
									// misleading as well.
	...
	public RTPSessionMgr(DataSource datasource) throws IOException {
		...
		try {
			...
			logger.log(Level.WARNING, "error retrieving address " + s + " by name" + throwable1.getMessage(),
					throwable1);
	...
	public void addPeer(SessionAddress sessionaddress) throws IOException, InvalidSessionAddressException {
		...
		try {
			...
			if (inetaddress != null) {
				rtprawreceiver = new RTPRawReceiver(j, inetaddress.getHostAddress(), defaultstats);
			}
		...
		try {
			...
			if (inetaddress != null) {
				rtprawreceiver = new RTPRawReceiver(j, inetaddress2.getHostAddress(), defaultstats);
			}
		...
		cache.ourssrc.reporter.transmit.getSender().peerlist.addElement(sessionaddress);
		if (cache != null) {
			for (Enumeration<SSRCInfo> elements = cache.cache.elements(); elements.hasMoreElements();) {
	...
	public void addTarget(SessionAddress sessionaddress) throws IOException {
		...
		try {
			rtcpRawReceiver = new RTCPRawReceiver(localAddress, sessionaddress, defaultstats, streamSynch,
					controlSocket);
			rtpRawReceiver = new RTPRawReceiver(localAddress, sessionaddress, defaultstats, dataSocket);
		} catch (SocketException socketexception) {
			throw new IOException(socketexception.getMessage());
		...
		rtcpForwarder.startPF("RTCP Forwarder for address" + sessionaddress.getControlHostAddress() + " port "
				+ sessionaddress.getControlPort());
		if (rtpForwarder != null) {
			rtpForwarder.startPF("RTP Forwarder for address " + sessionaddress.getDataHostAddress() + " port "
	...
	public void closeSession(String s) {
		...
		if (cache != null) {
			...
			for (Enumeration<SSRCInfo> elements = cache.cache.elements(); elements
					.hasMoreElements(); stopParticipating(s, ssrcinfo)) {
	...
	@Override
	public SendStream createSendStream(int ssrc, javax.media.protocol.DataSource datasource, int j)
			throws UnsupportedFormatException, IOException, SSRCInUseException {
		if (sendercount == 0) {
			/*
			 * It is pointless to try to detect a collision with the specified SSRC because
		...
		if (sendercount == 0) {
			...
			obj.ours = true;
			if (!nosockets) {
				obj.reporter = startParticipating(controlport, controladdress.getHostAddress(), obj);
			} else {
	...
	public SendStream createSendStream(javax.media.protocol.DataSource datasource, int i)
		...
		if ((sendercount == 0) && (cache.ourssrc != null)) {
			/*
			 * It is pointless to generate a new SSRC because it will not be used anyway.
			 */
	...
	public void dispose() {
		if (rtpConnector != null) {
			rtpConnector.close();
			/*
			 * lyub0m1r: Make sure rtpConnector will not be accessed by assigning null to it
			 * because it may resurrect any of the control and/or data input and/or output
			 * streams (and, thus, their underlying sockets).
			 */
			rtpConnector = null;
		}
		if (defaultsource != null)
		...
		if (cache != null) {
			SSRCInfo ssrcinfo;
			for (Enumeration<SSRCInfo> elements = cache.cache.elements(); elements
					.hasMoreElements(); stopParticipating("dispose", ssrcinfo)) {
				ssrcinfo = elements.nextElement();
				if (ssrcinfo.dstream != null)
					ssrcinfo.dstream.close();
				if (ssrcinfo instanceof SendSSRCInfo)
		...
		/*
		 * damencho: If no targets are added, dataSocket and controlSocket are not
		 * closed. So close them explicitly.
		 */
		if (dataSocket != null)
			dataSocket.close();
	 ...
	 *
	 * @param cause a <tt>GenerateSSRCCause</tt> value which indicates the cause of
	...
	public Vector getActiveParticipants() {
		...
		for (Enumeration<RTPSourceInfo> enumeration = hashtable.elements(); enumeration.hasMoreElements();) {
	...
	public SessionAddress getLocalSessionAddress() {
		if (newRtpInterface) {
			...
			SessionAddress sessionaddress = new SessionAddress(localDataAddress, localDataPort, localControlAddress,
					localControlPort);
	 ...
	 * <tt>RTPSessionMgr</tt> or <tt>Long.MAX_VALUE</tt> if no such SSRC has been
	 * generated yet or is unknown at this time (for whatever reason).
	 * <p>
	...
	public long getLocalSSRC() {
		return ((cache == null) || (cache.ourssrc == null)) ? Long.MAX_VALUE : cache.ourssrc.ssrc;
	}
	...
	/**
	 * Exposes the <tt>SSRCCache</tt> of this <tt>RTPSessionMgr</tt>. At the time of
	 * this writing this is used to allow alternate implementations of
	...
	public void initialize(RTPConnector rtpconnector) {
		...
		SourceDescription asourcedescription[] = { new SourceDescription(3, SOURCE_DESC_EMAIL, 1, false),
				new SourceDescription(1, cname, 1, false), new SourceDescription(6, SOURCE_DESC_TOOL, 1, false) };
		...
		double rtcpSenderBandwidthFraction = rtpConnector.getRTCPSenderBandwidthFraction();
	...
	public void initialize(SessionAddress sessionaddress) throws InvalidSessionAddressException {
		...
		SourceDescription asourcedescription[] = { new SourceDescription(3, SOURCE_DESC_EMAIL, 1, false),
				new SourceDescription(1, s, 1, false), new SourceDescription(6, SOURCE_DESC_TOOL, 1, false) };
	...
	@Override
	public void initialize(SessionAddress asessionaddress[], SourceDescription asourcedescription[], double d,
			double d1, EncryptionInfo encryptioninfo) throws InvalidSessionAddressException {
		if (initialized) {
			return;
		}
		newRtpInterface = true;
		...
		if (localAddress.getDataAddress().isMulticastAddress()) {
			...
			boolean flag1 = true;
			try {
				logger.fine("Looking for local data address: " + localAddress.getDataAddress() + " and control address"
				...
				for (Enumeration intfs = NetworkInterface.getNetworkInterfaces(); intfs.hasMoreElements();) {
					if (flag && flag1)
						break;

					NetworkInterface intf = (NetworkInterface) intfs.nextElement();
					for (Enumeration addrs = intf.getInetAddresses(); addrs.hasMoreElements();) {
						try {
							InetAddress addr = (InetAddress) addrs.nextElement();
							logger.fine("Testing iface address " + localAddress.getDataAddress());
			...
			if (!localAddress.getDataAddress().isMulticastAddress()) {
				try {
					...
					throw new InvalidSessionAddressException(
							"Can't open local data port: " + localAddress.getDataPort());
				}
			}
			if (!localAddress.getControlAddress().isMulticastAddress()) {
				try {
					...
					throw new InvalidSessionAddressException(
							"Can't open local control port: " + localAddress.getControlPort());
				}
			}
		}
		cache = new SSRCCache(this);
	...
	@Override
	public int initSession(SessionAddress sessionaddress, long l, SourceDescription asourcedescription[], double d,
			double d1) throws InvalidSessionAddressException {
		if (initialized) {
			return -1;
		}
		if (d == 0.0D) {
	...
	void removeSendStream(SendStream sendstream) {
		...
		if (sendstreamlist.size() == 0 && cache.ourssrc != null) {
			...
			/*
			 * If we've just removed (i.e. said RTCP BYE) the SendStream with the SSRC of
			 * this RTPSessionMgr, then it's time to change the SSRC of this RTPSessionMgr.
			 */
			if ((cache.ourssrc.ssrc == sendstreamAsSendSSRCInfo.ssrc) && (sendstreamAsSendSSRCInfo.reporter == null)) {
				/*
				 * The code bellow tries to mimic #initialize(RTPConnector) in order to
				 * correctly generate a new SSRC for this RTPSessionMgr.
				 */
				...
				do {
					...
					/*
					 * The synchronization source (SSRC) identifier factory is allowed to cancel the
					 * operation.
					 */
				...
				if (lNewSSRC == Long.MAX_VALUE) {
					/*
					 * The synchronization source (SSRC) identifier factory has canceled the
					 * operation.
					 */
					...
					passivessrcinfo.setSourceDescription(
							isCNAME(asourcedescription) ? asourcedescription : setCNAME(asourcedescription));
			...
			/*
			 * damencho: Create reporter in order to process further RTCP requests when
	...
	private SourceDescription[] setCNAME(SourceDescription asourcedescription[]) {
		...
		for (int l = 0; l < asourcedescription.length; l++) {
			asourcedescription1[k] = new SourceDescription(asourcedescription[l].getType(),
	...
	private SourceDescription[] setSDES() {
		SourceDescription asourcedescription[] = new SourceDescription[3];
		{
			...
			asourcedescription[2] = new SourceDescription(6, SOURCE_DESC_TOOL, 1, false);
			return asourcedescription;
		}
	}
	...
	private synchronized RTCPReporter startParticipating(DatagramSocket datagramsocket) throws IOException {
		...
		// udppacketsender);
		RTCPRawSender rtcprawsender = new RTCPRawSender(remoteAddress.getControlPort(),
				remoteAddress.getControlAddress().getHostAddress(), udppacketsender);
	...
	private synchronized RTCPReporter startParticipating(int i, String s, SSRCInfo ssrcinfo) throws IOException {
		...
		RTCPTransmitter rtcptransmitter = getOrCreateRTCPTransmitterFactory().newRTCPTransmitter(cache, rtcprawsender);
	...
	private synchronized RTCPReporter startParticipating(SessionAddress sessionaddress, SessionAddress sessionaddress1,
			SSRCInfo ssrcinfo, DatagramSocket datagramsocket) throws IOException {
		...
		RTCPRawSender rtcprawsender = new RTCPRawSender(controlport, controladdress.getHostAddress(), udppacketsender);
		RTCPTransmitter rtcptransmitter = getOrCreateRTCPTransmitterFactory().newRTCPTransmitter(cache, rtcprawsender);
		rtcptransmitter.setSSRCInfo(ssrcinfo);
		return new RTCPReporter(cache, rtcptransmitter);
	}
	...
	void startRTCPReports(InetAddress inetaddress) {
		if (!nonparticipating && !startedparticipating) {
			try {
				if (cache.ourssrc != null) {
					cache.ourssrc.reporter = startParticipating(controlport, inetaddress.getHostAddress(),
							cache.ourssrc);
				}
			} catch (IOException ioexception) {
	...
	private int startSession(RTPPushDataSource rtppushdatasource, RTPPushDataSource rtppushdatasource1,
		...
		cache.sessionbandwidth = 0x5dc00;
		RTPRawReceiver rtprawreceiver = new RTPRawReceiver(rtppushdatasource, defaultstats);
	...
	public int startSession(SessionAddress sessionaddress, int i, EncryptionInfo encryptioninfo)
		...
		try {
			rtcprawreceiver = new RTCPRawReceiver(controlport, controladdress.getHostAddress(), defaultstats,
					streamSynch);
			if (dataaddress != null) {
				rtprawreceiver = new RTPRawReceiver(dataport, dataaddress.getHostAddress(), defaultstats);
			}
		} catch (SocketException socketexception) {
			throw new IOException(socketexception.getMessage());
		} finally {
		...
		try {
			...
			if (dataaddress != null) {
				rtprawreceiver = new RTPRawReceiver(dataport, inetaddress.getHostAddress(), defaultstats);
			}
		} catch (SocketException socketexception1) {
			throw new IOException(socketexception1.getMessage());
		} finally {
	...
	@Override
	public int startSession(SessionAddress sessionaddress, SessionAddress sessionaddress1,
			SessionAddress sessionaddress2, EncryptionInfo encryptioninfo)
			throws IOException, InvalidSessionAddressException {
		RTCPRawReceiver rtcprawreceiver;
		RTPRawReceiver rtprawreceiver;
		Object aobj[];
		...
		dataaddress = sessionaddress.getDataAddress();
		controladdress = sessionaddress.getControlAddress();
		if (dataaddress.isMulticastAddress() || controladdress.isMulticastAddress() || isBroadcast(dataaddress)
				|| isBroadcast(controladdress)) {
		...
		cleaner = new SSRCCacheCleaner(cache, streamSynch);
		if (!nonparticipating && !unicast && cache.ourssrc != null) {
			cache.ourssrc.reporter = startParticipating(sessionaddress, sessionaddress1, cache.ourssrc,
					rtcprawreceiver.socket);
		}
		started = true;
		return 0;
	}
	...
	public void UpdateEncodings(javax.media.protocol.DataSource datasource) {
		...
		if (rtpcontrolimpl != null && rtpcontrolimpl.codeclist != null) {
			Integer integer;
			for (Enumeration<Integer> enumeration = rtpcontrolimpl.codeclist.keys(); enumeration
					.hasMoreElements(); formatinfo.add(integer, rtpcontrolimpl.codeclist.get(integer))) {
