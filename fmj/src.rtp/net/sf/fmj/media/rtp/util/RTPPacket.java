...
public class RTPPacket extends Packet {
	...
	public void assemble(int len, boolean encrypted) {
		...
		/*
		 * XXX We cannot reuse the data of super/this because it may be the same as the
		 * data of base at this time.
		 */
		byte[] d = new byte[len];
		...
		try {
			...
			dos.writeInt(ssrc);
			if (headerExtension != null) {
				int extensionLengthInWords = (headerExtension.value.length + 3) / 4;
				if (extensionLengthInWords > 0) {
					...
					dos.writeByte((headerExtension.id << 4) | (headerExtension.value.length - 1));
					dos.write(headerExtension.value, 0, headerExtension.value.length);
	...
	public int calcLength() {
		...
		if (headerExtension != null) {
			int headerExtensionsLengthInWords = (headerExtension.value.length + 3) / 4;

			// An extra word for "defined by profile" plus "length".
			headerExtensionsLengthInBytes = (headerExtensionsLengthInWords + 1) * 4;
		}
		return 12 /* RTP fixed header */ + headerExtensionsLengthInBytes + payloadlength;
	}
	...
	public String toString() {
		String s = "RTP Packet:\n\tPayload Type: " + payloadType + "    Marker: " + marker + "\n\tSequence Number: "
				...
				+ payloadlength + "    Payload Offset: " + payloadoffset + "\n";
		if (csrc.length > 0) {
