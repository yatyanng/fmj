...
public class RTCPReceiver implements PacketConsumer {
	...
	public void sendTo(RTCPPacket rtcppacket) {
		...
		if (flag) {
			InetAddress remoteAddress = ((UDPPacket) rtcppacket.base).remoteAddress;
			if (!rtcpstarted) {
		...
		case RTCPPacket.SR:
			...
			if (rtcppacket.base instanceof UDPPacket)
				ssrcinfo = cache.get(rtcpsrpacket.ssrc, ((UDPPacket) rtcppacket.base).remoteAddress,
			...
			if (ssrcinfo.quiet) {
				...
				ActiveReceiveStreamEvent activereceivestreamevent = new ActiveReceiveStreamEvent(cache.sm,
						ssrcinfo.sourceInfo, (ssrcinfo instanceof ReceiveStream) ? (ReceiveStream) ssrcinfo : null);
			...
			if (!ssrcinfo.recvstrmap && ssrcinfo.sourceInfo != null) {
				...
				StreamMappedEvent streammappedevent = new StreamMappedEvent(cache.sm, (ReceiveStream) ssrcinfo,
						ssrcinfo.sourceInfo);
		...
		case RTCPPacket.RR:
			...
			type = 2;
			if (rtcppacket.base instanceof UDPPacket)
				ssrcinfo = cache.get(rtcprrpacket.ssrc, ((UDPPacket) rtcppacket.base).remoteAddress,
						((UDPPacket) rtcppacket.base).remotePort, type);
			else
			...
			if (ssrcinfo.quiet) {
				...
				if (ssrcinfo instanceof ReceiveStream)
					activereceivestreamevent1 = new ActiveReceiveStreamEvent(cache.sm, ssrcinfo.sourceInfo,
		...
		case RTCPPacket.SDES:
			...
			for (int k1 = 0; k1 < rtcpsdespacket.sdes.length; k1++) {
				...
				if (type == 1 || type == 2) {
					if (rtcppacket.base instanceof UDPPacket)
						ssrcinfo = cache.get(rtcpsdes.ssrc, ((UDPPacket) rtcppacket.base).remoteAddress,
								((UDPPacket) rtcppacket.base).remotePort, type);
					else
			...
			if (ssrcinfo != null && !ssrcinfo.recvstrmap && ssrcinfo.sourceInfo != null
					&& (ssrcinfo instanceof RecvSSRCInfo)) {
				ssrcinfo.recvstrmap = true;
				StreamMappedEvent streammappedevent1 = new StreamMappedEvent(cache.sm, (ReceiveStream) ssrcinfo,
						ssrcinfo.sourceInfo);
		...
		case RTCPPacket.BYE:
			...
			SSRCInfo ssrcinfo1;
			if (rtcppacket.base instanceof UDPPacket)
				ssrcinfo1 = cache.get(rtcpbyepacket.ssrc[0], ((UDPPacket) rtcppacket.base).remoteAddress,
			...
			for (int l1 = 0; l1 < rtcpbyepacket.ssrc.length; l1++) {
				if (rtcppacket.base instanceof UDPPacket)
					ssrcinfo1 = cache.get(rtcpbyepacket.ssrc[l1], ((UDPPacket) rtcppacket.base).remoteAddress,
							((UDPPacket) rtcppacket.base).remotePort);
				else
			...
			if (ssrcinfo1.quiet) {
				...
				ActiveReceiveStreamEvent activereceivestreamevent2 = new ActiveReceiveStreamEvent(cache.sm,
						ssrcinfo1.sourceInfo, (ssrcinfo1 instanceof ReceiveStream) ? (ReceiveStream) ssrcinfo1 : null);
			...
			if (ssrcinfo1 instanceof RecvSSRCInfo)
				byeevent = new ByeEvent(cache.sm, ssrcinfo1.sourceInfo, (ReceiveStream) ssrcinfo1,
			...
			/*
			 * damencho: Remove the ssrc from the cache table because we have received a bye
			 * for the stream. If we continue to receive the stream, it will be reported as
			 * new one. It is related to the problem with reINVITEs and the changing of the
			 * codecs.
			 */
		...
		case RTCPPacket.APP:
			...
			SSRCInfo ssrcinfo2;
			if (rtcppacket.base instanceof UDPPacket)
				ssrcinfo2 = cache.get(rtcpapppacket.ssrc, ((UDPPacket) rtcppacket.base).remoteAddress,
						((UDPPacket) rtcppacket.base).remotePort);
			else
			...
			if (ssrcinfo2.quiet) {
				...
				ActiveReceiveStreamEvent activereceivestreamevent3 = new ActiveReceiveStreamEvent(cache.sm,
						ssrcinfo2.sourceInfo, (ssrcinfo2 instanceof ReceiveStream) ? (ReceiveStream) ssrcinfo2 : null);
				cache.eventhandler.postEvent(activereceivestreamevent3);
			}
			ApplicationEvent applicationevent = new ApplicationEvent(cache.sm, ssrcinfo2.sourceInfo,
					(ssrcinfo2 instanceof ReceiveStream) ? (ReceiveStream) ssrcinfo2 : null, rtcpapppacket.subtype,
