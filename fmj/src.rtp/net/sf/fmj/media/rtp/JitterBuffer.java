...
class JitterBuffer {
	...
	/**
	 * The <tt>Buffer</tt>s of this <tt>JitterBuffer</tt> which may contain valid
	 * media data to be read out of this instance (referred to as &quot;fill&quot;)
	 * or may represent preallocated <tt>Buffer</tt> instances for the purposes of
	 * reducing the effects of allocation and garbage collection (referred to as
	 * &quot;free&quot;). The storage is of a circular nature with the first
	 * &quot;fill&quot; at index {@link #offset} and the number of &quot;fill&quot;
	...
	/**
	 * The index in {@link #elements} of the <tt>Buffer</tt>, if any, which has been
	 * retrieved from this queue and has not been returned yet.
	 */
	private int locked;

	/**
	 * The index in {@link #elements} of the first &quot;fill&quot; <tt>Buffer</tt>.
	 */
	private int offset;

	/**
	 * Initializes a new <tt>JitterBuffer</tt> instance with a specific capacity of
	 * <tt>Buffer</tt>s.
	 *
	...
	/**
	 * Inserts <tt>buffer</tt> in its proper place in this queue according to its
	 * sequence number. The elements are always kept in ascending order by sequence
	 * number.
	 *
	 ...
	 *
	 * @param buffer the <tt>Buffer</tt> to be added to the end of this queue
	 */
	 ...
	 *
	 * @throws IllegalStateException if no <tt>Buffer</tt> has been retrieved from
	 *                               this <tt>JitterBuffer</tt> and has not been
	 *                               returned yet
	 */
	private void assertLocked(Buffer buffer) throws IllegalStateException {
		if (locked == -1) {
			throw new IllegalStateException(
					"No Buffer has been retrieved from this JitterBuffer" + " and has not been returned yet.");
		}
	 ...
	 * <tt>JitterBuffer</tt> and has not been returned yet.
	 *
	 * @throws IllegalStateException if a <tt>Buffer</tt> has been retrieved from
	 *                               this <tt>JitterBuffer</tt> and has not been
	 *                               returned yet
	 */
	private void assertNotLocked() throws IllegalStateException {
		if (locked != -1) {
			throw new IllegalStateException(
					"A Buffer has been retrieved from this JitterBuffer" + " and has not been returned yet.");
		}
	}
	...
	/**
	 * Removes the first element (the one with the least sequence number) from
	 * <tt>fill</tt> and releases it to be reused (adds it to <tt>free</tt>)
	 */
	 ...
	 * queue.
	 *
	 * @return <tt>true</tt> if there are &quot;fill&quot; <tt>Buffer</tt>s in this
	 *         queue; otherwise, <tt>false</tt>
	 */
	 ...
	 * queue.
	 *
	 * @return <tt>true</tt> if there are &quot;free&quot; <tt>Buffer</tt>s in this
	 *         queue; otherwise, <tt>false</tt>
	 */
	 ...
	 * valid media data.
	 *
	 * @return the element/<tt>Buffer</tt> at the head of this queue which contains
	 *         valid media data
	 */
	...
	/**
	 * Gets the sequence number of the element/<tt>Buffer</tt> at the head of this
	 * queue or <tt>Buffer.SEQUENCE_UNKNOWN</tt> if this queue is empty.
	 *
	...
	public long getFirstSeq() {
		return (length == 0) ? Buffer.SEQUENCE_UNKNOWN : elements[offset].getSequenceNumber();
	}
	...
	/**
	 * Gets the sequence number of the element/<tt>Buffer</tt> at the tail of this
	 * queue or <tt>Buffer.SEQUENCE_UNKNOWN</tt> if this queue is empty.
	 *
	...
	public long getLastSeq() {
		return (length == 0) ? Buffer.SEQUENCE_UNKNOWN : elements[(offset + length - 1) % capacity].getSequenceNumber();
	}

	/**
	 * Inserts <tt>buffer</tt> in the correct place in the queue, so that the order
	 * is preserved. The order is by ascending sequence numbers.
	 *
	 * Note: This could potentially be slow, since all the elements 'bigger' than
	 * <tt>buffer</tt> are moved.
	 *
	 ...
	 *
	 * @param buffer the <tt>Buffer</tt> to add to the beginning of this queue
	 */
	 ...
	 * <tt>Buffer</tt>s that it may contain.
	 *
	 * @param capacity the capacity of this instance in terms of the maximum number
	 *                 of <tt>Buffer</tt>s that it may contain
	 */
