...
public abstract class SSRCInfo implements Report {
	...
	public Vector<RTCPReportBlock> getFeedbackReports() {
		Vector<RTCPReportBlock> reportlist = new Vector<RTCPReportBlock>(reports.size());
		if (reports.size() == 0)
			return reportlist;
	...
	public Participant getParticipant() {
		if ((sourceInfo instanceof LocalParticipant) && cache.sm.IsNonParticipating())
			return null;
		else
			return sourceInfo;
	}
	...
	void setSourceDescription(SourceDescription userdesclist[]) {
		...
		if (sourceInfo != null)
			sourceinfocname = sourceInfo.getCNAME();
		if (sourceInfo != null && cname != null && !cname.equals(sourceinfocname)) {
			sourceInfo.removeSSRC(this);
			sourceInfo = null;
		}
		if (sourceInfo == null) {
		...
		for (SourceDescription currdesc : userdesclist) {
			if (currdesc != null)
				...
				case 2: // '\002'
					if (name == null)
						name = new SourceDescription(2, currdesc.getDescription(), 0, false);
					else
				...
				case 3: // '\003'
					if (email == null)
						email = new SourceDescription(3, currdesc.getDescription(), 0, false);
					else
				...
				case 4: // '\004'
					if (phone == null)
						phone = new SourceDescription(4, currdesc.getDescription(), 0, false);
					else
				...
				case 5: // '\005'
					if (loc == null)
						loc = new SourceDescription(5, currdesc.getDescription(), 0, false);
					else
				...
				case 6: // '\006'
					if (tool == null)
						tool = new SourceDescription(6, currdesc.getDescription(), 0, false);
					else
				...
				case 7: // '\007'
					if (note == null)
						note = new SourceDescription(7, currdesc.getDescription(), 0, false);
					else
				...
				case 8: // '\b'
					if (priv == null)
						priv = new SourceDescription(8, currdesc.getDescription(), 0, false);
					else
	 ...
	 * reception/transmission as defined by RFC 3550 i.e. the extended last sequence
	 * number received less the initial sequence number received.
	 *
	 ...
	 * receiver has received RTP data packets since the last report as per RFC3550.
	 * It provides statistics about the data received from this particular source.
	 *
	 * @param time
	 * @return
	 */
