...
/**
 * Implements <tt>JitterBufferBehaviour</tt> for video media data. It realizes a
 * jitter buffer which is adaptive.
 *
...
class VideoJitterBufferBehaviour extends BasicJitterBufferBehaviour {
	...
	public VideoJitterBufferBehaviour(RTPSourceStream stream) {
		...
		MIN_SIZE = com.sun.media.util.Registry.getInt("video_jitter_buffer_MIN_SIZE", 4);
	...
	/**
	 * Removes an element from the queue and releases it to be reused. The element
	 * is chosen in a way specific to MPEG.
	 */
	 ...
	 * {@inheritDoc}
	 *
	 * <tt>VideoJitterBufferBehaviour</tt> always returns <tt>true</tt> to indicate
	 * that it implements an adaptive jitter buffer.
	 */
	@Override
	...
	protected int monitorQSize(Buffer buffer) {
		...
		if (size > MIN_BUF_CHECK && i1 < size / 4) {
			if (tooMuchBufferingCount++ > pktsPerFrame * fps * BUF_CHECK_INTERVAL) {
				cutByHalf();
			...
			size = q.getCapacity();
			Log.comment("RTP video buffer size: " + size + " pkts, " + aprxBufferLengthInPkts * stats.getSizePerPacket()
					+ " bytes.\n");
			tooMuchBufferingCount = 0;
		} else
			tooMuchBufferingCount = 0;
