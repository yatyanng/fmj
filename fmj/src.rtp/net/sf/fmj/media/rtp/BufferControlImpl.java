...
public class BufferControlImpl implements BufferControl {
	private class BufferControlPanel extends Panel {
		...
		public BufferControlPanel() {
			...
			add(new Label("Actual buffer & threshold sizes (in millisec) not displayed until media type is determined"),
					"Center");
			add(threshold, "South");
			setVisible(true);
		}
	...
	private Vector<RTPSourceStream> sourcestreamlist = new Vector<RTPSourceStream>(1);
	...
	protected void initBufferControl(Format f) {
		if (f instanceof AudioFormat) {
			defBuffer = (defBuffer != NOT_SPECIFIED) ? currBuffer : AUDIO_DEFAULT_BUFFER;
			defThreshold = (defThreshold != NOT_SPECIFIED) ? currThreshold : AUDIO_DEFAULT_THRESHOLD;
			maxBuffer = (maxBuffer != NOT_SPECIFIED) ? maxBuffer : AUDIO_MAX_BUFFER;
			maxThreshold = (maxThreshold != NOT_SPECIFIED) ? maxThreshold : AUDIO_MAX_THRESHOLD;
			currBuffer = (currBuffer != NOT_SPECIFIED) ? currBuffer : defBuffer;
			currThreshold = (currThreshold != NOT_SPECIFIED) ? currThreshold : defThreshold;
		} else if (f instanceof VideoFormat) {
			defBuffer = (defBuffer != NOT_SPECIFIED) ? currBuffer : VIDEO_DEFAULT_BUFFER;
			defThreshold = (defThreshold != NOT_SPECIFIED) ? currThreshold : VIDEO_DEFAULT_THRESHOLD;
			maxBuffer = (maxBuffer != NOT_SPECIFIED) ? maxBuffer : VIDEO_MAX_BUFFER;
			maxThreshold = (maxThreshold != NOT_SPECIFIED) ? maxThreshold : VIDEO_MAX_THRESHOLD;
			currBuffer = (currBuffer != NOT_SPECIFIED) ? currBuffer : defBuffer;
			currThreshold = (currThreshold != NOT_SPECIFIED) ? currThreshold : defThreshold;
		}
