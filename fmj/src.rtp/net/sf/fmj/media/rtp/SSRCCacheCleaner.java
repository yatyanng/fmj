...
public class SSRCCacheCleaner implements Runnable {
	...
	/**
	 * The synchronization source identifiers (SSRCs) of {@link #cache} retrieved
	 * during the last execution of {@link #cleannow(long)}. Cached for the purposes
	 * of reducing the number of allocations in particular and the effects of the
	 * garbage collector in general.
	 */
	private int[] ssrcs;
	...
	private long cleannow(long time) {
		...
		for (int ssrc : (ssrcs = infos.keysToArray(ssrcs))) {
			...
			if (info.byeReceived) {
				...
				ByeEvent ev = new ByeEvent(cache.sm, sourceInfo, receiveStream, info.byereason,
						sourceInfo != null && sourceInfo.getStreamCount() == 0);
				...
				if (!info.inactivesent) {
					...
					if (info instanceof ReceiveStream) {
						receiveStream = (ReceiveStream) info;
					} else if (info.lastHeardFrom + reportInterval * TIMEOUT_MULTIPLIER <= time) {
						receiveStream = null;
					} else {
						continue;
					}

					InactiveReceiveStreamEvent ev = new InactiveReceiveStreamEvent(cache.sm, sourceInfo, receiveStream,
							sourceInfo != null && sourceInfo.getStreamCount() == 1);
				...
				// reasonable value in practical situations.
					...
					RTPSourceInfo sourceInfo = info.sourceInfo;
					TimeoutEvent ev = new TimeoutEvent(cache.sm, sourceInfo,
							(info instanceof ReceiveStream) ? (ReceiveStream) info : null,
							(sourceInfo != null) && (sourceInfo.getStreamCount() == 0));
		...
		do {
			...
			synchronized (this) {
				if (killed) {
					...
					timeout = (lastCleaned == -1L) ? 0L : Math.min(lastCleaned + RUN_INTERVAL - now, timeout);
					if (timeout <= 0) {
