...
public class RTCPPacketParser {
	...
	public RTCPPacket parse(Packet packet) throws BadFormatException {
		...
		try {
			...
			for (int offset = 0; offset < base.length; offset += length) {
				...
				if ((firstbyte & 0xc0) != 128) {
					throw new BadVersionException(
							"version must be 2. (base.length " + base.length + ", base.offset " + base.offset
									+ ", firstbyte 0x" + Integer.toHexString(firstbyte) + ", offset " + offset + ")");
				}
				...
				case RTCPPacket.SR:
					 ...
					 * +=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+ report |
					 * SSRC_2 (SSRC of second source) | block
					 * +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+ 2 : ... :
					 * +=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+ |
			...
			throw new BadFormatException("Failed to parse an RTCP packet: " + e.getMessage());
		} catch (IOException e) {
			throw new IllegalArgumentException("Failed to parse an RTCP packet: ", e);
		}
	...
	protected RTCPPacket parse(RTCPCompoundPacket base, int firstbyte, int type, int length, DataInputStream in)
