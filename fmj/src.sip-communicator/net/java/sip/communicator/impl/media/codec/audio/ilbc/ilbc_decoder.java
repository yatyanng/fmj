	...
	*---------------------------------------------------------------*/
	...
	public void Decode(float decresidual[], /* (o) decoded residual frame */
			...
			int extra_gain_index[], /*
									 ...
									 */
			int state_first) /*
								 * (i) 1 if non adaptive part of start state comes first 0 if that part comes
								 * last
								 */
	{
		...
		ilbc_common.StateConstructW(idxForMax, idxVec, syntdenum, (start - 1) * (ilbc_constants.LPC_FILTERORDER + 1),
	...
	*---------------------------------------------------------------*/
			...
			float Out[]) /* (o) downsampled output */
	{
		...
		for (i = ilbc_constants.DELAY_DS; i < lengthIn; i += ilbc_constants.FACTOR_DS) {
			...
			// stop = (i < ilbc_constants.FILTERORDER_DS) ? i + 1 :
		...
		for (i = (lengthIn + ilbc_constants.FACTOR_DS); i < (lengthIn
				+ ilbc_constants.DELAY_DS); i += ilbc_constants.FACTOR_DS) {
...
// 	       6*sizeof(float));
		...
		DownSample(enh_buf, (ilbc_constants.ENH_NBLOCKS_EXTRA + ioffset) * ilbc_constants.ENH_BLOCKL - 120,
				ilbc_constants.lpFilt_coefsTbl, inLen - ioffset * ilbc_constants.ENH_BLOCKL, lpState, downsampled);

		/* Estimate the pitch in the down sampled domain. */
		for (iblock = 0; iblock < ilbc_constants.ENH_NBLOCKS - ioffset; iblock++) {

			lag = 10;
			maxcc = xCorrCoef(downsampled, 60 + iblock * ilbc_constants.ENH_BLOCKL_HALF, downsampled,
					60 + iblock * ilbc_constants.ENH_BLOCKL_HALF - lag, ilbc_constants.ENH_BLOCKL_HALF);
			for (ilag = 11; ilag < 60; ilag++) {
				cc = xCorrCoef(downsampled, 60 + iblock * ilbc_constants.ENH_BLOCKL_HALF, downsampled,
						60 + iblock * ilbc_constants.ENH_BLOCKL_HALF - ilag, ilbc_constants.ENH_BLOCKL_HALF);
		...
		if (this.prev_enh_pl == 1) {
			...
			/*
			 * compute new concealed residual for the old lookahead, mix the forward PLC
			 * with a backward PLC from the new frame
			 */
			...
			if (ftmp1 > 2.0f * ftmp2 && ftmp1 > 0.0) {
				...
				for (i = plc_blockl - 10; i < plc_blockl; i++) {
					plc_pred[i] *= (i - plc_blockl + 10) * (1.0f - (float) 2.0 * ftmp2 / ftmp1) / (10)
							+ 2.0f * ftmp2 / ftmp1;
				}
			}
		...
		if (this.ULP_inst.mode == 20) {
			...
			for (iblock = 0; iblock < 2; iblock++) {
				enhancer(out, iblock * ilbc_constants.ENH_BLOCKL, enh_buf, ilbc_constants.ENH_BUFL,
						(5 + iblock) * ilbc_constants.ENH_BLOCKL + 40, ilbc_constants.ENH_ALPHA0, enh_period,
						ilbc_constants.enh_plocsTbl, ilbc_constants.ENH_NBLOCKS_TOT);
			}
		} else if (this.ULP_inst.mode == 30) {
			/* Enhancer with 80 samples delay */
			for (iblock = 0; iblock < 3; iblock++) {
				enhancer(out, iblock * ilbc_constants.ENH_BLOCKL, enh_buf, ilbc_constants.ENH_BUFL,
						(4 + iblock) * ilbc_constants.ENH_BLOCKL, ilbc_constants.ENH_ALPHA0, enh_period,
						ilbc_constants.enh_plocsTbl, ilbc_constants.ENH_NBLOCKS_TOT);
			}
		}
...
//	memcpy(psseq, idata+centerStartPos, ENH_BLOCKL*sizeof(float));
		...
		for (q = hl - 1; q >= 0; q--) {
			...
			lagBlock[q] = NearestNeighbor(plocs,
					blockStartPos[q] + ilbc_constants.ENH_BLOCKL_HALF - period[lagBlock[q + 1]], periodl);

			if (blockStartPos[q] - ilbc_constants.ENH_OVERHANG >= 0) {
				blockStartPos[q] = refiner(sseq, q * ilbc_constants.ENH_BLOCKL, idata, idatal, centerStartPos,
						blockStartPos[q], period[lagBlock[q + 1]]);
			} else {
...
//		memset(psseq, 0, ENH_BLOCKL*sizeof(float));
		...
		for (q = hl + 1; q <= 2 * hl; q++) {
			...
			if (blockStartPos[q] + ilbc_constants.ENH_BLOCKL + ilbc_constants.ENH_OVERHANG < idatal) {
				blockStartPos[q] = refiner(sseq, q * ilbc_constants.ENH_BLOCKL, idata, idatal, centerStartPos,
						blockStartPos[q], period[lagBlock[q]]);
			} else {
				psseq = q * ilbc_constants.ENH_BLOCKL;
// 		psseq=sseq+q*ENH_BLOCKL;
				for (int li = 0; li < ilbc_constants.ENH_BLOCKL; li++)
...
// 				&pos);
						cb_index[i * ilbc_constants.CB_NSTAGES + k] = bytes.packcombine(
								cb_index[i * ilbc_constants.CB_NSTAGES + k], lastpart,
								this.ULP_inst.cb_index[i][k][ulp]);
...
// 		       this.ULP_inst.blockl*sizeof(float));
		...
		if (mode == 0) {
			/*
			 * the data is bad (either a PLC call was made or a severe bit error was
			 * detected)
			 */
...
// 		System.out.println("run 1");
				syntFilter(data, i * ilbc_constants.SUBL, this.old_syntdenum,
						(i + this.ULP_inst.nsub - 1) * (ilbc_constants.LPC_FILTERORDER + 1), ilbc_constants.SUBL,
						this.syntMem);
// 		System.out.println("runs 2");
				for (i = 1; i < this.ULP_inst.nsub; i++) {
// 		    System.out.println("pass " + i);
					syntFilter(data, i * ilbc_constants.SUBL, syntdenum, (i - 1) * (ilbc_constants.LPC_FILTERORDER + 1),
...
// 		System.out.println("runs 3");
				for (i = 0; i < 2; i++) {
					syntFilter(data, i * ilbc_constants.SUBL, this.old_syntdenum,
							(i + this.ULP_inst.nsub - 2) * (ilbc_constants.LPC_FILTERORDER + 1), ilbc_constants.SUBL,
							this.syntMem);
				}
				for (i = 2; i < this.ULP_inst.nsub; i++) {
// 		    System.out.println("runs 4");
					syntFilter(data, i * ilbc_constants.SUBL, syntdenum, (i - 2) * (ilbc_constants.LPC_FILTERORDER + 1),
							ilbc_constants.SUBL, this.syntMem);
				}
			}
			...
			/* Find last lag */
			lag = 20;
			maxcc = xCorrCoef(decresidual, ilbc_constants.BLOCKL_MAX - ilbc_constants.ENH_BLOCKL, decresidual,
					ilbc_constants.BLOCKL_MAX - ilbc_constants.ENH_BLOCKL - lag, ilbc_constants.ENH_BLOCKL);

			for (ilag = 21; ilag < 120; ilag++) {
				cc = xCorrCoef(decresidual, ilbc_constants.BLOCKL_MAX - ilbc_constants.ENH_BLOCKL, decresidual,
						ilbc_constants.BLOCKL_MAX - ilbc_constants.ENH_BLOCKL - ilag, ilbc_constants.ENH_BLOCKL);
...
// 	    System.out.println("runs 5");
			for (i = 0; i < this.ULP_inst.nsub; i++) {
				syntFilter(data, i * ilbc_constants.SUBL, syntdenum, i * (ilbc_constants.LPC_FILTERORDER + 1),
						ilbc_constants.SUBL, this.syntMem);
			}
...
// 	System.out.println("appel 1");
		mycorr1(corrVec, 0, idata, searchSegStartPos, corrdim + ilbc_constants.ENH_BLOCKL - 1, idata, centerStartPos,
	...
	*---------------------------------------------------------------*/
			...
			float alpha0)/* (i) max smoothing energy fraction */
	{
		...
		float[] surround = new float[ilbc_constants.BLOCKL_MAX]; /*
																	 * shape contributed by other than current
																	 */
		float[] wt = new float[2 * ilbc_constants.ENH_HL + 1]; /*
																 * waveform weighting to get surround shape
																 */
		float denom;
		int psseq;
