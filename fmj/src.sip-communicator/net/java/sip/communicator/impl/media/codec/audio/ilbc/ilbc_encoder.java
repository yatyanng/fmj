...
class ilbc_encoder {
			...
			int freq_idx, float a[]) /* (i) lpc coefficients */
	{
		...
		/*
		 * Here we loop through lsp_index to find all the LPC_FILTERORDER roots for
		 * omega.
		 */

		for (lsp_index = 0; lsp_index < ilbc_constants.LPC_FILTERORDER; lsp_index++) {
			/*
			 * Depending on lsp_index being even or odd, we alternatively solve the roots
			 * for the two LSP equations.
			 */
	...
	void AbsQuantW(float in[], /* (i) vector to encode */
		...
		if (state_first != 0) {
			ilbc_common.AllPoleFilter(in, in_idx, weightDenum, weightDenum_idx, ilbc_constants.SUBL,
					ilbc_constants.LPC_FILTERORDER);
		} else {
			ilbc_common.AllPoleFilter(in, in_idx, weightDenum, weightDenum_idx,
					this.ULP_inst.state_short_len - ilbc_constants.SUBL, ilbc_constants.LPC_FILTERORDER);
		}
	...
	private void anaFilter(float In[], int in_idx, float a[], int a_idx, int len, float Out[], int out_idx,
			float mem[]) {
		int i, j;
		int po, pi, pm, pa;
	...
	public short encode(short encoded_data[], short data[]) {
		...
		for (k = 0; k < encoded_data.length; k++)
			encoded_data[k] = (short) (((en_data.buffer[2 * k] << 8) & 0xff00)
					| (((short) en_data.buffer[2 * k + 1]) & 0x00ff));
			...
			float residual[]) /* (i) lpc residual signal */
	{
		...
		float[] ssqEn_win = { (float) 0.8, (float) 0.9, 1.0f, (float) 0.9, (float) 0.8 };

		float[] sampEn_win = { 1.0f / (float) 6.0, 2.0f / (float) 6.0, 3.0f / (float) 6.0, 4.0f / (float) 6.0,
				5.0f / (float) 6.0 };
			...
			int block) /* (i) the sub-block number */
	{
		...
		ilbc_common.AllPoleFilter(buf, ilbc_constants.LPC_FILTERORDER, weightDenum, weightDenum_idx, lMem + lTarget,
				ilbc_constants.LPC_FILTERORDER);
		...
		/*
		 * Prepare search over one more codebook section. This section is created by
		 * filtering the original buffer with a filter.
		 */
		...
		for (stage = 0; stage < nStages; stage++) {
			...
			/*
			 * Loop over augmented part in the first codebook section, full search. The
			...
			if ((lTarget == ilbc_constants.SUBL) && (sIndAug != 0)) {
				...
				b[0] = best_index;
				c[0] = gain;
				searchAugmentedCB(sIndAug, eIndAug, stage, 2 * base_size - 20, target, cbvectors, lMem, a, b, c, energy,
						invenergy);
				max_measure = a[0];
				best_index = b[0];
				gain = c[0];
			...
			if (lTarget == (ilbc_constants.STATE_LEN - this.ULP_inst.state_short_len)) {
				...
				if (index[index_idx + stage] < base_size) {
					...
					if (position < (base_size - 20)) {
						...
						createAugmentedVec(index[index_idx + stage] - (filterno + 1) * base_size + 40, cbvectors,
			...
			/*
			 * Subtract the best codebook vector, according to measure, from the target
			 * vector
			 */
			...
			float block[]) /* (o) speech vector to encode */
	{
		...
		/*
		 * high pass filtering of input signal if such is not done prior to calling this
		 * function
		 */
		...
		StateSearchW(residual, start_pos, syntdenum, (start - 1) * (ilbc_constants.LPC_FILTERORDER + 1), weightdenum,
				(start - 1) * (ilbc_constants.LPC_FILTERORDER + 1), idxForMax, idxVec, this.ULP_inst.state_short_len,
				state_first);

		ilbc_common.StateConstructW(idxForMax[0], idxVec, syntdenum, (start - 1) * (ilbc_constants.LPC_FILTERORDER + 1),
		...
		if (state_first != 0) { /* put adaptive part in the end */
			...
			for (int li = 0; li < ilbc_constants.CB_MEML - this.ULP_inst.state_short_len; li++)
				mem[li] = 0.0f;
			System.arraycopy(decresidual, start_pos, mem, ilbc_constants.CB_MEML - this.ULP_inst.state_short_len,
			...
			iCBSearch(extra_cb_index, 0, extra_gain_index, 0, residual, start_pos + this.ULP_inst.state_short_len, mem,
					ilbc_constants.CB_MEML - ilbc_constants.stMemLTbl, ilbc_constants.stMemLTbl, diff,
					ilbc_constants.CB_NSTAGES, weightdenum, start * (ilbc_constants.LPC_FILTERORDER + 1), weightState,
					0);

			/* construct decoded vector */

			ilbc_common.iCBConstruct(decresidual, start_pos + this.ULP_inst.state_short_len, extra_cb_index, 0,
					extra_gain_index, 0, mem, ilbc_constants.CB_MEML - ilbc_constants.stMemLTbl,
			...
			iCBSearch(extra_cb_index, 0, extra_gain_index, 0, reverseResidual, 0, mem,
					ilbc_constants.CB_MEML - ilbc_constants.stMemLTbl, ilbc_constants.stMemLTbl, diff,
					ilbc_constants.CB_NSTAGES, weightdenum, (start - 1) * (ilbc_constants.LPC_FILTERORDER + 1),
					weightState, 0);

			/* construct decoded vector */

			ilbc_common.iCBConstruct(reverseDecresidual, 0, extra_cb_index, 0, extra_gain_index, 0, mem,
					ilbc_constants.CB_MEML - ilbc_constants.stMemLTbl, ilbc_constants.stMemLTbl, diff,
		...
		if (Nfor > 0) {
			...
			for (subframe = 0; subframe < Nfor; subframe++) {
				...
				iCBSearch(cb_index, subcount * ilbc_constants.CB_NSTAGES, gain_index,
						subcount * ilbc_constants.CB_NSTAGES, residual, (start + 1 + subframe) * ilbc_constants.SUBL,
						mem, ilbc_constants.CB_MEML - ilbc_constants.memLfTbl[subcount],
						ilbc_constants.memLfTbl[subcount], ilbc_constants.SUBL, ilbc_constants.CB_NSTAGES, weightdenum,
						(start + 1 + subframe) * (ilbc_constants.LPC_FILTERORDER + 1), weightState, subcount + 1);

				/* construct decoded vector */

				ilbc_common.iCBConstruct(decresidual, (start + 1 + subframe) * ilbc_constants.SUBL, cb_index,
						subcount * ilbc_constants.CB_NSTAGES, gain_index, subcount * ilbc_constants.CB_NSTAGES, mem,
						ilbc_constants.CB_MEML - ilbc_constants.memLfTbl[subcount], ilbc_constants.memLfTbl[subcount],
		...
		if (Nback > 0) {
			...
			for (n = 0; n < Nback; n++) {
				for (k = 0; k < ilbc_constants.SUBL; k++) {
					reverseResidual[n * ilbc_constants.SUBL + k] = residual[(start - 1) * ilbc_constants.SUBL - 1
							- n * ilbc_constants.SUBL - k];
					reverseDecresidual[n * ilbc_constants.SUBL + k] = decresidual[(start - 1) * ilbc_constants.SUBL - 1
							- n * ilbc_constants.SUBL - k];
				}
			}
			...
			for (subframe = 0; subframe < Nback; subframe++) {
				...
				iCBSearch(cb_index, subcount * ilbc_constants.CB_NSTAGES, gain_index,
						subcount * ilbc_constants.CB_NSTAGES, reverseResidual, subframe * ilbc_constants.SUBL, mem,
						ilbc_constants.CB_MEML - ilbc_constants.memLfTbl[subcount], ilbc_constants.memLfTbl[subcount],
						ilbc_constants.SUBL, ilbc_constants.CB_NSTAGES, weightdenum,
						(start - 2 - subframe) * (ilbc_constants.LPC_FILTERORDER + 1), weightState, subcount + 1);

				/* construct decoded vector */

				ilbc_common.iCBConstruct(reverseDecresidual, subframe * ilbc_constants.SUBL, cb_index,
						subcount * ilbc_constants.CB_NSTAGES, gain_index, subcount * ilbc_constants.CB_NSTAGES, mem,
						ilbc_constants.CB_MEML - ilbc_constants.memLfTbl[subcount], ilbc_constants.memLfTbl[subcount],
						ilbc_constants.SUBL, ilbc_constants.CB_NSTAGES);
		...
		for (ulp = 0; ulp < 3; ulp++) {
			...
			for (k = 0; k < ilbc_constants.LSF_NSPLIT * this.ULP_inst.lpc_n; k++) {
				...
				pack = bytes.packsplit(lsf_i[k], this.ULP_inst.lsf_bits[k][ulp], this.ULP_inst.lsf_bits[k][ulp]
						+ this.ULP_inst.lsf_bits[k][ulp + 1] + this.ULP_inst.lsf_bits[k][ulp + 2]);
			...
			pack = bytes.packsplit(start, this.ULP_inst.start_bits[ulp], this.ULP_inst.start_bits[ulp]
					+ this.ULP_inst.start_bits[ulp + 1] + this.ULP_inst.start_bits[ulp + 2]);
			firstpart = pack.get_firstpart();
			start = pack.get_rest();
			...
			pack = bytes.packsplit(state_first, this.ULP_inst.startfirst_bits[ulp], this.ULP_inst.startfirst_bits[ulp]
					+ this.ULP_inst.startfirst_bits[ulp + 1] + this.ULP_inst.startfirst_bits[ulp + 2]);
			firstpart = pack.get_firstpart();
			state_first = pack.get_rest();
			bytes.dopack(firstpart, this.ULP_inst.startfirst_bits[ulp]);

			// System.out.println("scale bits");
			pack = bytes.packsplit(idxForMax[0], this.ULP_inst.scale_bits[ulp], this.ULP_inst.scale_bits[ulp]
					+ this.ULP_inst.scale_bits[ulp + 1] + this.ULP_inst.scale_bits[ulp + 2]);
			firstpart = pack.get_firstpart();
			...
			for (k = 0; k < this.ULP_inst.state_short_len; k++) {
				...
				pack = bytes.packsplit(idxVec[k], this.ULP_inst.state_bits[ulp], this.ULP_inst.state_bits[ulp]
						+ this.ULP_inst.state_bits[ulp + 1] + this.ULP_inst.state_bits[ulp + 2]);
				firstpart = pack.get_firstpart();
				...
				for (k = 0; k < ilbc_constants.CB_NSTAGES; k++) {
					pack = bytes.packsplit(cb_index[i * ilbc_constants.CB_NSTAGES + k],
							this.ULP_inst.cb_index[i][k][ulp], this.ULP_inst.cb_index[i][k][ulp]
									+ this.ULP_inst.cb_index[i][k][ulp + 1] + this.ULP_inst.cb_index[i][k][ulp + 2]);
					firstpart = pack.get_firstpart();
				...
				for (k = 0; k < ilbc_constants.CB_NSTAGES; k++) {
					pack = bytes.packsplit(gain_index[i * ilbc_constants.CB_NSTAGES + k],
							this.ULP_inst.cb_gain[i][k][ulp], this.ULP_inst.cb_gain[i][k][ulp]
									+ this.ULP_inst.cb_gain[i][k][ulp + 1] + this.ULP_inst.cb_gain[i][k][ulp + 2]);
					firstpart = pack.get_firstpart();
		...
		/*
		 * set the last bit to zero (otherwise the decoder will treat it as a lost
		 * frame)
		 */
			...
			float data[]) /* (i) new data vector */
	{
		...
		System.arraycopy(this.lpc_buffer, ilbc_constants.LPC_LOOKBACK + ilbc_constants.BLOCKL_MAX - is, this.lpc_buffer,
				0, is);
	...
	void SimpleInterpolateLSF(float syntdenum[], /*
													 * (o) the synthesis filter denominator resulting from the quantized
													 * interpolated lsf
													 */
			float weightdenum[], /*
			...
			int length) /* (i) should equate LPC_FILTERORDER */
	{
		...
		if (this.ULP_inst.mode == 30) {
			...
			/*
			 * sub-frame 2 to 6: Interpolation between first and second set of lsf
			 * coefficients
			 */
			...
			for (i = 0; i < this.ULP_inst.nsub; i++) {
				...
				for (int li = 0; li < lp_length; li++)
					...
					// memcpy(syntdenum+pos,lp,lp_length*sizeof(float));
					LSFinterpolate2a_enc(lp, lsfold, lsf, 0, ilbc_constants.lsf_weightTbl_20ms[i], length);
				ilbc_common.bwexpand(weightdenum, pos, lp, ilbc_constants.LPC_CHIRP_WEIGHTDENUM, lp_length);
				pos += lp_length;
			}
		}
								 ...
								 */
	{
		...
		/*
		 * decoding of the maximum amplitude representation value, and corresponding
