...
public class Utils {
	/**
	 * Converts a byte array into a short array. Since a byte is 8-bits, and a short
	 * is 16-bits, the returned short array will be half in length than the byte
	 * array. If the length of the byte array is odd, the length of the short array
	 * will be <tt>(byteArray.length - 1)/2</tt>, i.e., the last byte is discarded.
	 *
	 ...
	 */
	public static short[] byteToShortArray(byte[] byteArray, int offset, int length, boolean little)
		...
		if (0 < length && (offset + length) <= byteArray.length) {
			...
			throw new ArrayIndexOutOfBoundsException(
					"offset: " + offset + ", length: " + length + ", array length: " + byteArray.length);
		}
	}

	/**
	 * The result array must be twice as the input one. Since a byte is 8-bits, and
	 * a short is 16-bits.
	 */
