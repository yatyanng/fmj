...
class ilbc_common {
	...
	public static void AllPoleFilter(float InOut[], /*
													 * (i/o) on entrance InOut[-orderCoef] to InOut[-1] contain the
	...
	public static void AllZeroFilter(float In[], /*
													 * (i) In[0] to In[lengthInOut-1] contain filter input samples
													 */
			int In_idx, float Coef[], /*
										 * (i) filter coefficients (Coef[0] is assumed to be 1.0f)
										 */
	...
	public static void getCBvec(float cbvec[], /*
			...
			int mem_idx, int index, /* (i) Codebook index */
	...
	public static void iCBConstruct(float decvector[], /* (o) Decoded vector */
			int decvector_idx, int index[], /* (i) Codebook indices */
			int index_idx, int gain_index[], /* (i) Gain quantization indices */
			int gain_index_idx, float mem[], /*
	...
	public static void lsf2a(float a_coef[], float freq[]) {
		...
		/*
		 * Check input for ill-conditioned cases. This part is not found in the TIA
		 * standard. It involves the following 2 IF blocks. If "freq" is judged
		 * ill-conditioned, then we first modify freq[0] and freq[LPC_HALFORDER-1]
		 * (normally LPC_HALFORDER = 10 for LPC applications), then we adjust the other
		...
		if ((freq[0] <= 0.0f) || (freq[ilbc_constants.LPC_FILTERORDER - 1] >= 0.5)) {
			...
			hlp = (freq[ilbc_constants.LPC_FILTERORDER - 1] - freq[0]) / (ilbc_constants.LPC_FILTERORDER - 1);
		...
		/*
		 * p[i] and q[i] compute cos(2*pi*omega_{2j}) and cos(2*pi*omega_{2j-1} in eqs.
		 * 4.2.2.2-1 and 4.2.2.2-2. Note that for this code p[i] specifies the
		 * coefficients used in .Q_A(z) while q[i] specifies the coefficients used in
		 * .P_A(z)
		 */
	...
	public static void StateConstructW(int idxForMax, /*
														 * (i) 6-bit index for the quantization of max amplitude
														 */
			int idxVec[], /* (i) vector of quantization indexes */
			float syntDenum[], /* (i) synthesis filter denumerator */
			int syntDenum_idx, float out[], /* (o) the decoded state vector */
			int out_idx, int len /* (i) length of a state vector */
	) {
		float maxVal;
		...
		for (k = 0; k < ilbc_constants.LPC_FILTERORDER; k++) {
			...
			// " (( " + syntDenum[syntDenum_idx + ilbc_constants.LPC_FILTERORDER
			// - k]);
		}
	...
	public static void ZeroPoleFilter(float In[], /*
			...
			int In_idx, float ZeroCoef[], /*
											 * (i) filter coefficients for all-zero section (ZeroCoef[0] is assumed to
											 * be 1.0f)
											 */
			float PoleCoef[], /*
								 * (i) filter coefficients for all-pole section (ZeroCoef[0] is assumed to be
								 * 1.0f)
								 */
