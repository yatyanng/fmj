...
public abstract class AbstractPlayer extends AbstractController implements Player, ControllerListener {
	 ...
	 *
	 * @param newController The Controller to add
	 *
	 * @exception NotRealizedError              If this Player or the new Controller
	 *                                          are not Realized.
	 *
	 * @exception ClockStartedError             If this Player or the new Controller
	 *                                          are in the Started state.
	 *
	 * @exception IncompatibleTimeBaseException Thrown by
	 *                                          newController.setTimeBase() if the
	 *                                          new Controller cannot use this
	 *                                          player's TimeBase
	 */
	@Override
	...
	/**
	 * Checks to see if the all of this Player's managed Controller have stopped.
	 * Usually called from within endOfMedia() to see if an EndOfMediaEvent can be
	 * posted.
	 *
	 * @return true if all of this Player's managed Controller have stopped, false
	 *         otherwise.
	 */
	 ...
	 * keeping track of the actions of each Controller, the Player can synchronize
	 * the state transtitions of multiple Controllers.
	 * <p>
	 * This method is final because the controller-management functionality of the
	 * AbstractPlayer will not work if it is overridden.
	 *
	 * @param e The ControllerEvent posted by one of the managed Controllers.
	 */
	@Override
	...
	/**
	 * Realize player and all of its managed Controllers. Subclasses should override
	 * doPlayerRealize() to do the actual work to transition the Controller.
	 * <p>
	 * This method should not be called directly. Instead, call realize().
	 */
	@Override
	...
	/**
	 * Sets the rate for this Player and all of its managed Controllers. If any of
	 * the above cannot accomodate the given rate, then the rate is set to 1.0 for
	 * all of the above.
	 * <p>
	 * This method should not be called directly. Instead, call setRate().
	 *
	...
	/**
	 * Stop player on current thread. Subclasses should override doPlayerStop() to
	...
	/**
	 * Start player and all of its managed Controllers at the given time. Subclasses
	 * should override doPlayerSyncStart() to do the actual work to transition the
	 * Controller.
	 * <p>
	...
	/**
	 * Returns the duration of the media played by <U>this</U> Player only. It does
	 * not consider any of the Controllers that this Player may be managing.
	 */
	public abstract Time getPlayerDuration();

	/**
	 * Returns the start latency of the media played by <U>this</U> Player only. It
	 * does not consider any of the Controllers that this Player may be managing.
	 */
	...
	/**
	 * Gets the start latency of this Player. It is defined to be the longest start
	 * latency between this Player and any of the Controllers that this Player may
	 * be managing. If any of the Controllers returns LATENCY_UNKNOWN, it's value is
	 * skipped in the calculation of the maximum latency.
	 *
	...
	public synchronized Time getStartLatency() {
		...
		for (int i = 0, n = controllers.size(); i < n; i++) {
			...
			if (latency == LATENCY_UNKNOWN || l.getNanoseconds() > latency.getNanoseconds()) {
				latency = l;
			}
		}
	...
	/**
	 * Post a ManagedControllerErrorEvent to the Media Event Queue. Automatically
	 * fill in the managing Player, the ControllerErrorEvent, and the message
	 * properties of the ManagedControllerErrorEvent.
	 */
	private void postManagedControllerErrorEvent() {
		String message = "Managing Player " + getClass().getName() + " received ControllerErrorEvent from "
				+ controllerError.getSourceController().getClass().getName();

		postEvent(new ManagedControllerErrorEvent(this, controllerError, message));
	 ...
	 *
	 * @param oldController The Controller to remove
	 */
	@Override
	...
	/**
	 * Sets the controller error that occurred while waiting for a state transition.
	 */
	 ...
	 *
	 * @param c A java.awt.Component providing control over this AbstractPlayer's
	 *          media.
	 */
	...
	/**
	 * Set the GainControl for this AbstractPlayer. If the AbstractPlayer does not
	 * support audio media, this method should return null.
	 *
	 * @param c The GainControl allowing control of the volume of this
	...
	/**
	 * Sets the rate. If the Player is in the Started state, it is stopped before
	 * setting the rate and restarted afterwards.
	 *
	 ...
	 * this Player can support the given DataSource. If it can't, this method should
	 * throw a IncompatibleSourceException. Our only requirement here is that the
	 * DataSource has not already been set. Subclasses may wish to override this
	 * method to extend the acceptance criteria.
	 *
	 * @param source The DataSource to test
	 *
	 * @exception IncompatibleSourceException Thrown if the DataSource has already
	 *                                        been set on this MediaHandler, or if
	 *                                        the DataSource is not a PullDataSource
	 */
	@Override
	...
	/**
	 * Set the visual Component for this AbstractPlayer. If the AbstractPlayer does
	 * not support video media, this method should return null.
	 *
	 * @param c A java.awt.Component on which the media is rendered.
	 */
	...
	/**
	 * Start the player on a new thread. If necessary, the Player will be prefetched
	 * before being started. Subclasses should override doSyncStart() to do the
	 * actual work to transition the Controller.
	 * <p>
	 * Checks for Player state prerequisites and creates a StartThread to start the
	 * AbstractPlayer. If there is already a thread transitioning the AbstractPlayer
	 * forward, then the target state of the AbstractPlayer is set to Started and
	 * the method returns.
	 * <p>
	...
	/**
	 * Update the duration of this Player. It is defined to be the longest duration
	 * between this Player and any of the Controllers that this Player may be
	 * managing. If any of the Controllers returns DURATION_UNKNOWN or
	...
	private synchronized final void updateDuration() {
		...
		if (duration != DURATION_UNKNOWN) {
			for (int i = 0, n = controllers.size(); i < n; i++) {
				...
				if (duration != DURATION_UNBOUNDED
						&& (d == DURATION_UNBOUNDED || d.getNanoseconds() > duration.getNanoseconds())) {
					duration = d;
				}
			}
		}
		...
		if (duration == DURATION_UNKNOWN || duration == DURATION_UNBOUNDED || this.duration == DURATION_UNKNOWN
		...
		if (this.duration == null || duration.getNanoseconds() != this.duration.getNanoseconds()) {
			newDuration = true;
		}
