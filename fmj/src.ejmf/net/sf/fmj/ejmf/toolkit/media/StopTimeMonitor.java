...
public class StopTimeMonitor extends CloseableThread implements ControllerListener {
	 ...
	 *
	 * @param controller The AbstractController to whose stop time to monitor.
	 */
	 ...
	 *
	 * @param e The ControllerEvent
	 */
	@Override
	public synchronized void controllerUpdate(ControllerEvent e) {
		if (e instanceof StopTimeChangeEvent || e instanceof RateChangeEvent || e instanceof MediaTimeSetEvent
				|| e instanceof StartEvent || (e instanceof StopEvent && !(e instanceof DeallocateEvent))) {
			wokenUp = true;
			notifyAll();
		}
	}
	...
	/**
	 * Calculates the time that the monitor should sleep (in milliseconds) before
	 ...
	 * until a. the controller is started, and b. a stop time has been set on the
	 * AbstractController. Then calculate the how long to wait before stopping the
	 * controller, based on the current media time, rate, and stop time. If we are
	 * woken up by an event, then recalculate and begin again. Otherwise, stop the
