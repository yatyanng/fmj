...
public class AbstractClock implements Clock {
	...
	/**
	 * Calculates the media time on a started Clock based on the assumption that the
	 * Clock is started.
	 */
	...
	/**
	 * Get the media time the media is scheduled to start (if the Clock is stopped),
	 * or the media time at which the Clock started (if the Clock is started).
	 */
	 ...
	 * <tt>TimeBase</tt>.
	 * <p>
	 * For example, a rate of 2.0 indicates that <i>media time</i> will pass twice
	 * as fast as the <tt>TimeBase</tt> time once the <tt>Clock</tt> starts.
	 * Similarly, a negative rate indicates that the <tt>Clock</tt> runs in the
	 * opposite direction of its <tt>TimeBase</tt>. All <tt>Clocks</tt> are
	 * guaranteed to support a rate of 1.0, the default rate. <tt>Clocks</tt> are
	 * not required to support any other rate.
	 * <p>
	 */
	@Override
	 ...
	 *
	 * @return The time remaining until the time-base start-time if this Clock is
	 *         Started and the time-base start-time has not yet been reached, or the
	 *         media time otherwise.
	 */
	@Override
	 ...
	 * stopped), or the time-base time at which the Clock started (if the Clock is
	 * started).
	 */
	 ...
	 * calculate.
	 *
	 * @param t A media time to be mapped to a time-base time.
	 *
	 * @return A time-base time.
	 *
	 * @exception ClockStoppedException If the clock has not started.
	 */
	@Override
	 ...
	 *
	 * @exception ClockStartedError If the Clock is Started.
	 */
	@Override
	...
	/**
	 * Set the <i>media time</i> at which you want the <tt>Clock</tt> to stop. The
	 * <tt>Clock</tt> will stop when its <i>media time</i> passes the stop-time. To
	 * clear the stop time, set it to: <tt>Clock.RESET</tt>.
	 * <p>
	 * You can always call <tt>setStopTime</tt> on a <i>Stopped</i> <tt>Clock</tt>.
	 * <p>
	 * On a <i>Started</i> <tt>Clock</tt>, the stop-time can only be set
	 * <I>once</I>. A <tt>StopTimeSetError</tt> is thrown if <tt>setStopTime</tt> is
