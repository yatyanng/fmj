...
public abstract class AbstractController extends AbstractClock implements Controller {
	 ...
	 *
	 * @param listener The listener to which the Controller will post events.
	 */
	@Override
	...
	/**
	 * For a given time-base start time, block until the AbstractController should
	 * be started. For a given time- base start time (t), this method will get the
	 * AbstractController's start latency (l) and block until (t - l). This method
	 * is useful for implementations of doSyncStart().
	 * <p>
	...
	/**
	 * Close the Controller. Release resources held by this Controller. Subclasses
	 * should implement doClose() to add additional functionality.
	 */
	@Override
	 ...
	 * doDeallocate() returns true, then the Controller is placed in the appropriate
	 * state and a DeallocateCompleteEvent is posted. Otherwise, it is assumed that
	 * the controller has posted a ControllerErrorEvent detailing the reasons for
	 * it's failure.
	 * <p>
	...
	/**
	 * Close the Controller. Typically this method will release as many resources as
	 * possible, especially those that may be needed by other Controllers.
	 * <p>
	...
	/**
	 * Override to provide implementation-specific functionality. When this method
	 * is called, it is guaranteed that the Controller is Stopped and that the given
	 * time is within the Controller's duration.
	 * <p>
	 * This method should not be called directly. Instead, call setMediaTime().
	 *
	 * @param t The media time to set
	 */
	public abstract void doSetMediaTime(Time t);

	/**
	 * Override to provide implementation-specific functionality. When this method
	 * is called, it is guaranteed that the Controller is Stopped.
	 * <p>
	 * This method should not be called directly. Instead, call setRate().
	 *
	 * @param rate The requested rate to set
	 *
	...
	/**
	 * Indicates to the framework that the end of media has been reached. Marks the
	 * media time, sets the current and target states to Prefetched, and posts an
	 * EndOfMediaEvent.
	 *
	 * @exception ClockStoppedException If the AbstractController is not in the
	 *                                  Started state.
	 */
	 ...
	 * class or interface name should be specified. Null is returned if the Control
	 * is not supported.
	 *
	...
	/**
	 * Get a list of the Control objects that this Controller supports. If there are
	 * no controls, an array of length zero is returned.
	 *
	...
	/**
	 * Returns DURATION_UNKNOWN. This method should be overridden to report a more
	 * precise duration.
	 */
	@Override
	...
	/**
	 * Calculates the current media time based on the current time-base time, the
	 * time-base start time, the media start time, and the rate.
	 *
	...
	/**
	 * Returns LATENCY_UNKNOWN. This method should be overridden to report a more
	 * precise start latency.
	 */
	@Override
	...
	/**
	 * Post a DataStarvedEvent to the Media Event Queue. Automatically fill in the
	 * Previous State, Current State, Target State, and Media Time properties of the
	 * DataStarvedEvent.
	 */
	protected void postDataStarvedEvent() {
		postEvent(new DataStarvedEvent(this, previousState, currentState, targetState, getMediaTime()));
	}

	/**
	 * Post a DeallocateEvent to the Media Event Queue. Automatically fill in the
	 * Previous State, Current State, Target State, and Media Time properties of the
	 * DeallocateEvent.
	 */
	protected void postDeallocateEvent() {
		postEvent(new DeallocateEvent(this, previousState, currentState, targetState, getMediaTime()));
	}
	...
	/**
	 * Post a EndOfMediaEvent to the Media Event Queue. Automatically fill in the
	 * Previous State, Current State, Target State, and Media Time properties of the
	 * EndOfMediaEvent.
	 */
	protected void postEndOfMediaEvent() {
		postEvent(new EndOfMediaEvent(this, previousState, currentState, targetState, getMediaTime()));
	}

	/**
	 * Post a ControllerEvent to the Media Event Queue
	 *
	 * @param event The ControllerEvent to post.
	 */
	...
	/**
	 * Post a PrefetchCompleteEvent to the Media Event Queue. Automatically fill in
	 * the Previous State, Current State, and Target State properties of the
	 * PrefetchCompleteEvent.
	 */
	protected void postPrefetchCompleteEvent() {
		postEvent(new PrefetchCompleteEvent(this, previousState, currentState, targetState));
	}

	/**
	 * Post a RealizeCompleteEvent to the Media Event Queue. Automatically fill in
	 * the Previous State, Current State, and Target State properties of the
	 * RealizeCompleteEvent.
	 */
	protected void postRealizeCompleteEvent() {
		postEvent(new RealizeCompleteEvent(this, previousState, currentState, targetState));
	}

	/**
	 * Post a RestartingEvent to the Media Event Queue. Automatically fill in the
	 * Previous State, Current State, Target State, and Media Time properties of the
	 * RestartingEvent.
	 */
	protected void postRestartingEvent() {
		postEvent(new RestartingEvent(this, previousState, currentState, targetState, getMediaTime()));
	}

	/**
	 * Post a StartEvent to the Media Event Queue. Automatically fill in the
	 * Previous State, Current State, Target State, Media Time, and Time-base Time
	 * properties of the StartEvent.
	 */
	protected void postStartEvent() {
		postEvent(new StartEvent(this, previousState, currentState, targetState, getMediaStartTime(),
				getTimeBaseStartTime()));
	}

	/**
	 * Post a StopAtTimeEvent to the Media Event Queue. Automatically fill in the
	 * Previous State, Current State, Target State, and Media Time properties of the
	 * StopAtTimeEvent.
	 */
	protected void postStopAtTimeEvent() {
		postEvent(new StopAtTimeEvent(this, previousState, currentState, targetState, getMediaTime()));
	}
	...
	/**
	 * Post a StopByRequestEvent to the Media Event Queue. Automatically fill in the
	 * Previous State, Current State, Target State, and Media Time properties of the
	 * StopByRequestEvent.
	 */
	protected void postStopByRequestEvent() {
		postEvent(new StopByRequestEvent(this, previousState, currentState, targetState, getMediaTime()));
	}
	...
	/**
	 * Post a StopEvent to the Media Event Queue. Automatically fill in the Previous
	 * State, Current State, Target State, and Media Time properties of the
	 * StopEvent.
	 */
	protected void postStopEvent() {
		postEvent(new StopEvent(this, previousState, currentState, targetState, getMediaTime()));
	}

	/**
	 * Post a TransitionEvent to the Media Event Queue. Automatically fill in the
	 * Previous State, Current State, and Target State properties of the
	 * TransitionEvent.
	 */
	protected void postTransitionEvent() {
		postEvent(new TransitionEvent(this, previousState, currentState, targetState));
	}

	/**
	 * Prefetch Controller on new thread. Subclasses should override doPrefetch() to
	 * do the actual work to transition the Controller.
	 * <p>
	 * Checks for Controller state prerequisites and creates a PrefetchThread to
	 * prefetch the AbstractController. If there is already a thread transitioning
	 * the AbstractController forward, then the target state of the
	 * AbstractController is set to Prefetched and the method returns.
	 * <p>
	...
	/**
	 * Realize Controller on new thread. Subclasses should override doRealize() to
	 * do the actual work to transition the Controller.
	 * <p>
	 * Checks for Controller state prerequisites and creates a RealizeThread to
	 * realize the AbstractController. If there is already a thread transitioning
	 * the AbstractController forward, then the target state of the
	 * AbstractController is set to Realized and the method returns.
	 * <p>
	 * Asynchronous method -- Start synchronous transition on another thread and
	 * return ASAP.
	 */
	 ...
	 *
	 * @param oldControl The Control to remove.
	 */
	 ...
	 * @param t The media time to set
	 *
	 * @exception NotRealizedError  If the Controller is not Realized.
	 *
	 * @exception ClockStartedError If the Controller is Started.
	 */
	@Override
	...
	/**
	 * Set the temporal scale factor. The argument <i>suggests</i> the scale factor
	 * to use.
	 * <p>
	 ...
	 *
	 * @param rate The temporal scale factor (rate) to set.
	 *
	 * @exception NotRealizedError  If the Controller is not Realized.
	 *
	 * @exception ClockStartedError If the Controller is Started.
	...
	/**
	 * Sets the stop time for this AbstractController. Posts a StopTimeChangeEvent
	 * if the stop time given is different than the current stop time.
	 *
	 * @param mediaStopTime The time at which you want the <tt>Clock</tt> to stop,
	 *                      in <i>media time</i>.
	 *
	 * @exception NotRealizedError  If the Controller is not Realized.
	 *
	...
	/**
	 * Set the <tt>TimeBase</tt> for this <tt>Clock</tt>. This method can only be
	 * called on a <i>Stopped</i> <tt>Clock</tt>. A <tt>ClockStartedError</tt> is
	 * thrown if <tt>setTimeBase</tt> is called on a <i>Started</i> <tt>Clock</tt>.
	 * <p>
	 * A <tt>Clock</tt> has a default <tt>TimeBase</tt> that is determined by the
	 * implementation. To reset a <tt>Clock</tt> to its default <tt>TimeBase</tt>,
	 * call <tt>setTimeBase(null)</tt>.
	 *
	 * @param timebase The new <tt>TimeBase</tt> or <tt>null</tt> to reset the
	 *                 <tt>Clock</tt> to its default <tt>TimeBase</tt>.
	 *
	 * @exception IncompatibleTimeBaseException Thrown if the <tt>Clock</tt> can't
	 *                                          use the specified <tt>TimeBase</tt>.
	 */
	@Override
	...
	/**
	 * Stop Controller on current thread and post a StopByRequestEvent. Subclasses
	...
	/**
	 * Stop the controller. If the Controller is Realizing or Prefetching, then the
	 * target state will be set to Realized or Prefetched, respectively, and the
	 * Controller will stop when it completes the transition. If the Controller is
	 * Started, this method will call doStop(). If doStop() returns true, then the
	 * Controller is placed in the Prefetched state and a StopEvent is posted. If
	 * doStop() returns false, it is assumed that the controller has posted a
	 * ControllerErrorEvent detailing the reasons for it's failure.
	 * <p>
	 * Synchronous method -- return when transition complete
	 *
	 ...
	 * should override doStop() to do the actual work to stop the Controller.
	 * <p>
	 * This method is usually only called (indirectly) by Player.setMediaTime() or
	 * Player.setRate() when a managed Controller must be stopped before its media
	 * time and rate, respectively, can be set.
	 * <p>
	 * Synchronous method -- return when transition complete
	 */
	...
	/**
	 * SyncStart the AbstractController synchronously at the previously-specified
	...
	/**
	 * SyncStart Controller on new thread. Subclasses should override doSyncStart()
	 * to do the actual work to transition the Controller.
	 * <p>
	 * Checks for Controller state prerequisites and creates a SyncStartThread to
	 * syncstart the AbstractController. The target state of the AbstractController
	 * is then set to Started and the thread is started.
	 * <p>
	 * Asynchronous method -- Start synchronous transition on another thread and
	 * return ASAP.
	 */
