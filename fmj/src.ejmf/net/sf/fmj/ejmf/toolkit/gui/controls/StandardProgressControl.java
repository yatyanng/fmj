...
public class StandardProgressControl extends ChangeListenerControl
	...
	/**
	 * If the progress slider is operational, the controllerUpdate method starts and
	 * stops its time based on Start- and StopEvents from the Controller. If
	 * response to MediaTimeSetEvent, the value of the progress slider is explicitly
	 * set.
	 * <p>
	 * The controllerUpdate method is also responsible for setting the operational
	 * state of the Control based on duration value. This is done in response to
	 * first PrefetchCompleteEvent.
	 */
	@Override
	...
	protected EventListener createControlListener() {
		return new ChangeListener() {
			...
			public void stateChanged(ChangeEvent e) {
				...
				long mediaNanos = ((value * duration) / (s.getMaximum() - s.getMinimum()));
	...
	/**
	 * As part of TimeSource interface, getTime returns the current media time in
	 * nanoseconds.
	 */
	@Override
	public long getTime() {
	...
	private void init() {
		...
		boolean flg = d != Duration.DURATION_UNBOUNDED && d != Duration.DURATION_UNKNOWN;
	 ...
	 * Controller and forcing operational state to false. Availability of progress
	 * bar is determined only after Controller is prefetch and duration is
	 * determinate.
	 */
	@Override
	...
	/**
	 * This method implements the SourcedTimerListener interface. Each timer tick
	 * causes slider thumbnail to move if a ProgressBar was built for this control
	 * panel.
	 *
