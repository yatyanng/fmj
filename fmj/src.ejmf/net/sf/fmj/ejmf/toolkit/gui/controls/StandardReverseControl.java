...
public class StandardReverseControl extends MouseListenerControl {
	...
	/**
	 * Determine operational state of Control based on ability to support negative
	 * rate. If a negative rate is supported, I can use setRate to affect Controller
	 * reverse. Test here for negative rate and then reset old rate.
	 */
	@Override
	 ...
	 * <tt>true</tt>, then the listener semantics are applied in response to mouse
	 * activity. Otherwise, the listener semantics are not applied.
	 *
	 * This is used to disable default semantics if client control panel simulates
	 * reversing media. In this case, <tt>setController</tt> will call
