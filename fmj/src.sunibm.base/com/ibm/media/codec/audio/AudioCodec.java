...
public abstract class AudioCodec extends BasicCodec {
	...
	public Format[] getSupportedOutputFormats(Format in) {
		...
		if (!(in instanceof AudioFormat) || (matches(in, supportedInputFormats) == null)) {
			return new Format[0];
	...
	public Format setInputFormat(Format format) {
		if (!(format instanceof AudioFormat) || (null == matches(format, supportedInputFormats)))
			return null;
	...
	public Format setOutputFormat(Format format) {
		...
		if (!(format instanceof AudioFormat) || (null == matches(format, getMatchingOutputFormats(inputFormat))))
			return null;
