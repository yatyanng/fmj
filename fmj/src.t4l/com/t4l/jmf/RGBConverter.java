...
public class RGBConverter {
	public static void flipVertical(int[] data, int width, int height) {
		...
		/*
		 * int t, offset; for(int y = 0; y<size.height; y++) { offset = y*size.width;
		 * for(int x = 0; x<size.width/2; x++) { t = data[offset+size.width-1-x];
		 * data[offset+size.width-1-x] = data[offset+x]; data[offset+x] = t; } }
		 */
	}
	 ...
	 *         <P>
	 *         For example, if you pass "0xff" this should return zero, because the
	 *         data is already in the domain of [0,255]. If you pass "0xff0000" this
	 *         should return 16, because you need to call: <BR>
	 ...
	 * provided.
	 *
	 * @param image  this needs to be of type INT_ARGB, INT_RGB or INT_ARGB_PRE for
	 *               best performance
	 * @param dest   the int array to populate with red, green and blue components.
	 * @param format the format the dest array needs to be written in
	 */
	public static void populateArray(BufferedImage image, int[] dest, RGBFormat format) {
		...
		if (!(rMask == 0xff0000 && gMask == 0xff00 && bMask == 0xff && format.getLineStride() == width
				&& format.getPixelStride() == 1)) {
	...
	/**
	 * Takes the data in <tt>array</tt> and fills the BufferedImage with that image.
	 *
	 * @param array the array of pixel data
	 * @param image the image to store the data in
	 * @param vf    the format the array is provided in
	 */
	...
	/**
	 * This takes the array of generic red, green and blue data from the Buffer and
	 * converts it into RGB space.
	 * <P>
	 * Note that just because the data is in an RGBFormat does NOT mean that the
	 * pixel data is ordered red-green-blue. It could be BGR. The masks may vary,
	 * there is the scanline stride to account for, etc.
	 * <P>
	 * This method rewrites <tt>array</tt> so it is simply RGB data. (Or ARGB data,
	 * if targetType is TYPE_INT_ARGB)
	 *
	 * @param array      the raw data
	 * @param vf         the RGBFormat the <tt>array</tt> argument is encoded in
	 * @param targetType this should be BufferedImage.TYPE_INT_RGB or
	...
	private static void processData(int[] array, int arrayOffset, RGBFormat vf, int targetType) {
		...
		if (arrayOffset == 0 && vf.getPixelStride() == 1 && padding == 0 && rMask == 0xff0000 && gMask == 0xff00
				&& bMask == 0xff) { // it's already encoded as RGB, with no padding
		...
		/**
		 * This is the generic catch-all solution. This handles arbitrary RGB masks and
		 * padding:
		 */
		int color, r, g, b;
		int i = 0;
