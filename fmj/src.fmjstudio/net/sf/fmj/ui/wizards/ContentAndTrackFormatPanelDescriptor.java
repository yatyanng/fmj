...
public class ContentAndTrackFormatPanelDescriptor extends WizardPanelDescriptor {
	...
	public ContentAndTrackFormatPanelDescriptor(Object nextPanelDescriptor,
			ContentDescriptorFilter contentDescriptorFilter, final ProcessorWizardConfig config,
	...
	public boolean aboutToDisplayPanel(Object prevId) {
		if (prevId == getBackPanelDescriptor()) {
			...
			getContentAndTrackFormatPanel().getComboFormat()
					.setModel(new javax.swing.DefaultComboBoxModel(contentDescriptors));
			...
			for (int i = 0; i < trackControls.length; i++) {
				...
				/*
				 * Available track format: dvi/rtp, 8000.0 Hz, 4-bit, Mono Available track
				 * format: dvi/rtp, 11025.0 Hz, 4-bit, Mono Available track format: dvi/rtp,
				 * 22050.0 Hz, 4-bit, Mono Available track format: ULAW/rtp, 8000.0 Hz, 8-bit,
				 * Mono, FrameSize=8 bits Available track format: gsm/rtp, 8000.0 Hz, Mono,
				 * FrameSize=264 bits
				 */
				...
				// TODO: hard-coded
				getContentAndTrackFormatPanel().getAudioFormatPanel().getComboAudioEncoding()
						.setModel(new javax.swing.DefaultComboBoxModel(new String[] { AudioFormat.ULAW_RTP }));
				// getContentAndTrackFormatPanel().getAudioFormatPanel().getComboAudioEncoding().setModel(
				// new javax.swing.DefaultComboBoxModel(new String[] {
				// AudioFormat.LINEAR }));
				getContentAndTrackFormatPanel().getAudioFormatPanel().getComboAudioSampleRate()
						.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "8000" }));

				if (config.trackConfigs != null && config.trackConfigs.length >= i + 1
						&& config.trackConfigs[i].format instanceof AudioFormat) { // copy from prefs
					trackConfigs[i].format = config.trackConfigs[i].format;
					trackConfigs[i].enabled = config.trackConfigs[i].enabled;
				} else { // default
					trackConfigs[i].format = new AudioFormat(AudioFormat.ULAW_RTP, 8000.0, 8, 1,
				...
				for (int j = 0; j < formats.length; ++j) {
					...
					if (formats[j].matches(trackConfigs[i].format)) {
						...
						getContentAndTrackFormatPanel().addTrack(i, trackConfigs[i].enabled,
	...
	public boolean aboutToHidePanel(Object idOfNext) {
		if (idOfNext == getNextPanelDescriptor()) {
			...
			for (int i = 0; i < trackConfigs.length; i++) {
				trackConfigs[i].enabled = getContentAndTrackFormatPanel().getTrackControlPanel(i)
						.getCheckBoxEnableTrack().isSelected();

				if (trackConfigs[i].enabled) { // TODO: do the conversion to Format here, so we can validate.
					trackConfigs[i].format = getContentAndTrackFormatPanel().getTrackControlPanel(i)
							.getAudioFormatPanel().getAudioFormat();
				}
			}
