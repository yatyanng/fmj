...
public class EntryPanel extends JPanel {
	...
	private void commit() {
		try {
			...
			case TYPE_CAPTURE_DEVICE:
				// add them as listed
				CaptureDeviceInfo[] cdi = new CaptureDeviceInfo[getEntryListModel().getSize()];
				for (int i = 0; i < getEntryListModel().getSize(); i++) {
					Object value = getEntryListModel().getElementAt(i);
				...
				for (int i = 0; i < deviceList.size(); i++) {
					CaptureDeviceManager.removeDevice((CaptureDeviceInfo) deviceList.elementAt(i));
				}
			...
			removeButton.addActionListener(new java.awt.event.ActionListener() {
				...
				public void actionPerformed(java.awt.event.ActionEvent e) {
					...
					for (int i = selectedIndices.length - 1; i >= 0; i--) {
						...
						Object entry = getEntryListModel().getElementAt(index);
						// remove plugin
						String value = (String) getEntryListModel().getElementAt(index);
						removeEntry(value);
					}
				}
			});
	...
	void load() {
		...
		switch (type) {
		case TYPE_DEMUX:
			list = PlugInManager.getPlugInList(null, null, PlugInManager.DEMULTIPLEXER);
		...
		case TYPE_EFFECT:
			list = PlugInManager.getPlugInList(null, null, PlugInManager.EFFECT);
			setTitle("Effect");
			break;

		case TYPE_RENDERER:
			list = PlugInManager.getPlugInList(null, null, PlugInManager.RENDERER);
			setTitle("Renderer");
			break;
	...
	private void updateDetailsPane(int selectedIndex, Object selectedValue) {
		...
		case TYPE_MUX: {
			...
			final Format[] inputFormats = PlugInManager.getSupportedInputFormats(plugIn, type);
			final Format[] outputFormats = PlugInManager.getSupportedOutputFormats(plugIn, type);
