...
public class ContainerPlayer {
		...
		public void controllerUpdate(ControllerEvent event) {
			...
			if (event instanceof RealizeCompleteEvent) {
				...
				notifyStatusListener(
						ContainerPlayerStatusListener.ERROR_PREFIX + ((ControllerErrorEvent) event).getMessage());
			...
			if (source.getProtocol().equals("file")) {
				...
				if (newUrl != null) {
					...
					if (!source.toExternalForm().equals(newSource.toExternalForm())) {
						logger.warning("Changing file URL to absolute for JMF, from " + source.toExternalForm() + " to "
								+ newSource);
						source = newSource;
					}
				}
	...
	public void setMediaLocation(String mediaLocation, boolean startAutomatically)
			throws NoDataSourceException, NoPlayerException, IOException {
	...
	/**
	 * Used in the case of things like RTP wizard, which create a player/processor
	 * explicitly.
	 */
