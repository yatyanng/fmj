...
public class PlayerPanel extends JPanel {
	...
	private JButton getOpenCaptureDeviceButton() {
		if (openCaptureDeviceButton == null) {
			...
			openCaptureDeviceButton.setIcon(new ImageIcon(getClass().getResource("/net/sf/fmj/ui/images/webcam.png"))); // TODO:
																														// different
																														// icon
	...
	private SwingLookControlPanel getTransportControlPanel() {
		if (transportControlPanel == null) {
			transportControlPanel = getContainerPlayer().getTransportControlPanel();
		}
	...
	private void setDefaultPrefs() {
		...
		prefs.recentUrls.add("http://fmj.larsontechnologies.com/samplemedia/Apollo_15_liftoff_from_inside_LM.ogg"); // Apollo
																													// 15
																													// movie
																													// from
																													// wikimedia
		...
		prefs.recentUrls.add("http://www.surfshooterhawaii.com//cgi-bin/axispush555.cgi?dummy=garb"); // sample
																										// IP
																										// surf
																										// cam,
																										// streaming
		...
		prefs.rtpTransmitWizardConfig.trackConfigs = new TrackConfig[] { new TrackConfig(true,
				new AudioFormat(AudioFormat.ULAW_RTP, 8000.0, 8, 1, AudioFormat.LITTLE_ENDIAN, AudioFormat.SIGNED)) };
		...
		prefs.transcodeWizardConfig.trackConfigs = new TrackConfig[] {
				new TrackConfig(true, new AudioFormat(AudioFormat.LINEAR, 8000.0, 8, 1, -1, AudioFormat.UNSIGNED)) };
		prefs.transcodeWizardConfig.destUrl = URLUtils
				.createUrlStr(new File(PathUtils.getTempPath(), "gulp2-transcoded.wav"));
		// TODO: format
