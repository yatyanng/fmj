...
public class NativeOggParser extends AbstractDemultiplexer {
		...
		public void readFrame(Buffer buffer) {
			synchronized (OGG_SYNC_OBJ) {
				...
				// && videobuf_time<=get_time()
					...
					final Buffer b = net.sf.fmj.media.util.ImageToBuffer.createBuffer(bi, format.getFrameRate());
	...
	private int audiofd_fragsize; /*
									 * read and write only complete fragments so that SNDCTL_DSP_GETOSPACE is
									 * accurate immediately after a bank switch
									 */
	...
	private ContentDescriptor[] supportedInputContentDescriptors = new ContentDescriptor[] {
			new ContentDescriptor("video.ogg"), new ContentDescriptor("audio.ogg"),
			new ContentDescriptor("application.ogg"), new ContentDescriptor("application.x_ogg"),
	...
	public static AudioFormat convertCodecAudioFormat(vorbis_info vi) {
		return new AudioFormat(AudioFormat.LINEAR, vi.rate.floatValue(), 16, vi.channels, AudioFormat.LITTLE_ENDIAN,
				AudioFormat.SIGNED);
	}
	 ...
	 * actually make use of the information in this example; a real player should
	 * attempt to perform color correction for whatever display device it supports.
	 */
	...
	private void nextAudioBuffer() throws IOException {
		synchronized (OGG_SYNC_OBJ) {
			...
			int j;
			/*
			 * we want a video and audio frame ready to go at all times. If we have to
			 * buffer incoming, buffer the compressed data (ie, let ogg do the buffering)
			 */
		...
		synchronized (OGG_SYNC_OBJ) {
			while (theora_p != 0 && videobuf_ready == 0) {
				...
				if (OGG.ogg_stream_packetout(to, op) > 0) {
					...
					if (videobuf_time == 0.0) { // TODO: for some reason, some videos, like
		...
		synchronized (OGG_SYNC_OBJ) {
			try {
				...
				while ((theora_p != 0 && theora_p < 3) || (vorbis_p != 0 && vorbis_p < 3)) {
					...
					/*
					 * The header pages/packets will arrive before anything else we care about, or
					 * the stream is not obeying spec
					 */
				...
				if (theora_p != 0) {
					...
					logger.info("Ogg logical stream " + Integer.toHexString(to.serialno.intValue()) + " is Theora "
							+ ti.width + "x" + ti.height + " " + fps + " fps");
					...
					if (ti.width != ti.frame_width || ti.height != ti.frame_height) {
						logger.warning("  Frame content is " + ti.frame_width + "x" + ti.frame_height + " with offset ("
				...
				if (vorbis_p != 0) {
					...
					logger.info("Ogg logical stream " + Integer.toHexString(vo.serialno.intValue()) + " is Vorbis "
							+ vi.channels + " channel " + vi.rate.intValue() + " Hz audio.");
				} else {
	...
	/**
	 * helper: push a page into the appropriate steam this can be done blindly; a
	 * stream won't accept a page that doesn't belong to it
	 */
