...
public class JavaOggParser extends AbstractDemultiplexer {
	...
	public static AudioFormat convertCodecAudioFormat(com.jcraft.jorbis.Info vi) {
		return new AudioFormat(AudioFormat.LINEAR, vi.rate, 16, vi.channels, AudioFormat.LITTLE_ENDIAN,
				AudioFormat.SIGNED);
	}
		...
		synchronized (OGG_SYNC_OBJ) {
			...
			try {
				...
				if (theora_p != 0) {
					...
					logger.info("Ogg logical stream " + Integer.toHexString(getSerialNo(to)) + " is Theora " + ti.width
							+ "x" + ti.height + " " + fps + " fps");
				...
				if (vorbis_p != 0) {
					...
					vb.init(vd);
					logger.info("Ogg logical stream " + Integer.toHexString(getSerialNo(vo)) + " is Vorbis "
							+ vi.channels + " channel " + vi.rate + " Hz audio.");
